...
if (isset($_POST['update'])) {
	...
	if ($query2) {
		...
		user_activity($empid, $file_name, 'Rejected TA updating', 'ADFM updating Rejected TA');
		echo "<script>alert('TA Updated Successfully...');</script>";
		echo "<script> window.location='Show_unclaimed_TA.php'; </script>";
	} else {
		$empid = $_SESSION['empid'];
		$file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
		user_activity($empid, $file_name, 'Rejected TA updating', 'ADFM unable to update Rejected TA');
