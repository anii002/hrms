...
function fetchEmployee1($id)
{
  ...
  $result = mysql_query($query);
  $value = mysql_fetch_array($result);
//   $qu=mysql_query("select empid from users where empid='$id' AND (role='12' OR role='13') ");
...
function generatesummary($selected_list_emp,$selected_list,$title,$description,$yy,$mm,$summary_id,$dept_id)
{
  ...
  // $query_summary = mysql_query("INSERT into tbl_summary (title,description, othe_dept) values('$title','$description', '1')");
    ...
    $query_summary = mysql_query("INSERT into master_summary (title,description,generated_date,forward_status,estcrk_status,month,year,summary_id,dept_id) values('".$title1."','".$description1."','".$d."','0','0','".$TAMonth."','".$TAYear."','".$summary_id."','".$dept_id."')");
    echo mysql_error();
    $id = mysql_insert_id();
    $cnt = 0;
