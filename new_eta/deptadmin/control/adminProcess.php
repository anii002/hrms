...
include('adminFunction.php');
  ...
  case 'activeUser':
    ...
    case 'fwESTCLERK':
        ...
        //echo mysql_error();
        $d_query=mysql_query("select dept_id from master_summary where summary_id='".$sumid."'");
        $d_row=mysql_fetch_array($d_query);
        
        if($d_row['dept_id'] == "13")
        {
            $u_query1="UPDATE master_summary SET pa_status='1',PA_approved_time='".$date."' where summary_id='".$sumid."' ";
            $result1 = mysql_query($u_query1);
        }
        ...
        $query_select = mysql_query("SELECT distinct(empid),reference_no FROM `tasummarydetails` WHERE summary_id='".$sumid."' ");
        ...
        {
            $bu_sql=mysql_query("SELECT BU FROM `employees` WHERE pfno='".$res['empid']."' ");
            $bu_row=mysql_fetch_array($bu_sql);
