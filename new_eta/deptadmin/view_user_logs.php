												...
												<div class="table-scrollable summary-table">
													<table id="example1" class="table table-hover table-bordered display">
														...
														<tbody>
															...
															$date = date('d/m/Y');
															$m = date('m');
															$query_emp = "SELECT empid,name,ip_address,file_name,action,message,created_at FROM `audit_table`, employees WHERE employees.pfno = audit_table.empid AND employees.dept='" . $_SESSION['dept'] . "' AND MONTH(created_at)='" . $m . "' ORDER BY audit_table.id DESC";
