...
if(isset($_POST['submit']))
{
	   ...
	   //	echo "<br>";
		...
		$total_otherp_amt=($row['otherp_amt']+$otherp_amt)-$otherp_amt1;
			...
			else
			{
							...
							if($lm=="0")
							{
								for($k=0; $k<$total_rows; $k++)
								{
									...
									if($mths[$mcnt]==$split_month2)
									{
										...
										$duplicate_query="SELECT empid, reference_no, taDate, departT, arrivalT, percent,amount FROM taentrydetails WHERE empid='".$_SESSION['empid']."' AND reference_no='".$reference."' AND taDate='".$date."' order by id desc limit 1";
            						 $result=mysql_query($duplicate_query);
            						 $fet=mysql_fetch_array($result);
            						 $get_per=$fet['percent'];
            						 if($get_per=="30%" || $get_per=="30"){
            						...
            						{
            							// Update Previous Date Calculation
            							 $p_update="UPDATE `taentrydetails` SET `percent`='0',`amount`='0' WHERE `reference_no`='".$reference."' and empid='".$_SESSION['empid']."' and taDate='".$date."'";
            							$p_res=mysql_query($p_update);
            							$time_array=array();
            							$result=mysql_query($duplicate_query);
            							while ($fet_data=mysql_fetch_array($result)){
            							...
            							$t_time=second_to_hhmm($time);
            							//echo strtotime($t_time);
            							//echo $t_time="0".$t_time;
            							if(strtotime($t_time) <= strtotime("06:00")){
            								 $per="30%";
            								 $amt=$u_amount*0.3;	
            							}
                ...
                user_activity($empid,$file_name,'Add more TA','DA add more TA with another month');
		 	}
		}
	...
	else
	{
		...
		$empid=$_SESSION['empid'];
		$file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
        user_activity($empid,$file_name,'Add more TA','DA add more TA in existing month.');
	}
