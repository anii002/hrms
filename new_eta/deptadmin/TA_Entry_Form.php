		...
		<div class="portlet box blue">
			...
			<div class="portlet-body form">
				...
				<form action="taprocess.php" method="POST" autocomplete="off">
					...
					<input type="hidden" name="u_pfn" id="u_pfn" value="<?php echo $_SESSION['empid']; ?>" class="form-control">
...
<script type="text/javascript">
	...
	$(document).on("change", "#month", function() {
		...
		$.ajax({
			...
			success: function(data) {
				var str = data.trim();
				if (str === 'claimed') {
					alert("You have already claimed TA for the month of " + month);
					window.location = 'index.php';
				} else if (str != 0) {
					alert("You have already saved TA for the month of " + month + ". Please fill TA from the ADD MORE OPTION in Saved TA.");
	...
	$(".addrowbtn").on("click", function() {
		...
		data += '<tr class="odd gradeX">' +
			...
			'<label class="control-label">Date <br> दिनांक </label>' +
			'<div><input class="form-control datepicker" readonly type="text" name="date' + sr + '" id="date' + sr + '" val=' + sr + ' value=' + prevdate + ' placeholder="dd/mm/yyyy"></div>' +
			'</div></td>' +
			'<td style="width: 10%"><div class="form-group">' +
			'<label class="control-label">Journey Type <br> यात्रा का प्रकार</label>' +
			'<div><select class="form-control j_type" name="type' + sr + '" id="type' + sr + '" val=' + sr + '> <?php $query1 = "SELECT * FROM `journey_type_master`";
			...
			'<td style="width: 10%"><div class="form-group">' +
			'<label class="control-label">Train No. <br> गाडी नं. </label>' +
			'<input type="text" class="form-control val train_no" placeholder="Train No." name="trainno' + sr + '" id="trainno' + sr + '" val=' + sr + '>' +
			'</div></td>' +
			'<td style="width: 10%"><div class="form-group">' +
			'<label class="control-label">Other <br> अन्य</label>' +
			'<div><select class="form-control purpose" val=' + sr + ' name="other' + sr + '" id="other' + sr + '"> <?php $query2 = "SELECT * FROM `journey_purpose_master`";
																													$sql2 = mysqli_query($conn, $query2);
																													while ($row2 = mysqli_fetch_array($sql2)) {
																														echo "<option value=" . $row2["id"] . ">" . $row2["journey_purpose"] . "</option>";
																													} ?> </select></div>' +
			'</div></td>' +
			'<td style="width: 8%"><div class="form-group">' +
			'<label class="control-label">Depart Time <br> प्रस्थान समय</label>' +
			'<input type="text" name="dtime' + sr + '" val=' + sr + ' id="dtime' + sr + '" class="form-control changedtime timevalue" placeholder="hh:mm">' +
			'</div></td>' +
			'<td style="width: 8%"><div class="form-group">' +
			'<label class="control-label">Arri. Time <br> आगमन समय</label>' +
			'<input type="text" class="form-control ta_calculation time_val timevalue" name="atime' + sr + '" val=' + sr + ' id="atime' + sr + '" placeholder="hh:mm">' +
			'</div></td>' +
			'<td style="width: 12%"><div class="form-group">' +
			'<label class="control-label">Depart Station <br> प्रस्थान स्टेशन</label>' +
			'<input type="text" list="dstation' + sr + '" style="text-transform:uppercase" name="dstn' + sr + '" id="dstn' + sr + '" placeholder="select Station" val=' + sr + ' class="departClass form-control"><datalist id="dstation' + sr + '"> <?php $sql = "SELECT stationdesc FROM station";
																																																														$query = mysqli_query($conn,$sql);
																																																														while ($row = mysqli_fetch_array($query)) {
																																																															echo "<option value='" . $row["stationdesc"] . "'>";
																																																														} ?> </datalist>' +
			...
			'<td style="width: 12%"><div class="form-group">' +
			'<label class="control-label">Arri. Station <br> आगमन स्टेशन</label>' +
			'<input type="text" list="astation' + sr + '" style="text-transform:uppercase" name="astn' + sr + '" id="astn' + sr + '" placeholder="select Station" val=' + sr + ' class="form-control arrivalstn"><datalist id="astation' + sr + '"> <?php $sql = "SELECT stationdesc FROM station";
																																																													$query = mysqli_query($conn,$sql);
																																																													while ($row = mysqli_fetch_array($query)) {
																																																														echo "<option value='" . $row["stationdesc"] . "'>";
																																																													} ?> </datalist>' +
			...
			'<label class="control-label">Distance <br> दुरी</label>' +
			'<input type="text" class="form-control" name="distance' + sr + '" id="distance' + sr + '" placeholder="Distance" val=' + sr + '>' +
			'</div></td>' +
			'<td><div class="form-group">' +
			'<label class="control-label">Percentage <br> प्रतिशत</label>' +
			'<input type="text" class="form-control changeper" name="per' + sr + '" id="per' + sr + '" placeholder="Percentage" readonly val=' + sr + '>' +
			'</div></td>' +
			'<td><div class="form-group">' +
			'<label class="control-label">Amount <br> राशि</label>' +
			'<input type="text" class="form-control" name="amt' + sr + '" id="amt' + sr + '" placeholder="Amount" readonly val=' + sr + '>' +
			'</div></td>' +
			'</tr>';
