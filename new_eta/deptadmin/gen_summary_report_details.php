...
$query = mysqli_query($conn,"SELECT title,description FROM `master_summary` WHERE summary_id='" . $_GET['sum_id'] . "' ");
$rows_summary = mysqli_fetch_array($query);
?>
...
// Check if 'sum_id' is set and sanitize input
if (isset($_GET['sum_id'])) {
    $sum_id = sanitize_input($_GET['sum_id']);

    // Ensure $_SESSION['dept'] is set and sanitize input
    if (isset($_SESSION['dept'])) {
        $dept = sanitize_input($_SESSION['dept']);
        ...
        $sql = "SELECT * FROM tasummarydetails 
                INNER JOIN employees ON tasummarydetails.empid = employees.pfno 
                WHERE summary_id = ? 
        ...
        mysqli_stmt_bind_param($stmt, "ss", $sum_id, $dept);
        ...
        while ($val = mysqli_fetch_array($res)) {
            ...
            $sql1 = "SELECT * FROM employees WHERE pfno = ?";
            $stmt1 = mysqli_prepare($conn, $sql1);
            mysqli_stmt_bind_param($stmt1, "s", $val['empid']);
            ...
            mysqli_stmt_bind_param($stmt2, "ii", $val1['level'], $val1['level']);
            ...
            // Fetch additional contributions if available
            $sqll = "SELECT SUM(amount) as amount FROM add_cont WHERE empid = ? AND reference_no = ?";
            $stmt3 = mysqli_prepare($conn, $sqll);
            mysqli_stmt_bind_param($stmt3, "ss", $val['empid'], $val['reference_no']);
            ...
            echo "
                <tr class='fontcss1'>
                    ...
                    <td>$amount</td>
                    <td>{$mon} - {$val['year']}</td>
                    <td>{$mmon} - " . substr($val['created_at'], 8, 2) . "</td>";
...
?>
								...
								<input type='hidden' id='summary_desc' value='<?php echo $rows_summary['description'];; ?>'>
							</div>
						</div>
					</div>
				</form>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var summary_desc = $("#summary_desc").val();
							...
							return {
								columns: [
									...
									},
									{
										...
										italics: true,
										bold: true,
										text: 'Summary Description: ' + summary_desc,
										fontSize: 10,
										margin: [10, 0]
									},
									{
