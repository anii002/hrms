...
function get_summary($sum_id)
{
    ...
    // Fetch summary details and employees data
    $sql = "SELECT tasummarydetails.*, employees.pfno, employees.BU, employees.level, employees.name, employees.bp, employees.desig 
    ...
    while ($val = $res->fetch_assoc()) {
        // Fetch level-based amount
        $level = $val['level'];
        $sql2 = "SELECT amount FROM ta_amount WHERE ? BETWEEN min AND max";
        $stmt2 = $conn->prepare($sql2);
        $stmt2->bind_param("i", $level);
        $stmt2->execute();
        ...
        $s = $mts[0];
        $y = substr($val['year'], 2);
        $mon = isset($month_array[$s]) ? $month_array[$s] : '';

        // Format creation date
        $cdate = substr($val['created_at'], 3, 2);
        $cyear = substr($val['created_at'], 8, 2);
        $mmon = isset($month_array[$cdate]) ? $month_array[$cdate] : '';

        // Fetch approval status
        $query1 = "SELECT est_approve FROM taentry_master WHERE empid = ? AND reference_no = ?";
        $stmt3 = $conn->prepare($query1);
        $stmt3->bind_param("ss", $val['empid'], $val['reference_no']);
        $stmt3->execute();
        ...
        $total_amt = 0;
        $total_amt += ($val['30p_cnt'] == '0' && $val['30p_amt'] == '0') ? $data .= "<td>-</td><td>-</td>" : $data .= "<td>" . $val['30p_cnt'] . "</td><td>" . $val['30p_amt'] . "</td>";
        $total_amt += ($val['70p_cnt'] == '0' && $val['70p_amt'] == '0') ? $data .= "<td>-</td><td>-</td>" : $data .= "<td>" . $val['70p_cnt'] . "</td><td>" . $val['70p_amt'] . "</td>";
        $total_amt += ($val['100p_cnt'] == '0' && $val['100p_amt'] == '0') ? $data .= "<td>-</td><td>-</td>" : $data .= "<td>" . $val['100p_cnt'] . "</td><td>" . $val['100p_amt'] . "</td>";

        $temp += $total_amt;

        $data .= "<td>$total_amt</td>";

        // Fetch additional amount
        $sqll = "SELECT SUM(amount) as amount FROM add_cont WHERE empid = ? AND reference_no = ?";
        $stmt4 = $conn->prepare($sqll);
        $stmt4->bind_param("ss", $val['empid'], $val['reference_no']);
        $stmt4->execute();
        $ress = $stmt4->get_result();
        $row = $ress->fetch_assoc();
        $additional_amt = $row['amount'];

        $temp1 += $additional_amt;

        $data .= ($additional_amt == '0' || $additional_amt == null) ? "<td>-</td>" : "<td>$additional_amt</td>";
