						...
						<div class="portlet-body">
								...
								<table id="example" class="table table-hover table-bordered display">
									...
									<tbody>
										...
										$vett_emp_query=mysql_query("SELECT taentry_master.* from employees,taentry_master where employees.pfno=taentry_master.empid AND taentry_master.est_approve='1' AND taentry_master.forward_status='1' AND dept='".$_GET['dept']."' AND BU='".$_GET['bu']."' AND TAMonth='".$_GET['mon']."' AND TAYear='".$_GET['year']."' ");
    	                                $sr=0;
                                	   // $for_pa_emp_cnt++;
                            	        while ($row = mysql_fetch_array($vett_emp_query)) 
									    {   
										    $sr++;
