...
switch ($_REQUEST['action']) {
  ...
  case 'deleteclaim':
    ...
    if ($total_rows > 1) {
      ...
      $sql2 = mysqli_query($conn, "SELECT empid,`30p_cnt`,`30p_amt`,`70p_cnt`,`70p_amt`,`100p_cnt`,`100p_amt`,`otherp_cnt`,`otherp_amt` FROM `tasummarydetails` WHERE `reference_no`='" . $reference . "' ");
      ...
      $tasummary_sql1 = mysqli_query($conn, "SELECT empid FROM tasummarydetails WHERE reference_no = '" . $reference . "' ");
      $res2 = mysqli_fetch_array($tasummary_sql1);
      $empid = $res2['empid'];
  ...
  case 'delcont':
    ...
    $query = mysqli_query($conn, "SELECT empid from master_cont where reference_no='" . $reference . "' ");
