    ...
    <div class="page-content">
        ...
        <div class="portlet box blue">
            ...
            <div class="portlet-body form">
                ...
                <form action="taprocess.php" autocomplete="off" method="POST">
                    ...
                    <input type="hidden" name="u_amount" id="u_amount" value="<?php echo $user_amount; ?>"
                        class="form-control">
                    <input type="hidden" name="u_pfno" id="u_pfno" value="<?php echo $_SESSION['empid']; ?>"
                        class="form-control">
                        ...
                        <div class="row add-train-title ">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" type="text" placeholder="Enter Token / Card Pass"
                                        name="cardpass" id="cardpass" required>
                                </div>
                            </div>
                                            ...
                                            <tr class="odd gradeX">
                                                <td style="width: 10%">
                                                    <div class="form-group ">
                                                        ...
                                                        <div class="">
                                                            <!--<input class="form-control datepicker_1" type="text" name="date0" id="date0" val='0' placeholder ="dd/mm/yyyy">-->
                                                            <input type="text"
                                                                class=" form-control datepicker datecheck" val='0'
                                                                id="date0" name="date0" placeholder="dd/mm/yyyy">
                                                        </div>
                                                    </div>
                                                </td>
                                                <td style="width: 10%">
...
$(document).ready(function() {
    // Function to handle time validation and focus shift
    ...
    // Function to handle journey type change and cardpass requirement
    ...
    function addRow() {
        ...
        data += createTableCell(createInputGroup(createInput('text', 'datepicker datecheck', 'date' + sr,
            'date' + sr, prevdate, sr, 'dd/mm/yyyy')));
        data += createTableCell(createInputGroup(createJourneyTypeSelect(sr)));
        data += createTableCell(createInputGroup(createInput('text', 'form-control val train_no', 'trainno' +
            sr, 'trainno' + sr, '', sr, 'Train No.')));
        data += createTableCell(createInputGroup(createPurposeSelect(sr)));
        data += createTableCell(createInputGroup(createInput('text', 'form-control changedtime timevalue',
            'dtime' + sr, 'dtime' + sr, '', sr, 'hh:mm')));
        data += createTableCell(createInputGroup(createInput('text',
            'form-control ta_calculation time_val timevalue', 'atime' + sr, 'atime' + sr, '', sr,
            'hh:mm')));
        data += createTableCell(createInputGroup(createStationInput('dstn' + sr, sr)));
        data += createTableCell(createInputGroup(createStationInput('astn' + sr, sr)));
        data += createTableCell(createInputGroup(createInput('text', 'form-control', 'distance' + sr,
            'distance' + sr, '', sr, 'Distance')));
        data += createTableCell(createInputGroup(createInput('text', 'form-control changeper', 'per' + sr,
            'per' + sr, '', sr, 'Percentage', true)));
        data += createTableCell(createInputGroup(createInput('text', 'form-control', 'amt' + sr, 'amt' + sr, '',
            sr, 'Amount', true)));
        data += '</tr>';
        ...
        updatePreviousStation(sr);
        setUpDatePicker();
    }

    // Helper functions to create input elements and groups
    function createTableCell(content) {
        return '<td style="width: 10%">' + content + '</td>';
    }

    function createInputGroup(content) {
        return '<div class="form-group"><div class="">' + content + '</div></div>';
    }

    function createInput(type, className, name, id, value = '', val = '', placeholder = '', readonly = false) {
        var readOnlyAttr = readonly ? 'readonly' : '';
        return '<input type="' + type + '" class="' + className + '" name="' + name + '" id="' + id +
            '" val="' + val + '" value="' + value + '" placeholder="' + placeholder + '" ' + readOnlyAttr + '>';
    }

    function createJourneyTypeSelect(sr) {
        var options = '<?php $query1 = "SELECT * FROM `journey_type_master`";
							$sql1 = mysqli_query($conn, $query1);
							while ($row1 = mysqli_fetch_array($sql1)) {
								echo '<option value=' . $row1['id'] . ' >' . $row1['journey_type'] . '  </option>';
							} ?>';
        return '<select class="form-control j_type" name="type' + sr + '" id="type' + sr + '" val="' + sr +
            '">' + options + '</select>';
    }

    function createPurposeSelect(sr) {
        var options = '<?php $query2 = "SELECT * FROM `journey_purpose_master`";
							$sql2 = mysqli_query($conn, $sql2);
							while ($row2 = mysqli_fetch_array($sql2)) {
								echo '<option value=' . $row2['id'] . ' >' . $row2['journey_purpose'] . '  </option>';
							} ?>';
        return '<select class="form-control purpose" name="other' + sr + '" id="other' + sr + '" val="' + sr +
            '">' + options + '</select>';
    }

    function createStationInput(id, sr) {
        var options = '<?php $sql = "SELECT stationdesc FROM station";
							$query = mysqli_query($conn,$sql);
							while ($row = mysqli_fetch_array($query)) {
								echo "<option value=\'" . $row['stationdesc'] . "\'>";
							} ?>';
        return '<input type="text" list="' + id + '" style="text-transform:uppercase" name="' + id + '" id="' +
            id + '" placeholder="select Station" val="' + sr +
            '" class="departClass form-control"><datalist id="' + id + '">' + options + '</datalist>';
    }

    // Function to update previous station
    function updatePreviousStation(sr) {
        var sr_1 = sr - 1;
        var prev_dstn = $("#astn" + sr_1).val();
        $("#dstn" + sr).val(prev_dstn);
    }

    // Function to setup date picker with min and max date
    function setUpDatePicker() {
        var x = 60; // or whatever offset
        var currentDate = new Date();
        currentDate.setDate(currentDate.getDate() - x);
        var minDate = '01/' + (currentDate.getMonth() + 1) + '/' + currentDate.getFullYear();

        currentDate = new Date();
        var maxDate = currentDate.getDate() + '/' + (currentDate.getMonth() + 1) + '/' + currentDate
            .getFullYear();
    ...
    // Event listeners
    $(document).on("change", ".changedtime", handleTimeChange);
    $(document).on("change", ".j_type", handleJourneyTypeChange);
    $(".addrowbtn").on("click", addRow);

    // Initial setup for date pickers
    setUpDatePicker();
});
...
$(function() {
    ...
    var CurrentDate = new Date();
    var mon2 = CurrentDate.getMonth() + 1;
    // max date
    var CurrentDate = new Date();
