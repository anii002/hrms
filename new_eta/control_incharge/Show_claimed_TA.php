...
<div class="page-content-wrapper">
    ...
    while ($row = mysqli_fetch_array($sql1)) {
        $query2 = "SELECT SUM(amount) AS total_amount, SUM(distance) AS distance FROM `taentrydetails` WHERE empid='" . $_SESSION['empid'] . "' AND reference_no='" . $row['reference_no'] . "'";
    ...
    ?>
        <tr>
            ...
            <td><?php echo isset($row2['distance']) ? htmlspecialchars($row2['distance']) : '0'; ?></td>
            <td><?php echo htmlspecialchars($total_amount); ?></td>
            <td><?php echo htmlspecialchars($row['created_date']); ?></td>
            <td><a href="claimed_details_1.php?ref_no=<?php echo htmlspecialchars($row['reference_no']); ?>" class="btn green btn_action">Show</a></td>
        </tr>
    <?php
    }
    ?>
