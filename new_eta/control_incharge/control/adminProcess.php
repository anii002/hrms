...
switch ($_REQUEST['action']) {
    ...
    case 'rejectTA_CI_PM':
        ...
        if (curl_errno($ch)) {
            ...
            if (mysqli_affected_rows($conn) >= 0) {
                ...
                $msg = 'CI rejected ' . $pmempid . ' employee TA';
                ...
                $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg = 'CI unable to reject ' . $pmempid . ' employee TA';
                user_activity($empid, $file_name, 'Forward TA', $msg);
    ...
    case 'delcont':
        ...
        if (mysqli_affected_rows($conn) >= 0) {
            ...
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid, $file_name, 'Delete Contingency', 'CI delete contingency with TA');

            echo "<script>alert('Contigency deleted successfully'); window.location='../Show_unclaimed_TA.php';</script>";
        } else {
            $empid = $_SESSION['empid'];
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid, $file_name, 'Delete Contingency', 'CI unable to delete contingency with TA');
    ...
    case 'updateGrievance':
        ...
        if ($sql) {
            $empid = $_SESSION['empid'];
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid, $file_name, 'Update Grievance in TA', 'CI updated  the Grievance');
            $data = "1";
        } else {
            $empid = $_SESSION['empid'];
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid, $file_name, 'Update Grievance in TA', 'CI Not Updated the grievance');
            $data = "0";
        }
        echo $data;
        break;
    ...
    case 'forward_TA':
        ...
        if ($result['otp'] == $c_otp) {
            ...
            if ($c_otp) {
                ...
                $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg = 'CI TA has been forwarded to CO ' . $forwardName . '';
                user_activity($empid, $file_name, 'Forward TA', $msg);
            } else {
                $flag = '0';

                $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg = 'CI TA has been NOT forwarded to CO ' . $forwardName . '';
                user_activity($empid, $file_name, 'Forward TA', $msg);
            }
        } else {
            $flag = '-1';
            ...
            // $empid=$_SESSION['empid'];
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid, $file_name, 'Forward TA', 'CI OTP Not matched');
        }
        echo $flag;
    ...
    case 'deleteclaim':
        ...
        if ($total_rows > 1) {
            ...
            if (mysqli_affected_rows($conn) >= 0) {
                ...
                $empid = $emp_id_dt;
                $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
                user_activity($empid, $file_name, 'Delete TA', 'CI delete the single TA');

                echo "<script>alert('Claim deleted successfully'); window.location='../Show_unclaimed_TA.php';</script>";
            } else {
                $empid = $emp_id_dt;
                $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
                user_activity($empid, $file_name, 'Delete TA', 'CI unable to delete the single TA');
            ...
            if (mysqli_affected_rows($conn) >= 0) {
                ...
                $empid = $emp_id_dt;
                $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
                user_activity($empid, $file_name, 'Delete TA', 'CI delete the overall TA');

                echo "<script>alert('Claim deleted successfully'); window.location='../Show_unclaimed_TA.php';</script>";
            } else {
                $empid = $emp_id_dt;
                $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
                user_activity($empid, $file_name, 'Delete TA', 'CI unable to delete the overall TA');
    ...
    case 'approveAndForward':
        ...
        if (approveAndForward($empid, $ref, $forwardName, $original_id)) {
            // $empid=$_SESSION['empid'];
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg = 'CI forward ' . $original_id . ' employee TA to ' . $forwardName . ' CO';
            user_activity($empid, $file_name, 'Forward TA', $msg);
            ...
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg = 'CI unable to forward ' . $original_id . ' employee TA to ' . $forwardName . ' CO';
            user_activity($empid, $file_name, 'Forward TA', $msg);
    ...
    case 'deletecont':
        ...
        if ($result1 && $result2) {
            $empid = $_SESSION['empid'];
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid, $file_name, 'Delete Contingency', 'CI Contingency deleted successfully');
            echo "<script>alert('Contingency deleted successfully'); window.location='../show_saved_cont.php';</script>";
        } else {
            $empid = $_SESSION['empid'];
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid, $file_name, 'Delete Contingency', 'CI unable to delte the Contingency');
    ...
    case 'updateEmployee1':
        ...
        if (updateEmployee($empid, $billunit, $panno, $fullname, $design, $station, $mobile, $email, $category, $dept, $txtworkdepot, $txtbasicpay, $gradepay, $dob_date, $app_date, $level, $forwardName, $isupdated)) {
            // $empid=$_SESSION['empid'];
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg = 'CI send request to ' . $forwardName . ' DA for upadte details';
            user_activity($empid, $file_name, 'Update Details', $msg);
            echo "1";
        } else {
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg = 'CI unable send request to ' . $forwardName . ' DA for upadte details';
            user_activity($empid, $file_name, 'Update Details', $msg);
            echo "0";
        }
    ...
    case 'updateEmpData':
        ...
        if (updateEmpData($id, $pfno, $billunit, $panno, $empname, $designation, $station, $mobile, $email, $catg, $dept, $depot_id, $bp, $gp, $bdate, $apdate, $level)) {
            $empid = $_SESSION['empid'];
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg = 'CI updated the ' . $pfno . ' employee details';
            user_activity($empid, $file_name, 'Update Employee Data', $msg);
            echo "<script>alert('User Data Updated successfully');window.location='../update_emp.php';</script>";
        } else {
            $empid = $_SESSION['empid'];
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg = 'CI failed to updated the ' . $pfno . ' employee details';
            user_activity($empid, $file_name, 'Update Employee Data', $msg);
        ...
        if ($forwardName) {
            ...
            if ($count > 0) {
                ...
                if (curl_errno($ch)) {
                    ...
                    $empid = $_SESSION['empid'];
                    $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
                    user_activity($empid, $file_name, 'Forward TA', 'CI, Send the OTP to employee for Forwarding the TA');
                }
                curl_close($ch);
            }
        } else {
            ...
            $empid = $_SESSION['empid'];
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid, $file_name, 'Forward TA', 'CI, Not Send the OTP to employee bcz mobile number not found for Forwarding the TA');
        }
        echo $flag;
    ...
    case 'forward_con':
        ...
        if ($query && $result) {
            $empid = $empid_session;
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg = 'CI forwarded ' . $empid . ' employee Contingency has been forwarded to CO ' . $forwardName . ' success';
            ...
            $empid = $_SESSION['empid'];
            $file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg = 'CI unable to forward ' . $empid . ' employee Contingency to CO ' . $forwardName . ' failed';
            user_activity($empid, $file_name, 'Forward Contingency', $msg);
