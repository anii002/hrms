				...
				<div class="dashboard-stat green-haze">
					...
					<div class="details">
						<div class="number">
							...
							$total = 0;
							while ($resultset = mysqli_fetch_array($query)) {
								$reference_no = $resultset['reference_no'];

								// Check if $reference_no is not null or empty
								if (!empty($reference_no)) {
									$sql = mysqli_query($conn, "SELECT summary_id FROM tasummarydetails WHERE is_summary_generated=1 AND reference_no='" . $reference_no . "' AND empid='" . $_SESSION['empid'] . "'");
									$resultset1 = mysqli_fetch_array($sql);

									if ($resultset1 && $resultset1['summary_id'] != 0) {
										...
										if ($resultset2) {
											$total += $resultset2['total'];
										}
									} else {
										$sql = mysqli_query($conn, "SELECT count(reference_no) AS total FROM tasummarydetails WHERE is_summary_generated=0 AND reference_no='" . $reference_no . "' AND empid='" . $_SESSION['empid'] . "'");
										$resultset1 = mysqli_fetch_array($sql);
										if ($resultset1) {
											$total += $resultset1['total'];
										}
									}
								}
							}
				...
				<div class="dashboard-stat blue-madison">
					...
					<div class="details">
						<div class="number">
							<?php
							$dep = $_SESSION['dept'];
							$result_emp = mysqli_query($conn, "SELECT `id`,`pfno`, `name`, `desig`,`station`, `mobile`, `email`,`dept`,`first_login`, `depot_id`,`status` FROM `employees` WHERE dept='" . $dep . "' ORDER BY id desc");
							$count = mysqli_num_rows($result_emp);

							echo $count;
							?>
						</div>
						<div class="desc">
							<p>पंजीकृत कर्मचारी /<br> Registered Employees</p>
						</div>
					</div>
					<a class="more" href="verify_emp.php">
						अधिक जानकारी / View more <i class="m-icon-swapright m-icon-white"></i>
					</a>
				</div>
			</div>
