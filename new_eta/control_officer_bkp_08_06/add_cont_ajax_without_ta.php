...
$time = date("Y/m/d")." ".date("h:i:sa");
 ...
 $insert_master_sql = "INSERT INTO `continjency_master`(`reference`, `month`, `year`, `empid`, `total_amount`, `set_number`) VALUES ('".$reference_no."','".$month."','".$year."','".$_SESSION['empid']."','".$sum."','0')";
...
if($result)
{
	...
	{
		$insert_sql = "INSERT INTO `continjency`(`cntdate`, `cntfrom`, `cntTo`, `kms`, `rate_per_km`, `total_amount`, `set_number`, `objective`, `cid`) VALUES ('".$_POST['date'.$i]."','".$_POST['from'.$i]."','".$_POST['to'.$i]."','".$_POST['kms'.$i]."', '".$_POST['rate'.$i]."', '".$_POST['total'.$i]."','0','".$_POST['objective']."','".$id1."')";
...
if($res == $total_rows){
	echo "<script>alert('Contigency bills are added');window.location='add_cont_without_ta.php';</script>";
}
