...
if($res == $total_rows){
    ...
    $user_activity($empid,$file_name,'Save Contingency','SO save the Contingency');
	echo "<script>alert('Contigency bills are added');window.location='show_saved_cont.php';</script>";
}
else{
    $empid=$_SESSION['empid'];
    $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
    $user_activity($empid,$file_name,'Save Contingency','SO unable to save the Contingency');
