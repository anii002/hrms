								...
								<table id="example1" class="table table-hover table-bordered display">
									...
									<tbody>
										<?php
        				                    ...
        				                    // $result_emp = mysqli_query("SELECT `id`,`pfno`, `name`, `desig`,`station`, `mobile`, `email`,`dept`, `depot_id`,`status` FROM `employees` WHERE dept='".$dep."' AND BU='' AND unit=''  ORDER BY id desc");
