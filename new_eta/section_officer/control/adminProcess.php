...
include('function.php');
  ...
  break;
        ...
        else
        {
            ...
            if(mysqli_affected_rows($conn) >= 0)
            {
              $drop="DELETE from forward_data where empid='".$pmempid."' AND reference_id='".$ref_no."' ";
                ...
                $msg='SO reject '.$pmempid.' Acct employee TA';
                user_activity($ciempid,$file_name,'Forward TA',$msg);
               echo "<script>alert('Claim rejected successfully...'); window.location='../ci_Show_claimed_TA.php';</script>";
            }
            else
            {
                $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg='SO unable to reject '.$pmempid.' Acct employee TA';
                user_activity($ciempid,$file_name,'Forward TA',$msg);
               echo "<script>alert('Something Wrong'); window.location='../ci_Show_claimed_TA.php';</script>";
            }
        }

    break;
    
    case 'rejectCont_CI_PM':
        $ciempid=$_POST['ciempid'];
        $pmempid=$_POST['pmempid'];
        $ref_no=$_POST['ref_no'];
        $reason=$_POST['reason'];
        ...
        if(mysqli_affected_rows($conn) >= 0)
        {
            ...
            $result1=mysqli_query($conn,$drop);
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SO reject '.$pmempid.' Acct employee contingency';
            user_activity($ciempid,$file_name,'Reject contingency',$msg);
           echo "<script>alert('Claim rejected successfully...'); window.location='../approved_list_cont.php';</script>";
        }
        else
        {
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SO unable to reject '.$pmempid.' Acct employee contingency';
            user_activity($ciempid,$file_name,'Reject contingency',$msg);
           echo "<script>alert('Something Wrong'); window.location='../approved_list_cont.php';</script>";
        }
    ...
    case 'deletecont':
        ...
        if($result1 && $result2)
        {
            ...
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid,$file_name,'Delete Contingency','SO delete the Contingency');
        ...
        else
        {
            $empid=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid,$file_name,'Delete Contingency','SO unable to delete the Contingency');
  ...
  break;
    ...
    case 'delcont':
        ...
        if(mysqli_affected_rows($conn) >= 0)
        {
          ...
          $res = mysqli_query($conn,$query_mastcont);
            $empid=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid,$file_name,'Delete Contingency','SO delete the Contingency with TA');
        ...
        else
        {
            $empid=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid,$file_name,'Delete Contingency','SO unable to delete the Contingency with TA');
    ...
    case 'updateGrievance':
        ...
        $data="0";
        $sql=mysqli_query($conn,"UPDATE `grievance` SET `remark`='".$description."',`updated_date`='".$date."' WHERE id='".$id."' ");
        if($sql)
        {
            $data="1"; 
        }
           ...
           if($month_array[$s])
            {
              ...
              if($result['otp'] == $c_otp)
              {
                ...
                if($c_otp)
                {
                    ...
                    $flag='1';
                    $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                    $msg='SO forward TA  to '.$forwardName.' SOA';
                    user_activity($empid,$file_name,'Forward TA',$msg);
                  }
                  else{
                    $flag='0';
                    $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                    $msg='SO unable to forward TA  to '.$forwardName.' SOA';
                    user_activity($empid,$file_name,'Forward TA',$msg);
                  }
              }
              else
              {
                $flag='-1';
                $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg='SO, OTP not matched for forward TA';
                user_activity($empid,$file_name,'Forward TA',$msg);
              }
              echo $flag;
   ...
   case 'deleteclaim':
    ...
    if($total_rows > 1)
    {
        ...
        // echo "<br><br>"."SELECT `30p_cnt`,`30p_amt`,`70p_cnt`,`70p_amt`,`100p_cnt`,`100p_amt`,`otherp_cnt`,`otherp_amt` FROM `tasummarydetails` WHERE `reference_no`='".$reference."' AND `empid`='".$emp_id_dt."' ";

        $sql2=mysqli_query($conn,"SELECT `30p_cnt`,`30p_amt`,`70p_cnt`,`70p_amt`,`100p_cnt`,`100p_amt`,`otherp_cnt`,`otherp_amt` FROM `tasummarydetails` WHERE `reference_no`='".$reference."' AND `empid`='".$emp_id_dt."' ");
        ...
        if(mysqli_affected_rows($conn) >= 0)
        {
            ...
            $empid=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid,$file_name,'Delete TA','SO deleted the single TA');
        ...
        else
        {
            $empid=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid,$file_name,'Delete TA','SO unable to delete the single TA');
          echo "<script>alert('Something went wrong');window.location='../Show_unclaimed_TA.php';</script>";
        }
    ...
    {
      // echo "HI";
        $query_delete = "DELETE from taentry_master where reference_no='".$reference."' AND empid='".$emp_id_dt."' ";

        $result = mysqli_query($conn,$query_delete);
        if(mysqli_affected_rows($conn) >= 0)
        {
          $taentry_sql = "DELETE FROM taentrydetails WHERE reference_no = '".$reference."' AND empid = '".$emp_id_dt."' ";
          $res = mysqli_query($conn,$taentry_sql);
          $tasummary_sql = "DELETE FROM tasummarydetails WHERE reference_no = '".$reference."' AND empid = '".$emp_id_dt."' ";
          $res1 = mysqli_query($conn,$tasummary_sql);
            $empid=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid,$file_name,'Delete TA','SO deleted the overall TA');
          echo "<script>alert('Claim deleted successfully'); window.location='../Show_unclaimed_TA.php';</script>";
        }
        else
        {
            $empid=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid,$file_name,'Delete TA','SO unable to delete the overall TA');
          echo "<script>alert('Something went wrong');window.location='../Show_unclaimed_TA.php';</script>";
        }
  ...
  break;
    ...
    case 'approveAndForward':
        ...
        $data=0;
          ...
          {
                $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg='SO forward '.$original_id.' Acct employee TA to '.$forwardName.' SOA';
                user_activity($empid,$file_name,'Forward TA',$msg);
          ...
          {
              $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg='SO unable to forward '.$original_id.' Acct employee TA to '.$forwardName.' SOA';
                user_activity($empid,$file_name,'Forward TA',$msg);
            echo "<script>alert('Something went wrong');window.location='../ci_Show_claimed_TA.php';</script>";
            $data=0;
          }
        //   echo $data;
    break;

    case 'cont_approveAndForward':
        $empid = $_REQUEST['empid'];
        $ref = $_REQUEST['ref'];
        $forwardName = $_REQUEST['forwardName'];
        $original_id = $_REQUEST['original_id'];
        $data=0;
          ...
          {
               $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg='SO forward '.$original_id.' Acct employee Contingency to '.$forwardName.' SOA';
                user_activity($empid,$file_name,'Forward Contingency',$msg);
            echo "<script>alert('Contingency Claim forwarded');window.location='../forwarded_list.php';</script>";
            $data=1;
          }
          else
          {
              $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg='SO unable to forward '.$original_id.' Acct employee Contingency to '.$forwardName.' SOA';
                user_activity($empid,$file_name,'Forward Contingency',$msg);
            echo "<script>alert('Something went wrong');window.location='../cont_details1.php';</script>";
            $data=0;
          }
        //   echo $data;
    break;
...
//--------------------------------------------------------------------------
    ...
    break;
     ...
     case 'updateEmployee1':
        ...
        {
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SO send the request for update bio details to '.$forwardName.' SOA';
            user_activity($empid,$file_name,'Request for update details',$msg);
            echo "1";
        }
        else
        {
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SO unable to send the request for update bio details to '.$forwardName.' SOA';
            user_activity($empid,$file_name,'Request for update details',$msg);
            echo "0";
        }
    ...
    case 'updateEmpData':
        ...
        if(updateEmpData($id,$pfno,$billunit,$panno,$empname,$designation,$station,$mobile,$email,$catg,$dept,$depot_id,$bp,$gp,$bdate,$apdate,$level))
        {
            $empid=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SO approve the '.$pfno.' acct employee deatils';
            user_activity($empid,$file_name,'Approve request for update details',$msg);
            echo "<script>alert('User Data Updated successfully');window.location='../update_emp.php';</script>";
        }
        else
        {
             $empid=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SO unable to approve the '.$pfno.' acct employee deatils';
            user_activity($empid,$file_name,'Approve request for update details',$msg);
      ...
      $forwardName = $_REQUEST['forwardName'];
              ...
              // Code to Send sms starts here
                ...
                echo 'error:' . curl_error($ch);
                  ...
                  //echo "<script>alert('OTP has been sent on your registered mobile ".$result_set['mobile'].".');window.location='../profile.php';</script>";
                    ...
                    $empid=$_SESSION['empid'];
            		$file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                    user_activity($empid,$file_name,'Forward TA','SO, Send the OTP to SO for Forwarding the TA');  
                }
                curl_close($ch);
        ...
        else
        {
          $flag="0";
            $empid=$_SESSION['empid'];
    		$file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid,$file_name,'Forward TA','SO, unable to Send the OTP to SO bcz mobile number not found');
        }
        echo $flag;
    ...
    case 'forward_Cont':
            ...
            {
                $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg='SO Contingency has been forwarded to '. $forwardName.' SOA success';
                user_activity($empid,$file_name,'Forward Contingency',$msg);
                $flag='1';
            }
            else
            {
                $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg='SO Contingency unable to forward to '. $forwardName.' SOA success';
                user_activity($empid,$file_name,'Forward Contingency',$msg);
                $flag='0';
            }
            echo $flag;
    ...
    case 'forward_con':
            ...
            $forwardName = $_REQUEST['fdname'];
                    ...
                    //$q = mysqli_query("UPDATE `continjency_master` SET `forward_status`='1' WHERE reference = '$ref'");
