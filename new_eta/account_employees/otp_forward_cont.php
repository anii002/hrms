									...
									<div class="form-group">
										<center>
											<label class="control-label">
												<h4 class="">Select Section Officer</h4>
											</label>
										</center>
									</div>
								</div>
	...
	$(document).on('click', '.btn_confirm_otp', function() {
		...
		if (fdname != '0') {
			...
			$.ajax({
				...
				data: "action=forward_Cont&fdname=" + fdname + "&empid=" + empid + "&ref_no=" + ref_no,
				success: function(data) {
					// alert(data);
					$("#otp_loader").hide();
					if (data == 1) {
						$.jGrowl("Your Contingency has been forwarded to " + fdname, {
							header: 'Forward Cont.'
						});
						var delay = 1500;
						setTimeout(function() {
							window.location = 'Show_claimed_cont.php'
						}, delay);
					} else {
			...
			$.jGrowl("Please select Section Officer to forward Contingency...", {
				header: 'Forward Cont.'
			});
		}
