				...
				<form id="frm_update" method="post" enctype="multipart/form-data" autocomplete="off" class="horizontal-form">
					...
					<div class="form-body">
						...
						<div class="row">
							...
							<div class="col-md-4 billunitzindex">
								<div class="form-group">
									<label class="control-label">Section Officer / अनुभाग अधिकारी</label>
									...
									$dptid = $resu1['id'];
									$sql_user = mysqli_query($conn,"SELECT * from users where dept='" . $dptid . "' AND role='22' ");
									$cnt = mysqli_num_rows($sql_user);
									?>
									<select name="forwardName" id="forwardName" class="form-control select2" style="width: 100%" required>
										<option readonly value='0' selected>Select Section Officer</option>
...
<script>
	$(document).on("submit", "#frm_update", function(e) {
		...
		if (fdnm == 0) {
			alert("Please select SO to submit request");
		} else {
			...
			$.ajax({
				...
				success: function(data, textStatus, jqXHR) {
					...
					console.log(data);
					if (data == 1) {
						alert('Update request sent to selected section officer');
