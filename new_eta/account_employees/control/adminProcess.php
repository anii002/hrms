...
include('adminFunction.php');
  ...
  case 'delcont':
    ...
    if(mysql_affected_rows() >= 0)
    {
        ...
        user_activity($empid,$file_name,'Delete Contingency','Acct Employee deleted the Contingency with TA');
    ...
    else
    {
        $empid=$_SESSION['empid'];
        $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
        user_activity($empid,$file_name,'Delete Contingency','Acct Employee unable to delete the Contingency with TA');
    ...
    case 'otp_forward_ta':
              ...
              // Code to Send sms starts here
                ...
                $query_insert = mysql_query("insert into tbl_otp(empid,otp,sent) values('".$_SESSION['empid']."','$otp',CURRENT_TIMESTAMP)");
                  ...
                  $flag=$result_set['mobile'];
                    $empid=$_SESSION['empid'];
        		    $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                    user_activity($empid,$file_name,'Forward TA','Acct Employee, Send the OTP to employee for Forwarding the TA');   
                }
                curl_close($ch);
        ...
        else
        {
          $flag="0";
            $empid=$_SESSION['empid'];
		    $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid,$file_name,'Forward TA','Acct Employee, mobile number not found for Forwarding the TA');
        }
        echo $flag;
  ...
  case 'forward_returned_ta':
    ...
    case 'updateEmployee':
        ...
        {
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='Acct Employee send request for update details to '.$forwardName.' SO';
        ...
        {
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='Acct Employee unable to send request for update details to '.$forwardName.' SO';
            user_activity($empid,$file_name,'Request for update details',$msg);
            echo "0";
        }
    break;
           ...
           if($month_array[$s])
            {
              ...
              if($result['otp'] == $c_otp)
              {
                ...
                if($c_otp)
                {
                    ...
                    $flag='1';
                    $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                    $msg='Acct Employee forward TA to '.$forwardName.' SO';
                    user_activity($empid,$file_name,'Forward TA',$msg);
                  }
                  else{
                    $flag='0';
                    $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                    $msg='Acct Employee unable to forward TA to '.$forwardName.' SO';
                    user_activity($empid,$file_name,'Forward TA',$msg);
                  }
              }
              else
              {
                $flag='-1';
                $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg='Acct Employee unable to forward TA because OTP not matched';
                user_activity($empid,$file_name,'Forward TA',$msg);
              }
              echo $flag;
  ...
  case 'deleteclaim':
        ...
        if($total_rows > 1)
        {
            ...
            if(mysql_affected_rows() >= 0)
            {
                ...
                $empid=$_SESSION['empid'];
                $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                user_activity($empid,$file_name,'Delete TA','Acct Employee deleted the single TA');
            ...
            else
            {
                $empid=$_SESSION['empid'];
                $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                user_activity($empid,$file_name,'Delete TA','Acct Employee unable to delete the single TA');
               echo "<script>alert('Something went wrong');window.location='../Show_unclaimed_TA.php';</script>";
            }
          ...
          // echo "HI";
            ...
            if(mysql_affected_rows() >= 0)
            {
              ...
              $res1 = mysql_query($tasummary_sql);
                $empid=$_SESSION['empid'];
                $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                user_activity($empid,$file_name,'Delete TA','Acct Employee delete the overall TA');
               echo "<script>alert('Claim deleted successfully'); window.location='../Show_unclaimed_TA.php';</script>";
            }
            else
            {
                $empid=$_SESSION['empid'];
                $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                user_activity($empid,$file_name,'Delete TA','Acct Employee unable to delete the overall TA');
              echo "<script>alert('Something went wrong');window.location='../Show_unclaimed_TA.php';</script>";
            }
  ...
  case 'deletecont':
    ...
    if($result1 && $result2)
    {
        $empid=$_SESSION['empid'];
        $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
        user_activity($empid,$file_name,'Delete Contingency','Acct Employee delete the Contingency');
    ...
    else
    {
        $empid=$_SESSION['empid'];
        $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
        user_activity($empid,$file_name,'Delete Contingency','Acct Employee unable to delete the Contingency');
