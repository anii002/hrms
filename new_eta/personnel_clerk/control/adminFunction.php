...
function fetchEmployee1($id)
{
  ...
  // var_dump($result);
  $value = mysqli_fetch_array($result);
//   $qu = mysqli_query("select empid from users where empid='$id' AND role='15' ");
...
function generatesummary($selected_list_emp, $selected_list, $title, $description, $yy, $mm, $summary_id, $dept_id)
{
  ...
  $query_summary = mysqli_query($conn,"INSERT into master_summary (title,description,generated_date,forward_status,estcrk_status,month,year,summary_id,dept_id,is_gazetted) values('" . $title . "','" . $description . "','" . $d . "','0','0','" . $TAMonth . "','" . $TAYear . "','" . $summary_id . "','" . $dept_id . "','1')");
  echo mysqli_error($conn);
  $id = mysqli_insert_id($conn);
  $cnt = 0;
  foreach ($selected_list as $list) {
    $query1 = "UPDATE forward_data set summary='1'where reference_id='$list'";
