...
function fetchEmployee1($id)
{
  ...
  // var_dump($result);
  $value = mysql_fetch_array($result);
//   $qu = mysql_query("select empid from users where empid='$id' AND role='15' ");
...
function generatesummary($selected_list_emp, $selected_list, $title, $description, $yy, $mm, $summary_id, $dept_id)
{
  ...
  $query_summary = mysql_query("INSERT into master_summary (title,description,generated_date,forward_status,estcrk_status,month,year,summary_id,dept_id,is_gazetted) values('" . $title . "','" . $description . "','" . $d . "','0','0','" . $TAMonth . "','" . $TAYear . "','" . $summary_id . "','" . $dept_id . "','1')");
  echo mysql_error();
  $id = mysql_insert_id();
  $cnt = 0;
