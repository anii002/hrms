				...
				// 		echo "SELECT `name` FROM `employees` WHERE pfno = '".$row['empid']."' ";
					...
					?>
							...
							for($i = 0; $i < $count; $i++)
							{
								...
								$qry3 = mysqli_query($conn,"SELECT employees.name as name,role FROM employees,users WHERE users.empid=employees.pfno and pfno = '".$history_row[$i]['fowarded_to']."'");
