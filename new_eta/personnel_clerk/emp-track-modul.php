	...
	<div class="page-content-wrapper">
		<div class="page-content">
			...
			<div class="portlet box blue">
				...
				// 		echo "SELECT `name` FROM `employees` WHERE pfno = '".$row['empid']."' ";
						$qry2 = mysql_query("SELECT `name` FROM `employees` WHERE pfno = '".$row['empid']."'");
						$row2 = mysql_fetch_array($qry2);
						
						$history_row = array();
						
						$qry1 = mysql_query("SELECT * FROM `forward_data` WHERE reference_id='".$_GET['ref_no']."' ORDER BY id ASC");
						$query2 = mysql_num_rows($qry1);
					...
					?>
							...
							for($i = 0; $i < $count; $i++)
							{
								...
								// echo "SELECT employees.name as name,role FROM employees,users WHERE users.empid=employees.pfno and pfno = '".$history_row[$i]['fowarded_to']."' ";
								$qry3 = mysql_query("SELECT employees.name as name,role FROM employees,users WHERE users.empid=employees.pfno and pfno = '".$history_row[$i]['fowarded_to']."'");
						...
						if($i==1 && $da==0){
							...
							//	echo "SELECT forward_status,estcrk_status,generated_date,DA_approved_time,EST_approved_time FROM `master_summary` WHERE `summary_id`='".$row5['summary_id']."'";
								...
								$qry7 = mysql_query("SELECT `empid`,role FROM `users` WHERE role = '11' AND dept = '".$_SESSION['dept']."'");
							...
							//	}
								...
								//echo $row6['estcrk_status'];
								// echo $_SESSION['dept'];
								$qry7 = mysql_query("SELECT `empid` FROM `users` WHERE role = '14' AND dept = '".$_SESSION['dept']."'");
								$row7 = mysql_fetch_array($qry7);
