...
<style>
	...
	<div id="updateDetails" class="modal fade modal-scroll " tabindex="-1" data-replace="true">
		...
		<form  action="control/adminProcess.php?action=updateAccData" method="post" enctype="multipart/form-data" autocomplete="off" class="horizontal-form">
			<!-- -->
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
								    ...
								    <div class="input-group">
										<span >
											<input class="form-control" readonly type="text" id="station_name" name="station_name" />	
										</span>
									</div>
									<!-- </div> -->
								</div>
						    ...
						    <div class="col-md-12">
								<div class="form-group">
									<label class="control-label">बिल यूनिट / Bill  Unit</label>
										...
										<select name="emp_bu[]" id="emp_bu" multiple="multiple" style="width:100%;" class="select2" tabindex="-1" aria-hidden="true" >
										    <option value="0" disabled> बिल यूनिट का चयन करेंं / Select Bill Unit</option>
        			                    		<?php
        			                    			$sql=mysql_query("SELECT billunit FROM `billunit` WHERE au='0107' ORDER BY `billunit` ASC");
        			                    			while($row = mysql_fetch_array($sql)) {
