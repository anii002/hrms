...
// $id = mysqli_insert_id() or die(mysqli_error());
...
if ($res == $total_rows) {
	...
	user_activity($empid, $file_name, 'Add Contingency', 'Employee added Contingency with TA');
	echo "<script>alert('Contigency bills are added');window.location='Show_unclaimed_TA.php';</script>";
} else if ($res < $total_rows && $res != 0) {
	$empid = $_SESSION['empid'];
	$file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
	user_activity($empid, $file_name, 'Add Contingency', 'Employee Contingency successfully added with TA, but some not added. Please Check!');
	...
	$empid = $_SESSION['empid'];
	$file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
	user_activity($empid, $file_name, 'Add Contingency', 'Employee Contingency bill not added');
