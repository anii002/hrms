						...
						<div class="row add-train-title ">
							<div class="col-md-4 billunitzindex">
								<div class="form-group">
									...
									<select name="month[]" id="month" multiple="multiple" data-placeholder="माह का चयन करे / Select a Months" required>
										<option value="00">माह का चयन करे / Select Month</option>
...
<script type="text/javascript">
	...
	$(".addrowbtn").on("click", function() {
		var data = '';
		var sr = $("#sr1").val(); // Get the current serial number
		var prevdate = $("#date" + sr).val(); // Get the previous date

		sr++; // Increment the serial number

		// Build the HTML string for the new row
		data += '<tr class="odd gradeX">';
		data += '<td style="width: 10%"><div class="form-group"><div class=""><input class="form-control datepicker datecheck" type="text" name="date' + sr + '" id="date' + sr + '" val=' + sr + ' value=' + prevdate + ' placeholder="dd/mm/yyyy" readonly></div></div></td>';
		data += '<td style="width: 10%"><div class="form-group"><div class=""><select class="form-control j_type" name="type' + sr + '" id="type' + sr + '" val=' + sr + '><?php $query1 = "SELECT * FROM `journey_type_master`";
																																											$sql1 = mysqli_query($conn, $query1);
																																											while ($row1 = mysqli_fetch_array($sql1)) {
																																												echo "<option value='" . $row1["id"] . "'>" . $row1["journey_type"] . "</option>";
																																											} ?></select></div></div></td>';
		data += '<td style="width: 10%"><div class="form-group"><input type="text" class="form-control val train_no" placeholder="Train No." name="trainno' + sr + '" id="trainno' + sr + '" val=' + sr + '></div></td>';
		data += '<td style="width: 10%"><div class="form-group"><div class=""><select class="form-control purpose" val=' + sr + ' name="other' + sr + '" id="other' + sr + '"><?php $query2 = "SELECT * FROM `journey_purpose_master`";
																																												$sql2 = mysqli_query($conn, $query2);
																																												while ($row2 = mysqli_fetch_array($sql2)) {
																																													echo "<option value='" . $row2["id"] . "'>" . $row2["journey_purpose"] . "</option>";
																																												} ?></select></div></div></td>';
		data += '<td style="width: 8%"><div class="form-group"><input type="text" name="dtime' + sr + '" val=' + sr + ' id="dtime' + sr + '" class="form-control changedtime timevalue" placeholder="hh:mm"></div></td>';
		data += '<td style="width: 8%"><div class="form-group"><input type="text" class="form-control ta_calculation time_val timevalue" name="atime' + sr + '" val=' + sr + ' id="atime' + sr + '" placeholder="hh:mm"></div></td>';
		data += '<td style="width: 12%"><div class="form-group"><input type="text" list="dstation' + sr + '" style="text-transform:uppercase" name="dstn' + sr + '" id="dstn' + sr + '" placeholder="select Station" val=' + sr + ' class="departClass form-control"><datalist id="dstation' + sr + '"><?php $sql = "SELECT stationdesc FROM station";
																																																																										$query = mysqli_query($conn, $sql);
																																																																										while ($row = mysqli_fetch_array($query)) {
																																																																											echo "<option value='" . $row["stationdesc"] . "'>";
																																																																										} ?></datalist></div></td>';
		data += '<td style="width: 12%"><div class="form-group"><input type="text" list="astation' + sr + '" style="text-transform:uppercase" name="astn' + sr + '" id="astn' + sr + '" placeholder="select Station" val=' + sr + ' class="form-control arrivalstn"><datalist id="astation' + sr + '"><?php $sql = "SELECT stationdesc FROM station";
																																																																										$query = mysqli_query($conn, $sql);
																																																																										while ($row = mysqli_fetch_array($query)) {
																																																																											echo "<option value='" . $row["stationdesc"] . "'>";
																																																																										} ?></datalist></div></td>';
		data += '<td><div class="form-group"><input type="text" class="form-control" name="distance' + sr + '" id="distance' + sr + '" placeholder="Distance" val=' + sr + '></div></td>';
		data += '<td><div class="form-group"><input type="text" class="form-control changeper" name="per' + sr + '" id="per' + sr + '" placeholder="Percentage" readonly val=' + sr + '></div></td>';
		data += '<td><div class="form-group"><input type="text" class="form-control" name="amt' + sr + '" id="amt' + sr + '" placeholder="Amount" readonly val=' + sr + '></div></td>';
		data += '</tr>';
		...
		// Copy previous destination station value
		var sr_1 = (sr - 1);
	...
	$(document).on("blur", ".ta_calculation", function() {
		...
		if (res == true) {
			var atime_sec = cal_sec(date, atime);
			var dtime_sec = cal_sec(date, dtime);
			...
			alert('Please enter valid time ELSE');
			$("#atime" + val).val('');
			$("#atime" + val).focus();
			ctr = 1;
		}
				...
				if (dtime == '' && prev_jtype != 6) {
					...
					if (date1 != date) {
						for (var i = val; i > 0; i--) {
							...
							if (prev_date == date) {
								var c_time = time_calculate_1(prev_date, date, prev_dtime, atime);
								if (c_time <= '06:00') {
									per = "30%";
									taamount = (amount * 0.3);
								...
								flag = '1';
								dtime = "00:00";
								var c_time = time_calculate(date, dtime, atime);
								if (c_time <= '06:00') {
									per = "30%";
									taamount = (amount * 0.3);
								} else if (c_time > '06:00' && c_time <= '12:00') {
					...
					var c_time = time_calculate(date1, dtime1, atime1);
					if (c_time <= '06:00') {
						per = "30%";
						taamount = (amount * 0.3);
					} else if (c_time > '06:00' && c_time <= '12:00') {
					...
					if (prev_date != date && prev_date != null && prev_jtype != 6 && prev_jtype != 2) {
						dtime1 = "00:00";
						var c_time = time_calculate(prev_date, dtime1, atime);
						if (c_time <= '06:00') {
							per = "30%";
							taamount = (amount * 0.3);
						} else if (c_time > '06:00' && c_time <= '12:00') {
...
<script>
	...
	$(document).on("change", "#month", function() {
		...
		//var days= daysInMonth(mon, year);
		var mindate = '01' + "/" + mon + "/" + year;
		// var maxdate=days+"/"+month+"/"+year1;
	...
	$(document).on("blur", ".datecheck", function() {
		var month = $("#month").val();
		var date = $(this).val();
		var res = date.substr(3, 2);
