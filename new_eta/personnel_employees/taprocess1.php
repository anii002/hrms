...
if (isset($_POST['submit'])) {
	...
	if ($month_count > 1) {
		...
		for ($mcnt = 0; $mcnt < $month_count; $mcnt++) {
			...
			$query1 = "INSERT INTO `taentry_master`(`TAMonth`, `TAYear`, `empid`, `reference_no`, `cardpass`, `objective`, `status`, `forward_status`, `created_date`, `is_rejected`, `reason`) VALUES ('" . $mths . "','" . $year . "','" . $_SESSION['empid'] . "','" . $reference . "','" . $cardpass . "','" . $object . "','0','0','" . $date1 . "','0','null' )";
			// print_r(query1);exit();
			...
			user_activity($empid, $file_name, 'Save TA', 'Employee saved the 2 months TA');
		}
		// exit();
	...
	// Normal Code start here
		...
		$empid = $_SESSION['empid'];
		$file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
		user_activity($empid, $file_name, 'Save TA', 'Employee saved the Single month TA');
	}
