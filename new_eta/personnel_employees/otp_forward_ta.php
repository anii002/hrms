							...
							<div class="row">
								...
								<div class="col-md-4 billunitzindex">
									<div class="form-group">
										...
										$query_emp = mysqli_query($conn,"SELECT department.DEPTNO as id,employees.station as station  FROM `employees` ,department WHERE department.DEPTNO=employees.dept AND pfno='" . $_SESSION['empid'] . "' ");
										$resu1 = mysqli_fetch_array($query_emp);
										$dptid = $resu1['id'];
										//echo $dptid;exit();
										//print_r($dptid);exit();
										//echo ("SELECT * from users where dept='".$dptid."' AND role='12' OR role='22' and status='1' ");
										$sql_user = mysqli_query($conn,"SELECT * from users where dept='" . $dptid . "' AND role='12' OR role='22' and status='1' ");
...
<script type="text/javascript">
	$(document).on('click', '.btn_action1', function() {
		...
		if (forwardName != '0') {
			...
			$.jGrowl("Please select Controlling Incahrge to forward TA...", {
				header: 'Forward TA'
			});
		}
