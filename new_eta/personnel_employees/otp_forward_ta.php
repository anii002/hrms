          			...
          			<div id="frd_form">
						<div class="form-body">
							...
							<div class="row">
				              ...
				              $query_emp =mysql_query("SELECT department.DEPTNO as id,employees.station as station  FROM `employees` ,department WHERE department.DEPTNO=employees.dept AND pfno='".$_SESSION['empid']."' ");
				              $resu1=mysql_fetch_array($query_emp);
				              $dptid=$resu1['id'];
				              //echo $dptid;exit();
				              //print_r($dptid);exit();
				              //echo ("SELECT * from users where dept='".$dptid."' AND role='12' OR role='22' and status='1' ");
				              $sql_user=mysql_query("SELECT * from users where dept='".$dptid."' AND role='12' OR role='22' and status='1' ");
				            ...
				            <select name="forwardName" id="forwardName" class="form-control select2 required" style="width: 100%" required>
				             ...
				             <?php
				              ...
				              {  
				                    $user_stations=array();
		                            $user_stations=$resu['station'];
		                            $user_station_final=explode(',',$user_stations);
		                        if($user_stations==""){
		                       ...
		                       else if(in_array($resu1['station'],$user_station_final))
		                        {
	  ...
	  // alert(ref_no +"_"+forwardName+"_"+empid);
	  	...
	  	{	
	  		...
	  		$.ajax({
			    ...
			    data:"action=otp_forward_ta&forwardName="+forwardName+"&empid="+empid+"&ref_no="+ref_no,
			    ...
			    {   
			      //alert(data);
			        ...
			        if(data != 0)
			        {
			        	...
			        	$.jGrowl("OTP has been sent to registered number. Please enter OTP ", { header: 'Forward TA' });
	  		...
	  		$.jGrowl("Please select Controlling Incahrge to forward TA...", { header: 'Forward TA' });
	  	}
...
// 		alert(empid+"_"+c_otp+"_"+fdname+"_"+ref_no);
		...
		{   
			...
			type:"post",
			url:"control/adminProcess.php",
			data:"action=forward_TA&fdname="+fdname+"&empid="+empid+"&ref_no="+ref_no+"&c_otp="+c_otp,
				success:function(data)
				{	
					//alert(data);
					 $("#otp_loader").hide();
					if(data == 1)
					{
		                ...
		                var delay = 1500;
		                setTimeout(function(){ window.location = 'Show_claimed_TA.php'  }, delay);  
					}
					else if(data == -1)
					{
