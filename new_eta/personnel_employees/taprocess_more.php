...
if (isset($_POST['submit'])) {
	...
	//$sql=mysqli_query("SELECT `set_number` FROM `taentrydetails` WHERE `reference_no`='".$old_reference."' AND empid='".$_SESSION['empid']."' ORDER BY id DESC ");
	...
	if ($month_count > 1) {
		...
		for ($mcnt = 0; $mcnt < $month_count; $mcnt++) {
			...
			if ($mcnt == 0) {
				...
				user_activity($empid, $file_name, 'Add more TA', 'Employee add more TA with another month');
			}
		}
	...
	// Normal Code start here
		...
		$empid = $_SESSION['empid'];
		$file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
		user_activity($empid, $file_name, 'Add more TA', 'Employee add more TA in existing month.');
	}
