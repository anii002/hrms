				...
				<form id="frm_update" method="post" enctype="multipart/form-data" autocomplete="off" class="horizontal-form">
					...
					<div class="form-body">
						...
						<div class="row">
							...
							<div class="col-md-4 billunitzindex">
								<div class="form-group">
									<label class="control-label">Controlling Incharge / नियंत्रण प्रभारी </label>
									...
									$dptid = $resu1['id'];
									$sql_user = mysqli_query($conn,"SELECT * from users where dept='" . $dptid . "' AND role='12' AND status='1' ");
									$cnt = mysqli_num_rows($sql_user);
									?>
									<select name="forwardName" id="forwardName" class="form-control select2" style="width: 100%" required>
										<option readonly value='0' selected>Select Controlling Incharge</option>
		...
		if (fdnm == 0) {
			...
			$.ajax({
				...
				success: function(data, textStatus, jqXHR) {
					...
					console.log(data);
					if (data == 1) {
						alert('Update request sent to selected incharge');
