...
function changePass($pass, $connfirm)
{
  global $conn;
...
/*function addEmployee($empid,$gradepay,$fullname,$dept,$billunit,$design,$mobile,$station,$email,$address)
{
  ...
  $result = mysqli_query($query) or die(mysqlii_error($conn));
  if($result)
    return true;
  else
    return false;
...
function get_summary($sum_id)
{
    ...
    while ($val = mysqli_fetch_array($res)) {
        ...
        if ($val2) {
            ...
            $amount = 0; // Set default amount or handle it accordingly
        }
        ...
        if ($month_array[$cdate]) {
            $mmon = $month_array[$cdate];
        }
        ...
        // Handle 30%, 70%, 100% details
        $data .= ($val['30p_cnt'] == '0' && $val['30p_amt'] == '0') ? "<td>-</td><td>-</td>" : "<td>" . $val['30p_cnt'] . "</td><td>" . $val['30p_amt'] . "</td>";
        $data .= ($val['70p_cnt'] == '0' && $val['70p_amt'] == '0') ? "<td>-</td><td>-</td>" : "<td>" . $val['70p_cnt'] . "</td><td>" . $val['70p_amt'] . "</td>";
        $data .= ($val['100p_cnt'] == '0' && $val['100p_amt'] == '0') ? "<td>-</td><td>-</td>" : "<td>" . $val['100p_cnt'] . "</td><td>" . $val['100p_amt'] . "</td>";

        // Calculate total amount
        $total_amt = (int)$val['30p_amt'] + (int)$val['70p_amt'] + (int)$val['100p_amt'];
        $temp += $total_amt;

        // Fetch additional contribution amount
        ...
        // Append additional contribution to data
        $data .= ($ress['amount'] == '0' || $ress['amount'] == null) ? "<td>-</td>" : "<td>" . $ress['amount'] . "</td>";

        // Add show button for details
