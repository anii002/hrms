			...
			<div class="portlet box blue">
				...
				<div class="portlet-body form">
					...
					<?php
						...
						$qry2 = mysqli_query($conn,"SELECT `name`,dept FROM `employees` WHERE pfno = '".$row['empid']."' and dept='".$_SESSION['dept']."' ");
					...
					?>
							...
							for($i = 0; $i < $count; $i++)
							{
								...
								$qry3 = mysqli_query($conn,"SELECT employees.name as name,role FROM employees,users WHERE users.empid=employees.pfno and pfno = '".$history_row[$i]['fowarded_to']."' and employees.dept='".$row2['dept']."'");
						...
						if($i==1 && $da==0){
							...
							//	echo "SELECT forward_status,estcrk_status,generated_date,DA_approved_time,EST_approved_time FROM `master_summary` WHERE `summary_id`='".$row5['summary_id']."'";
								...
								$qry7 = mysqli_query($conn,"SELECT `empid`,role FROM `users` WHERE role = '11' AND dept = '". $row2['dept']."'");
							...
							//	}
								...
								//echo $row6['estcrk_status'];
								// echo $_SESSION['dept'];
								$qry7 = mysqli_query($conn,"SELECT `empid` FROM `users` WHERE role = '14' AND dept = '". $row2['dept']."'");
								$row7 = mysqli_fetch_array($qry7);
