						...
						<div class="form-body">
							...
							<div class="row">
								...
								<div class="col-md-4">
									<div class="form-group">
										<center>
											<label class="control-label"><h4 class="">Select Personnel Clerk</h4></label>	
										</center>
									</div>						
								</div>
								<div class="col-md-4">	</div>
							...
							<div class="row">
				              ...
				              $resu1=mysqli_fetch_array($query_emp);
				               $dptid=$resu1['id'];
				              $sql_user=mysqli_query($conn,"SELECT * from users where dept='".$dptid."' AND role='16' ");
				            ...
				            <select name="forwardName" id="forwardName" class="form-control select2 required" style="width: 100%" required>
				              <option readonly value='0' selected >Select PC</option>
	...
	$(document).on('click','.btn_confirm_otp',function(){
		...
		var c_otp=$("#c_otp").val();
 		//alert(empid+"_"+c_otp+"_"+fdname+"_"+ref_no+"_"+empid_session);
			...
			data:"action=forward_cont&fdname="+fdname+"&empid="+empid+"&ref_no="+ref_no+"&c_otp="+c_otp+"&empid_session="+empid_session,
				...
				{	
					...
					if(data == 1)
					{
						...
						$.jGrowl("Your Claim has been forwarded to "+fdname, { header: 'Forward CONT' });
		                var delay = 1500;
