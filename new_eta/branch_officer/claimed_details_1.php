...
if (isset($_POST['getback'])) {
	...
	if ($query3 && $query3 == TRUE) {
		...
		user_activity($empid, $file_name, 'Get Back TA', 'BO Contingency Get Back Successfully');
		echo "<script>alert('TA Get Back Successfully...');</script>";
		echo "<script> window.location='Show_unclaimed_TA.php'; </script>";
	} else {
		$file_name = basename($_SERVER["SCRIPT_FILENAME"], '.php');
		user_activity($empid, $file_name, 'Get Back TA', 'BO unable to get back Contingency');
				...
				<div class="row">
					<label class="" style="font-size:16px;font-weight:100;margin-left:20px;">
						...
						<span style="border-bottom:1px solid black"><b>
								<?php
								foreach ($expl as $val) {
									// Ensure $val is an integer and then perform the subtraction
									echo (int)$years[(int)$val - 1] . ", ";
								}
								...
								<button class="btn yellow">Print</button>
											...
											<div class="col-md-4 summary-total">
												...
												<div class="table-scrollable">
													...
													if ($sql3 && mysqli_num_rows($sql3) > 0) {
														...
														$row3 = ['30p_cnt' => 0, '30p_amt' => 0, '70p_cnt' => 0, '70p_amt' => 0, '100p_cnt' => 0, '100p_amt' => 0];
														$total_amount = 0;
													}
													?>
										...
										<div class="col-md-12 trackprint-btn">
											<ul>
												...
												$query3 = "SELECT `30p_cnt`,`30p_amt`,`70p_cnt`,`70p_amt`,`100p_cnt`,`100p_amt`, `is_summary_generated` FROM `tasummarydetails` WHERE `empid`='" . $_SESSION['empid'] . "' AND `reference_no`='" . $_GET['ref_no'] . "' ";
												$sql3 = mysqli_query($conn, $query3);
												$is_summary_generated = 0; // Default value if no record is found
