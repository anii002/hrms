...
if(isset($_POST['getback']))
	{
		...
		$query = mysql_query("SELECT `empid` FROM `continjency_master` WHERE `reference`='".$ref_no."' ");
		$row = mysql_fetch_array($query);
		$empid = $row['empid'];
		
		$query2 = mysql_query("UPDATE `continjency_master` SET `forward_status`=0 WHERE reference = '".$ref_no."' and empid = '".$empid."' ");
		...
		if($query3 && $query3 == TRUE)
		{
		  //  $empid=$_SESSION['empid'];
            ...
            user_activity($empid,$file_name,'Get Back Contingency','BO Contingency Get Back Successfully');
			echo "<script>alert('Contingency Get Back Successfully...') window.location='show_saved_cont.php'; </script>";
		}
		else
		{
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            user_activity($empid,$file_name,'Get Back Contingency','BO unable to get back Contingency');
...
// if(isset($_POST['print_data']))
{
    ...
    $empl_query = mysql_query("select * from continjency_master where reference_no='".$_GET['ref_no']."'");
								...
								<table id="example" class="table table-hover table-bordered display">
									...
									<tbody align="center">
										...
										<tr>
											...
											<?php
											$query_f_s=mysql_query("SELECT forward_status FROM `continjency_master` WHERE reference='".$_GET['ref_no']."' ");
											$total_row_f_s=mysql_fetch_array($query_f_s);
							...
							<ul>
								...
								<?php
    							 ...
    							 //   print_r($month);
    							    ...
    							    $query1=mysql_query("SELECT forward_status FROM `continjency_master` WHERE empid='".$_SESSION['empid']."' AND reference='".$_GET['ref_no']."' ");
									$row = mysql_fetch_array($query1);

									if($row['forward_status'] != '1'){
							    ?>
								<li><a href="otp_forward_cont.php?reference_no=<?php echo $_GET['ref_no'];?>&empid=<?php echo $_GET['empid'];?>" class="btn green btnhide"  class="hide_print btn btn-primary pull-right" >Forword</a></li>
							<?php } ?>
