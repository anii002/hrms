...
{
  global $conn;
  $query = "select users.empid as empid,employees.name as name,employees.desig as desig,employees.mobile as mobile,users.role as role,users.station as station,employees.level as level from users,employees where users.empid=employees.pfno AND users.empid= '" . $id . "'";
  $result = mysqli_query($conn,$query);
  $value = mysqli_fetch_array($result);
...
function generatesummary($selected_list_emp, $selected_list, $title, $description, $yy, $mm, $summary_id, $dept_id)
{
  ...
  $query_summary = mysqli_query($conn,"INSERT into master_summary (title,description,generated_date,forward_status,estcrk_status,month,year,summary_id,dept_id) values('" . $title . "','" . $description . "','" . $d . "','0','0','" . $TAMonth . "','" . $TAYear . "','" . $summary_id . "','" . $dept_id . "')");
