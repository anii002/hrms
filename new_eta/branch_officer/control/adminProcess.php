...
switch ($_REQUEST['action']) {
  ...
  case 'fwESTCLERK':
    ...
    //    $res= mysqli_query($query_select);

    $u_query = mysqli_query($conn,"UPDATE master_summary SET forward_status='1',DA_approved_time='" . $date . "' where summary_id='" . $sumid . "' ");
    echo mysqli_error($conn);

    //$u_query=mysqli_query("UPDATE forward_data SET hold_status='0',arrived_time='".$date."',depart_time='".$date."' where reference_id='".$refno."' AND fowarded_to='".$_SESSION['empid']."'  ");

    // $ss=mysqli_query("SELECT * FROM `users` WHERE role='5' ");
    // $res1=mysqli_fetch_array($ss);
  ...
  case 'fwESTCLERK_cont':
    ...
    $u_query = mysqli_query($conn,"UPDATE cont_summary SET forward_status='1',DA_approved_time='" . $date . "' where summary_id='" . $sumid . "' ");
    echo mysqli_error($conn);
