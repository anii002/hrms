							...
							<div class="row">
								...
								<div class="col-md-6 billunitzindex">
									<div class="form-group">
										...
										<?php													$query_emp = "select stationcode,stationdesc from station";
											$result_emp = mysql_query($query_emp);
										?>
											<select name="stationcode" id="stationcode" class="select2 form-control">
												  <option value="0" selected disabled>Select Station</option>
													<?php
													while($value_emp = mysql_fetch_assoc($result_emp))
													{													 ?>

													 <option value="<?php echo $value_emp['stationcode']; ?>"><?php echo $value_emp['stationdesc']; ?></option>
													 <?php
								...
								<div class="col-md-6 billunitzindex">
									<div class="form-group">
										<label class="control-label">उपयोगकर्ता  / User</label>
											...
											<select name="role" id="role" class="form-control select2" tabindex="-1" aria-hidden="true" >
												<option value="0" selected disabled>Select User</option>
												<option value="22">Section Officer</option>
												<option value="24">ADFM</option>
											</select>
									</div>
								</div>
							</div>
						          ...
						          <div class="form-group">
						            ...
						            <div class="portlet-body">
						                ...
						                <table id="example1" class="table table-hover table-bordered display">
						                  ...
						                  <tbody>
    		<?php
              $query_emp = "SELECT employees.*, users.status as user_status, users.username ,users.role,users.dept,users.station from employees INNER JOIN users on employees.pfno = users.empid AND users.dept='".$_SESSION['dept']."' AND users.role IN(22,24)";
              $result_emp = mysql_query($query_emp);
              $sr=1;
              while($value_emp = mysql_fetch_array($result_emp))
              {
							...
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label">स्टेशन  / Station</label>
										...
										<select name="emp_station" id="emp_station" class="select2 form-control" tabindex="-1" aria-hidden="true" >
