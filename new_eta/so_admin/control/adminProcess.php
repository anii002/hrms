...
include('function.php');
  ...
  break;
    ...
    case 'role_transfer':
        ...
        if($rst_role_transfer)
        {
            ...
            $msg='SOA role transfer of '.$pfno.' user to '.$transfer_emp_id.' user';
        ...
        else
        {
            ...
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA unable to role transfer of '.$pfno.' user to '.$transfer_emp_id.' user';          
            user_activity($pfno,$file_name,'Role Transfer Of user',$msg);
    ...
    case 'rejectTA_SOA':
        ...
        else
        {
            ...
            if(mysqli_affected_rows($conn) >= 0)
            {
               ...
               echo "<script>alert('Claim rejected successfully...'); window.location='../generate_summary1.php';</script>";
            }
            else
            {
                $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
                $msg='SOA unable to reject '.$pmempid.' Acct employee TA';
                user_activity($ciempid,$file_name,'Reject TA',$msg);
    ...
    $update_tamaster="UPDATE continjency_master set is_rejected=1,rejected_by='".$ciempid.",".$role."',reason='".$reason."' where empid='".$pmempid."' AND reference='".$ref_no."'";
        ...
        {
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA unable to reject '.$pmempid.' Acct employee Contingency';
            user_activity($ciempid,$file_name,'Reject Contingency',$msg);
    ...
    case 'updateEmpData1':
        ...
        if(updateEmpData1($id,$pfno,$billunit,$panno,$empname,$designation,$station,$mobile,$email,$catg,$dept,$depot_id,$bp,$gp,$bdate,$apdate,$level))
        {
            $empid1=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA approve the request of update details of '.$pfno.' Acct Employee';
            user_activity($empid1,$file_name,'Approve the request of update Acct Emp details',$msg);
        ...
        else
        {
            $empid1=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA unable to approve the request of update details of '.$pfno.' Acct Employee';
            user_activity($empid1,$file_name,'Approve the request of update Acct Emp details',$msg);
  ...
  case 'activeUser':
      ...
      $pfno = $_REQUEST['id'];
        ...
        else
        {
            $empid1=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA unable to active the '.$pfno.' user';
            user_activity($empid1,$file_name,'Active User',$msg);
      ...
      if(deactiveUser($pfno,$active))
        {
            $empid1=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA deactive the '.$pfno.' user';
     ...
     $date=date("Y-m-d h:i:s");
      ...
      $u_query=mysqli_query($conn,"UPDATE master_summary SET forward_status='1',DA_approved_time='".$date."' where summary_id='".$sumid."' ");
       ...
       if($u_query)
       {
            $empid=$empid_session;
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA Forward '.$sumid.' summary to ADFM sir';
            user_activity($empid,$file_name,'Forward Summary',$msg);
          echo "<script>alert('successfully forwarded to ADFM Sir '); window.location='../gen_summary_report.php';</script>";
       } 
       else
       {
           $empid=$empid_session;
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA unable to Forward '.$sumid.' summary to ADFM sir';
            user_activity($empid,$file_name,'Forward Summary',$msg);
           ...
           if($month_array[$s])
            {
              ...
              if($result['otp'] == $c_otp)
              {
                ...
                if($c_otp)
                {
                    ...
                    //Your message to send, Add URL encoding here.
                    $msg = "Your Contingency claim for month of $mon - $y  and  amount $amt with $ref reference number has been submitted successfully.";
                    // $msg = "Your TA claim with $ref reference number has been submitted successfully.";
                    $message = urlencode("$msg");
  ...
  case 'forward_con':
    ...
    case 'generatesummarycont':
      ...
      if($query_summary && $flag == 1)
      {
            ...
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA generate the Contingency summary '.$summary_id.' of month '.$mm;
            user_activity($empid,$file_name,'Generate Cont Summary',$msg);
      ...
      else
      {
            $empid=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA unable to generate the Contingency summary '.$summary_id.' of month '.$mm;
            user_activity($empid,$file_name,'Generate Cont Summary',$msg);
  ...
  case 'fwESTCLERK_cont':
       ...
       if($u_query)
       {
            $empid=$empid_session;
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA Forward '.$sumid.' Contingency summary to ADFM sir';
            user_activity($empid,$file_name,'Forward Summary',$msg);
       ...
       else
       {
           $empid=$empid_session;
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA unable to Forward '.$sumid.' Contingency summary to ADFM sir';
            user_activity($empid,$file_name,'Forward Summary',$msg);
   ...
   case 'view_conti':
    ...
    case 'generatesummary':
        ...
        if(generatesummary($selected_list_emp,$selected_list,$title,$description,$yy,$mm,$summary_id,$dept_id))
        {
            $empid=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA generate the TA summary '.$summary_id.' of month '.$mm;
            user_activity($empid,$file_name,'Generate Summary',$msg);
        ...
        else
        {
            $empid=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA unable to generate the TA summary '.$summary_id.' of month '.$mm;
            user_activity($empid,$file_name,'Generate Summary',$msg);
    ...
    case 'AddControlAdmin':
        ...
        if(AddAdmin($empid,$username,$role,$dept,$station,$empname,$mobile,$email,$design,$paylevel))
        {
            $empid1=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA add the '.$empid.' user as role '.$role;
        ...
        else
        {
            $empid1=$_SESSION['empid'];
            $file_name=basename($_SERVER["SCRIPT_FILENAME"], '.php');
            $msg='SOA unable to add the '.$empid.' user as role '.$role;
            user_activity($empid1,$file_name,'Add User',$msg);
