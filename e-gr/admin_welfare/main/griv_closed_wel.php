                ...
                <div class="x_panel">
                            ...
                            <div class="row">
                                <div class="col-md-offset-1 col-md-12 table-responsive ">
									...
									return $a_c;
                                            ...
                                            $fire_all = mysqli_query($db_egr,"select  * from tbl_grievance where gri_ref_no='" . $gri_ref_no . "'", $db_egr);
                    ...
                    <div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
                        <table border="1" class="table table-bordered ">
                            ...
                            {
                                global $db_egr;
                                $second_user = mysqli_query($db_egr,"select user_name from tbl_user where user_id=$second_id", $db_egr);
                            ...
                            // {
                            //     global $db_egr;
                            //     $sql1 = mysqli_query("select status from status where id=$status", $db_egr);
                            //     while ($sql_query1 = mysqli_fetch_array($sql1)) {
                            //         $status_fetch = $sql_query1['status'];
                            //     }
                            //     return $status_fetch;
                            ...
                            {
                                global $db_egr;
                                $f_action = mysqli_query($db_egr,"select action from action where id=$action", $db_egr);
                                while ($action_f = mysqli_fetch_array($f_action)) {
                                    $a_c = $action_f['action'];
                                }
                                return $a_c;
                            ...
                            {
                                global $db_egr;
                                $s_action = mysqli_query($db_egr,"select action from return_action where id=$sec_action", $db_egr);
                                while ($action_s = mysqli_fetch_array($s_action)) {
                                    $s_a = $action_s['action'];
                                }
                                return $s_a;
                            }
                            $fire_all = mysqli_query($db_egr,"select  * from tbl_grievance_forward where griv_ref_no='$gri_ref_no'", $db_egr);
                            $count_doc = 1;
                            $cnt = 0;
                            while ($all_fetch = mysqli_fetch_array($fire_all)) {
                                // print_r($all_fetch);
                                ...
                                if ($user_id_forwarded != "") {
                                    ...
                                    $fetch_emp = mysqli_query($db_egr,"select name from register_user where emp_no='$emp_need'", $db_common);
                                ...
                                echo "<td>$status</td>";
                                $sql_doc_sec = mysqli_query($db_egr,"select * from doc where griv_ref_no='$gri_ref_no' and uploaded_by='" . $all_fetch['user_id'] . "'  AND doc_id='" . $doc_id . "'", $db_egr);
