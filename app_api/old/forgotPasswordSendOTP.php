...
if(isset($_REQUEST['user_id']))
{
	...
	else
	{
		$response["error"] =FALSE;
			//session_start();
			$selectQuery=mysql_query("SELECT mobile from emp_master1 WHERE emp_no=$user_id");
			...
			$otp = rand(1,10000);
				...
				// Code to Send sms starts here
				  ...
				  //Your authentication key
					...
					$msg = "Your OTP for HRMS forgot password is $otp";
					...
					if(curl_errno($ch))
					{
					 ...
					 echo json_encode($response);
						...
						echo json_encode($response);	
						$query_insert = mysql_query("insert into tbl_otp(emp_id,otp,sent) values('".$user_id."','$otp',CURRENT_TIMESTAMP)");
