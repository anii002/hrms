		...
		<div class="row">
			...
			<?php
				$pf_no = $_SESSION['username'];
					dbcon();
				$sql = "SELECT emp_no,reference_id,ref_id, scheme_id, created_at FROM tbl_form_details INNER JOIN tbl_form_forward ON tbl_form_details.reference_id = tbl_form_forward.ref_id WHERE tbl_form_forward.forwarded_to = '$pf_no' AND tbl_form_details.status = 1 AND tbl_form_forward.fw_status = 1 AND tbl_form_details.rejected = 0";
