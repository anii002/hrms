...
$id=$_GET['id'];
			...
			$cnt=0;
		    $i=0;
			$sql1=mysql_query("select distinct(count) from prft_reversion_track where rev_pf_no='$pf'");
			if($sql1){
			while($res=mysql_fetch_assoc($sql1))
			{
			    ...
			    if($get_cnt!=0)
					dbcon1();
					$sql=mysql_query("select * from prft_reversion_track where rev_pf_no='$pf' and count='$get_cnt' and id!=(SELECT MAX(id) FROM prft_reversion_track) ORDER by id desc");

					$count_records=mysql_num_rows($sql);
					$sql_last=mysql_query("select * from  prft_reversion_track where rev_pf_no='$pf' and count='$get_cnt' ORDER by id desc limit 1");
				...
				if($data[$temp]['rev_car_re_remark']!=$data_last['rev_car_re_remark'])	$npf_diff[34]=1;
					...
					if($count_records==0)
					{
						$sql=mysql_query("select * from  prft_reversion_track where rev_pf_no='$pf'");
						if($sql)
						{
