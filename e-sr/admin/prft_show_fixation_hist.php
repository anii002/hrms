...
$sql1 = mysqli_query($conn,"select distinct(count) from prft_fixaction_track where fix_pf_no='$pf'");
...
if ($sql1) {
	...
	while ($res = mysqli_fetch_assoc($sql1)) {
		...
		$sql = mysqli_query($conn,"select * from prft_fixaction_track where fix_pf_no='$pf' and count='$get_cnt' and id!=(SELECT MAX(id) FROM prft_fixaction_track) ORDER by id desc");



		$count_records = mysqli_num_rows($sql);

		$sql_last = mysqli_query($conn,"select * from prft_fixaction_track where fix_pf_no='$pf' and count='$get_cnt' ORDER by id desc limit 1");
		...
		if ($count_records == 0) {
			...
			$sql = mysqli_query($conn,"select * from  prft_fixaction_track where fix_pf_no='$pf'");
