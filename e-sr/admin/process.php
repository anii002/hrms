...
if (isset($_REQUEST['action'])) {
	switch (strtolower($_REQUEST['action'])) { 
	        ...
	        case 'get_present_work_detail':
				...
				dbcon1();
				$query=mysql_query("select * from present_work_temp where preapp_pf_number='$pf'");
				//echo "select * from present_work_temp where preapp_pf_no='$pf'".mysql_error();
           ...
           case 'get_last_entry':
				...
				$pf_data=$_REQUEST['pf'];
				dbcon1();
				$query=mysql_query("select * from  lastentry_temp where pf_number='$pf_data'");
				//echo "select * from  lastentry_temp where pf_number='$pf_data'".mysql_error();
           ...
           case 'get_pending_pf_report':
				...
				foreach($y as $key => $value){
					...
					if($billunit[0]==$b_v)
					{
						...
						$fetch=mysql_query("select * from biodata_temp where pf_number='$value'");
						...
						$fetch=mysql_query("select * from medical_temp where medi_pf_number='$value'");
						...
						$fetch=mysql_query("select * from appointment_temp where app_pf_number='$value'");
						...
						$fetch=mysql_query("select * from present_work_temp where preapp_pf_number='$value'");
						...
						$fetch=mysql_query("select * from family_temp where emp_pf='$value'");
						...
						$fetch=mysql_query("select * from nominee_temp where nom_pf_number='$value'");
			...
			case 'get_complete_pf_report':
				...
				/* $sql1=mysql_query("select preapp_pf_number from present_work_temp where preapp_billunit='$b_v' OR ogd_billunit='$b_v'");
				
				while($f=mysql_fetch_array($sql1))
				{
					$total_count=0;
					
					$fetch=mysql_query("select * from biodata_temp where pf_number='".$f['preapp_pf_number']."'");
						if(mysql_num_rows($fetch)>0)
						{
							$re=mysql_fetch_array($fetch);
							if($re['pf_number']!="" && $re['sr_no']!="" && $re['dob']!="" && $re['emp_name']!="" && $re['aadhar_number']!="" && $re['pan_number']!="" && $re['identification_mark']!="" && $re['religion']!="" && $re['community']!="" && $re['gender']!="" && $re['marrital_status']!="" && $re['recruit_code']!="" && $re['group_col']!="" && $re['education_ini']!="")
					...
					$fetch=mysql_query("select * from medical_temp where medi_pf_number='".$f['preapp_pf_number']."'");
						if(mysql_num_rows($fetch)>0)
						{
							$re=mysql_fetch_array($fetch);
							if($re['medi_pf_number']!="" && $re['medi_cate']!="" && $re['medi_dob']!="" && $re['medi_appo_date']!="" && $re['medi_design']!="" && $re['medi_class']!="" && $re['medi_examtype']!="" && $re['medi_certino']!="" && $re['medi_certidate']!="")
					...
					$fetch=mysql_query("select * from appointment_temp where app_pf_number='".$f['preapp_pf_number']."'");
						if(mysql_num_rows($fetch)>0)
						{
							$re=mysql_fetch_array($fetch);
							if($re['app_type']=="4")
					...
					$fetch=mysql_query("select * from present_work_temp where preapp_pf_number='".$f['preapp_pf_number']."'");
						if(mysql_num_rows($fetch)>0)
						{
							$total_count++;
					...
					$fetch=mysql_query("select emp_pf from family_temp where emp_pf='".$f['preapp_pf_number']."'");
						if(mysql_num_rows($fetch)>0)
						{
							$total_count++;
						}
					$fetch=mysql_query("select nom_pf_number from nominee_temp where nom_pf_number='".$f['preapp_pf_number']."'");
						if(mysql_num_rows($fetch)>0)
						{
							$total_count++;
						}
			...
			case 'get_all_report_detail':
				...
				dbcon1();
				$sql1=mysql_query("select * from present_work_temp where preapp_billunit='$b_v' OR ogd_billunit='$b_v'");
				...
				while($f=mysql_fetch_array($sql1))
				{
					...
					$fetch=mysql_query("select * from family_temp where emp_pf='".$f['preapp_pf_number']."'");
					...
					$fetch=mysql_query("select * from nominee_temp where nom_pf_number='".$f['preapp_pf_number']."'");
			...
			break;
				...
				if($f_n=='biodata')
				{
					...
					$sql2=mysql_query("select * from biodata_temp b INNER JOIN present_work_temp p ON b.pf_number=p.preapp_pf_number where date(b.date_time) between '$f_d' and '$t_d' and (p.preapp_billunit='$b_v' OR p.ogd_billunit='$b_v')");
				...
				} else if($f_n=='medical')
				{
					...
					$sql2=mysql_query("select * from medical_temp b INNER JOIN present_work_temp p ON b.medi_pf_number=p.preapp_pf_number where date(b.datetime) between '$f_d' and '$t_d' and (p.preapp_billunit='$b_v' OR p.ogd_billunit='$b_v')");
				...
				} else if($f_n=='init_appo')
				{
					...
					$sql2=mysql_query("select * from appointment_temp b INNER JOIN present_work_temp p ON b.app_pf_number=p.preapp_pf_number where date(b.date_time) between '$f_d' and '$t_d' and (p.preapp_billunit='$b_v' OR p.ogd_billunit='$b_v')");
				...
				}else if($f_n=='present_work')
				{
					...
					$sql2=mysql_query("select * from present_work_temp where date_time between '$f_d' and '$t_d' and (preapp_billunit='$b_v' OR ogd_billunit='$b_v')");
				...
				}else if($f_n=='family')
				{
					...
					$sql2=mysql_query("select * from family_temp b INNER JOIN present_work_temp p ON b.fmy_pf_number=p.preapp_pf_number where date(b.date_time) between '$f_d' and '$t_d' and (p.preapp_billunit='$b_v' OR p.ogd_billunit='$b_v')");
					...
					$sql2=mysql_query("select * from nominee_temp b INNER JOIN present_work_temp p ON b.nom_pf_number=p.preapp_pf_number where date(b.date_time) between '$f_d' and '$t_d' and (p.preapp_billunit='$b_v' OR p.ogd_billunit='$b_v')");
 ...
 case 'find_bank_detail':
			...
			{
				dbcon();
				$sql=mysql_query("select * from bank_detail where `ifsc_code`='$ifsc'");
				$ex=mysql_num_rows($sql);
				if($ex>0)
				{
		...
		case 'get_nom_value':
			...
			$data='';
			$val=$_POST['val'];
			$sql=mysql_query("select * from family_temp where id='$val'");
		...
		case 'fetch_history':
            ...
            $query = mysql_query("select final_transaction_id,$column,date(date_time) as date, time(date_time) as time from $table where $pfnm='$pf_no'") or die(mysql_error());
			$cnt = 0;
			$sr=1;
		...
		case 'get_training':
			...
			$data.="<br/><div class='row'><div class='col-md-6 col-sm-12 col-xs-12'><div class='form-group'><label class='control-label col-md-4 col-sm-3 col-xs-12' >Department</label><div class='col-md-8 col-sm-8 col-xs-12'><select name='tr_dept$t_count' id='tr_dept$t_count' class='form-control select2_dyn$t_count' style='margin-top:0px; width:100%;' required><option disabled selected > </option>";
							$sqlreligion=mysql_query("select * from department");
					while($rwDept=mysql_fetch_array($sqlreligion))
					{	
			...
			$data .="</select></div></div></div>  <div class='col-md-6 col-sm-12 col-xs-12'><div class='form-group'><label class='control-label col-md-4 col-sm-3 col-xs-12' >Designation</label><div class='col-md-8 col-sm-8 col-xs-12'><select name='tr_desig$t_count' id='tr_desig$t_count' class='form-control select2_dyn$t_count' style='margin-top:0px; width:100%;' required><option disabled selected > </option>";
							...
							$sqlreligion=mysql_query("select * from designation");
					while($rwDept=mysql_fetch_array($sqlreligion))
					{	
...
case 'get_award':
		...
		case 'get_increment':
			...
			$incr="<h3>$i_count Increment</h3><hr style='height:1px;border:none;color:#f39c12;background-color:#f39c12;'><div class='modal-body'><div class='row'><div class='col-md-6 col-sm-12 col-xs-12'><div class='form-group'><label class='control-label col-md-4 col-sm-3 col-xs-12' >Increment Type<span class=''></span></label><div class='col-md-8 col-sm-8 col-xs-12'><select class='form-control primary select2_dyn$i_count' id='incr_type$i_count' name='incr_type$i_count' style='width:100%;' required><option value='blank' selected></option>";
				$sqlDept=mysql_query("select * from increment_type");
				while($rwDept=mysql_fetch_array($sqlDept))
					{
		...
		case 'get_penalty':
			...
			$sqlDept=mysql_query('select * from penalty_type');
                while($rwDept=mysql_fetch_array($sqlDept))
				{					
			...
			$penalty .= "</select></div></div></div></div><br><div class='row'><div class='col-md-6 col-sm-12 col-xs-12'><div class='form-group'><label class='control-label col-md-4 col-sm-3 col-xs-12' >Penalty Sub Type</label><div class='col-md-8 col-sm-8 col-xs-12'><select name='p_sub_type$p_count' id='p_sub_type$p_count' class='form-control select2_dyn$p_count pen_sub_type' style='margin-top:0px; width:100%;' required></select></div></div></div></div><br><div class='clearfix'></div><div class='row'><div class='col-md-6 col-sm-12 col-xs-12'><div class='form-group'><label class='control-label col-md-4 col-sm-3 col-xs-12' >Penalty Issued</label><div class='col-md-8 col-sm-8 col-xs-12'><input type='text' id='pen_awarded$p_count' name='pen_awarded$p_count' class='form-control calender_picker_dyn$p_count'></div></div></div><div class='col-md-6 col-sm-12 col-xs-12'><div class='form-group'><label class='control-label col-md-4 col-sm-3 col-xs-12' >Penalty Effected</label><div class='col-md-8 col-sm-8 col-xs-12'><input type='text' id='pen_eff$p_count' name='pen_eff$p_count' class='form-control calender_picker_dyn$p_count calender_picker'></div></div></div></div><br><div class='clearfix'></div><div class='row'><div class='col-md-6 col-sm-12 col-xs-12'><div class='form-group'><label class='control-label col-md-4 col-sm-3 col-xs-12' >Letter No</label><div class='col-md-8 col-sm-8 col-xs-12'><input type='text' id='l_no$p_count' name='l_no$p_count' class='form-control' placeholder='Enter Letter No' required></div></div></div><div class='col-md-6 col-sm-12 col-xs-12'><div class='form-group'><label class='control-label col-md-4 col-sm-3 col-xs-12' >Letter Date</label><div class='col-md-8 col-sm-8 col-xs-12'><input type='text' id='ltr_date$p_count' name='ltr_date$p_count' class='form-control calender_picker  calender_picker_dyn$p_count' placeholder='Enter Date' required></div></div></div></div><br><div class='clearfix'></div><div class='row'><div class='col-md-6 col-sm-12 col-xs-12'><div class='form-group'><label class='control-label col-md-4 col-sm-3 col-xs-12' >ChargeSheet Status</label><div class='col-md-8 col-sm-8 col-xs-12'><select name='chrg_stat$p_count' id='chrg_stat$p_count' class='form-control select2_dyn$p_count' style='margin-top:0px; width:100%;' required><option value='blank' selected></option>";
				$sql_status=mysql_query('select * from charge_sheet_status');
	...
	case 'get_scale_all':
			...
			$value=$_POST['value'];
			if($value=='2' || $value=='3'){
				$sql=mysql_query("select * from `scale` where `pay_scale_type`='$value'");
				while($result=mysql_fetch_array($sql)){
	...
	case 'get_property_item':
			...
			$sql =mysql_query("SELECT * FROM `property_item` WHERE `type`='$pro_type'");
			while($result=mysql_fetch_array($sql)){
		...
		case 'fetchtraining':
			...
			$pf_no=$_POST['tra_pf'];
			$data='';
			$sql=mysql_query("select * from training_temp where pf_number='$pf_no'");
		...
		case 'fetch_present_penalty':
			...
			$pf_no=$_POST['pre_pf'];
			$data='';
			$sql=mysql_query("select * from penalty_temp where pen_pf_number='$pf_no'");
		...
		case 'fetch_present_work':
			...
			$pf_no=$_POST['pre_pf'];
			$data='';
			$sql=mysql_query("select * from present_work_temp where preapp_pf_number='$pf_no'");
			$sql_fetch=mysql_num_rows($sql);
			if($sql_fetch>0){
				while($result=mysql_fetch_array($sql))
				{
		...
		case 'get_pf':
			...
			$data = "<hr/><div class='box-header with-border'><h3 class='box-title'><i class='fa fa-book'></i> &nbsp;&nbsp;".$pf_count." Nominee</h3><hr/></div><div class='modal-body'><div class='row'><div class='col-md-6 col-sm-12 col-xs-12'><div class='form-group'>                    <label class='control-label col-md-4 col-sm-3 col-xs-12'>Nominee Type</label>
			 ...
			 dbcon();
			 $sql=mysql_query("select * from nominee_order_type");
		...
		case 'get_pincode':
			...
			$sql=mysql_query("select distinct(pincode) from pincode where state_u_t='$state'");
			while($result=mysql_fetch_array($sql))
			{
		...
		case 'get_medical_detail':
			...
			$sql=mysql_query("select * from medical_temp where medi_pf_number='$pf_no' ORDER BY id LIMIT 1");
		...
		case 'get_station':
			...
			if($division=='SUR')
			{
				$sql=mysql_query("select * from station where divisioncode='SUR'");
			...
			{
			$sql=mysql_query("select * from station where divisioncode='$division'");
			while($result=mysql_fetch_array($sql))
			{
		...
		case 'get_separate':
			...
			$sql=mysql_query("select * from billunit where id='$got_billunit'");
			while($result=mysql_fetch_array($sql))
			{
		...
		case 'get_bill_unit_depot':
			...
			if($unit=='0107')
			{
				$sql=mysql_query("select * from billunit where au='0107'");
				while($res=mysql_fetch_array($sql))
				{
			...
			{
			$sql=mysql_query("select * from billunit where au='$unit'");
			while($result=mysql_fetch_array($sql))
			{
		...
		case 'get_division':
			...
			if($zone=='01')
			{
				$sql=mysql_query("select * from division where divncode='SUR'");
				while($sql_fetch=mysql_fetch_array($sql))
						{
							$data = "<option selected value='".$sql_fetch['divncode']."'>".$sql_fetch['longdesc']."</option>";
						}

						$sq=mysql_query("select * from division where divncode <> 'SUR'");
			...
			{
			$sql=mysql_query("select * from division where rlycode='$zone'");
			while($result=mysql_fetch_array($sql))
			{
		...
		case 'get_division1':
			...
			if($zone=='01')
			{
				$sql=mysql_query("select * from unit where au='0107'");
				while($res_fetch=mysql_fetch_array($sql))
				{
			...
			{
			$sql=mysql_query("select * from unit where rlycode='$zone'");
			while($result=mysql_fetch_array($sql))
			{
		...
		case 'add_education':
			...
			$sql=mysql_query("insert into education(education)values('$religion')");
			if($sql){
		...
		case 'add_recruitment':
			...
			$sql=mysql_query("insert into recruitment_code(recruitment_code)values('$recruit')");
			if($sql){
		...
		case 'add_penalty':
			...
			$sql=mysql_query("insert into penalty_effected(penalty_effected)values('$penalty')");
			if($sql){
		...
		case 'add_property':
			...
			$sql=mysql_query("insert into property_source(property_source)values('$property')");
			if($sql){
		...
		case 'add_award':
			...
			$sql=mysql_query("insert into awards(awards)values('$awards')");
			if($sql){
		...
		case 'add_community':
			...
			$sql=mysql_query("insert into community(code,shortdesc,longdesc)values('$var','$shortdesc','$longdesc')");
			if($sql){
		...
		case 'add_religion':
			...
			$longdesc=$_POST['name'];
			$var=strtoupper($longdesc[0].$longdesc[1]);
			$sql=mysql_query("insert into religion(code,shortdesc,longdesc)values('$var','$shortdesc','$longdesc')");
			if($sql){
		...
		case 'add_movable_item':
			...
			$sql=mysql_query("insert into property_item_movable(property_item_movable)values('$movable')");
			if($sql){
		...
		case 'add_increment':
			...
			$sql=mysql_query("insert into increment_type(increment_type)values('$incr_type')");
	   ...
	   case 'update_inmovable_item':
		...
		case 'add_penalty_awarded':
			...
			$sql=mysql_query("insert into penalty_awarded(penalty_awarded)values('$penalty_awarded')");
			if($sql){
		...
		case 'add_inmovable_item':
			...
			$sql=mysql_query("insert into property_item_inmovable(property_item_inmovable)values('$property_item')");
			if($sql){
...
/*Yogesh Code End Here*/
		...
		dbcon();
		$query=mysql_query("select * from billunit where dept_id='$search'");
		// echo "select * from billunit where dept_id='$search'".mysql_error();
		$cnt=0;
		$cnt1=0;
	...
	break;
		...
		dbcon1();
				$pf=$_REQUEST['val3'];
				$query=mysql_query("select b.pf_number, b.emp_name,b.zone,b.aadhar_number,b.division,p.preapp_pf_number,p.preapp_department,p.preapp_designation,p.ps_type,p.preapp_scale,p.preapp_level from biodata_temp b inner join present_work_temp p on b.pf_number=p.preapp_pf_number where b.pf_number='$pf'");
				
			$sql_fetch=mysql_num_rows($query);
				if($sql_fetch>0)
				{
		...
		dbcon1();
			...
			$pass=hashPassword($password, SALT1, SALT2);
				...
				$query1=mysql_query("select * from user_login where pf_no='$pf'");
				// echo "select * from user_login where pf_no='$pf'".mysql_error();
				$res=mysql_num_rows($query1);
				if($res > 0)
				{
					echo "<script>window.location='add_user.php';alert('User Already Exists');</script>";
				}
				else{
			$query=mysql_query("insert into user_login(`pf_no`, `role`,`adminname`, `username`, `password`, `multi_bill_unit`)values('$pf','$usertype','$name','$username','$pass','$final_billunit')");
			
			 // echo ("insert into user_login(`pf_no`, `role`,`adminname`, `username`, `password`, `multi_bill_unit`)values('$pf','$usertype','$name','$username','$pass','$billunit')").mysql_error();
		...
		dbcon1();
			...
			$query=mysql_query("select b.pf_number,b.aadhar_number,u.pf_no from biodata_temp b inner join user_login u on b.pf_number=u.pf_no where pf_number='$pf_number' AND aadhar_number='$aadhar_no'");
		...
		case 'reset_pass':
			...
			// echo $pass;
			dbcon1();
			$query=mysql_query("update user_login set password='$pass' where pf_no='$pf_num'");
		...
		case 'only_sr_view':
		 ...
		 // echo $pan;
			dbcon1();
			$query=mysql_query("select id,pf_number,pan_number from biodata_temp where pf_number='$pf' AND pan_number='$pan'");
			
			$row=mysql_fetch_array($query);
			// echo "select id,pf_number,pan_number from biodata_temp where pf_number='$pf' AND pan_number='$pan'".mysql_error();
			$res=mysql_num_rows($query);
		...
		case 'get_retired_type':
			$sql=mysql_query("select * from retirement_type");
			$data='';
		...
		case 'get_transferd_type':
			$sql=mysql_query("select * from order_type_transfer");
			$data="";
		...
		case 'get_penalty_sub_type':
			...
			$data="";
			$sql=mysql_query("select * from major_minor_penalty where penalty_type='$pen_id'");
			while($res=mysql_fetch_array($sql))
			{
		...
		case 'deact_user':
			...
			{
				dbcon1();
				$query=mysql_query("update user_login set act_deact='1' where pf_no='$pf'");
				echo "<script>alert('User deactivated... !!!');</script>";
			}
			else
			{
				dbcon1();
				$query=mysql_query("update user_login set act_deact='0' where pf_no='$pf'");
