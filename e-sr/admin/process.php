...
if (isset($_REQUEST['action'])) {
	switch (strtolower($_REQUEST['action'])) {
		...
		case 'get_present_work_detail':
			...
			//echo "select * from present_work_temp where preapp_pf_no='$pf'".mysqli_error();
			$cnt = mysqli_num_rows($query);
			if ($cnt > 0) {
				while ($re = mysqli_fetch_array($query)) {
					if ($re['sgd_dropdwn' == '2']) {
						$data['preapp_department_id'] = $re['preapp_department'];
						$data['preapp_designation_id'] = $re['preapp_designation'];
						$data['pre_otherdesign'] = $re['pre_otherdesign'];
						$data['ps_type_id'] = $re['ps_type'];
						$data['preapp_scale'] = $re['preapp_scale'];
						$data['preapp_level'] = $re['preapp_level'];
						$data['preapp_group_id'] = $re['preapp_group'];
						$data['preapp_station'] = $re['preapp_station'];
						$data['preapp_billunit_id'] = $re['preapp_billunit'];
						$data['preapp_rop'] = $re['preapp_rop'];
						$data['preapp_depot_id'] = $re['preapp_depot'];
						//got value from id 
						$data['preapp_department'] = get_department($re['preapp_department']);
						$data['preapp_designation'] = get_designation($re['preapp_designation']);
						$data['ps_type'] = get_pay_scale_type($re['ps_type']);
						$data['preapp_group'] = get_group($re['preapp_group']);
						$data['preapp_billunit'] = get_billunit($re['preapp_billunit']);
						$data['preapp_depot'] = get_depot($re['preapp_depot']);
					} else {
						$data['preapp_department_id'] = $re['preapp_department'];
						$data['preapp_designation_id'] = $re['ogd_desig'];
						$data['pre_otherdesign'] = $re['preogd_otherdesign'];
						$data['ps_type_id'] = $re['ogd_pst'];
						$data['preapp_scale'] = $re['ogd_scale'];
						$data['preapp_level'] = $re['ogd_level'];
						$data['preapp_billunit_id'] = $re['ogd_billunit'];
						$data['preapp_depot_id'] = $re['ogd_depot'];
						$data['preapp_station'] = $re['ogd_station'];
						$data['preapp_group_id'] = $re['ogd_group'];
						$data['preapp_rop'] = $re['ogd_rop'];
						//got value from id 
						$data['preapp_department'] = get_department($re['preapp_department']);
						$data['preapp_designation'] = get_designation($re['ogd_desig']);
						$data['ps_type'] = get_pay_scale_type($re['ogd_pst']);
						$data['preapp_billunit'] = get_billunit($re['ogd_billunit']);
						$data['preapp_depot'] = get_depot($re['ogd_depot']);
						$data['preapp_group'] = get_group($re['ogd_group']);
					}
				}
			}
		...
		case 'get_last_entry':
			...
			$pf_data = $_REQUEST['pf'];
			$conn = dbcon1();
			$query = mysqli_query($conn, "select * from  lastentry_temp where pf_number='$pf_data'");
			//echo "select * from  lastentry_temp where pf_number='$pf_data'".mysqli_error();
			while ($fetch = mysqli_fetch_array($query)) {
				$data['date_of_join'] = date('d-m-Y', strtotime($fetch['date_of_join']));
				$data['retire_type'] = fetch_retirement_type($fetch['retire_type']);
				$data['retire_date'] = date('d-m-Y', strtotime($fetch['retire_date']));
				$data['retire_designation'] = get_designation($fetch['retire_designation']);
				$data['retire_designation_value'] = $fetch['retire_designation'];
				$data['department'] = get_department($fetch['department']);
				$data['department_value'] = $fetch['department'];
				$data['station'] = $fetch['station'];
				$data['rop'] = $fetch['rop'];
				$data['bill_unit'] = get_billunit($fetch['bill_unit']);
				$data['bill_unit_value'] = $fetch['bill_unit'];
				$data['scale'] = $fetch['scale'];
				$data['depot'] = get_depot($fetch['depot']);
				$data['total_years'] = $fetch['total_years'];
				$data['total_months'] = $fetch['total_months'];
				$data['total_days'] = $fetch['total_days'];
				$data['no_years'] = $fetch['no_years'];
				$data['no_months'] = $fetch['no_months'];
				$data['no_days'] = $fetch['no_days'];
				$data['lap'] = $fetch['lap'];
				$data['lhap'] = $fetch['lhap'];
				$data['due_type_amt'] = get_last_dues($fetch['due_type'], $fetch['due_amt']);
			}
		...
		case 'get_pending_pf_report':
			...
			foreach ($y as $key => $value) {
				...
				if ($billunit[0] == $b_v) {
					...
					$fetch = mysqli_query($conn, "select * from biodata_temp where pf_number='$value'");
					if (mysqli_num_rows($fetch) > 0) {
						...
						if ($re['pf_number'] != "" && $re['sr_no'] != "" && $re['dob'] != "" && $re['emp_name'] != "" && $re['aadhar_number'] != "" && $re['pan_number'] != "" && $re['identification_mark'] != "" && $re['religion'] != "" && $re['community'] != "" && $re['gender'] != "" && $re['marrital_status'] != "" && $re['recruit_code'] != "" && $re['group_col'] != "" && $re['education_ini'] != "") {
							...
							if ($re['identification_mark'] == "") {
								$arr[5] = "Identification Mark";
							}
							if ($re['religion'] == "") {
								$arr[6] = "Religion";
							}
							...
							if ($re['marrital_status'] == "") {
								$arr[9] = "Marrital Status";
							}
							if ($re['recruit_code'] == "") {
								$arr[10] = "Recruitment Code";
							}
							if ($re['group_col'] == "") {
								$arr[11] = "Group";
							}
							if ($re['education_ini'] == "") {
								$arr[12] = "Initial Education";
							}
							$arr_data = '';
					...
					$fetch = mysqli_query($conn, "select * from medical_temp where medi_pf_number='$value'");
					...
					if (mysqli_num_rows($fetch) > 0) {
						...
						if ($re['medi_pf_number'] != "" && $re['medi_cate'] != "" && $re['medi_dob'] != "" && $re['medi_appo_date'] != "" && $re['medi_design'] != "" && $re['medi_class'] != "" && $re['medi_examtype'] != "" && $re['medi_certino'] != "" && $re['medi_certidate'] != "") {
							...
							if ($re['medi_pf_number'] == "") {
								$arr[0] = "Medical Pf Number";
							}
							if ($re['medi_cate'] == "") {
								$arr[1] = "Medical Category";
							}
							if ($re['medi_dob'] == "") {
								$arr[2] = "Medical Date of Birth";
							}
							if ($re['medi_appo_date'] == "") {
								$arr[3] = "Appointment Date";
							}
							if ($re['medi_design'] == "") {
								$arr[4] = "Medical Design";
							}
							if ($re['medi_class'] == "") {
								$arr[5] = "Medical PME Class";
							}
							if ($re['medi_examtype'] == "") {
								$arr[6] = "Medical Exam Type";
							}
							if ($re['medi_certino'] == "") {
								$arr[7] = "Certificate Number";
							}
							if ($re['medi_certidate'] == "") {
								$arr[8] = "Certificate Date";
							}
							$arr_data = '';
							foreach ($arr as $values) {
								$arr_data .= "$values,";
					...
					$fetch = mysqli_query($conn, "select * from appointment_temp where app_pf_number='$value'");
					...
					if (mysqli_num_rows($fetch) > 0) {
						...
						if ($re['app_type'] == "4") {
							if ($re['app_type'] != "" && $re['app_pf_number'] != "" && $re['app_department'] != "" && $re['app_designation'] != "" && $re['app_date'] != "" && $re['app_regul_date'] != "" && $re['app_payscale'] != "" && $re['app_group'] != "" && $re['app_station'] != "" && $re['app_rop'] != "" && $re['app_depot'] != "") {
								...
								if ($re['app_type'] == "") {
									$arr[0] = "Appointment Type";
								}
								if ($re['app_pf_number'] == "") {
									$arr[1] = "Pf Number";
								}
								if ($re['app_department'] == "") {
									$arr[2] = "Appointment Department";
								}
								if ($re['app_designation'] == "") {
									$arr[3] = "Appointment Designation";
								}
								if ($re['app_date'] == "") {
									$arr[4] = "Appointment Date";
								}
								if ($re['app_regul_date'] == "") {
									$arr[5] = "Regularization Date";
								}
								if ($re['app_payscale'] == "") {
									$arr[6] = "Pay Scale Type";
								}
								if ($re['app_group'] == "") {
									$arr[7] = "Group";
								}
							...
							if ($re['app_type'] != "" && $re['app_pf_number'] != "" && $re['app_department'] != "" && $re['app_designation'] != "" && $re['app_regul_date'] != "" && $re['app_payscale'] != "" && $re['app_group'] != "" && $re['app_station'] != "" && $re['app_rop'] != "" && $re['app_depot'] != "") {
								...
								if ($re['app_regul_date'] == "") {
									$arr[4] = "Regularization Date";
								}
								if ($re['app_payscale'] == "") {
									$arr[5] = "Pay Scale Type";
								}
								if ($re['app_group'] == "") {
									$arr[6] = "Group";
								}
					...
					$fetch = mysqli_query($conn, "select * from present_work_temp where preapp_pf_number='$value'");
					...
					if (mysqli_num_rows($fetch) > 0) {
						...
						if ($re['sgd_dropdwn'] == '2') {
							...
							if ($re['preapp_pf_number'] != "" && $re['preapp_department'] != "" && $re['preapp_designation'] != "" && $re['ps_type'] != "" && $re['preapp_group'] != "" && $re['preapp_station'] != "" && $re['preapp_billunit'] != "" && $re['preapp_rop'] != "") {
								...
								if ($re['preapp_pf_number'] == "") {
									$arr[0] = "Pf Number";
								}
								if ($re['preapp_department'] == "") {
									$arr[1] = "Department";
								}
								if ($re['preapp_designation'] == "") {
									$arr[2] = "Designation";
								}
								if ($re['ps_type'] == "") {
									$arr[3] = "Pay Scale Type";
								}
								if ($re['preapp_group'] == "") {
									$arr[4] = "Group";
								}
								if ($re['preapp_station'] == "") {
									$arr[5] = "Station";
								}
								if ($re['preapp_billunit'] == "") {
									$arr[6] = "Billunit";
								}
								if ($re['preapp_rop'] == "") {
									$arr[7] = "Rate of Pay";
					...
					$fetch = mysqli_query($conn, "select * from family_temp where emp_pf='$value'");
					...
					$fetch = mysqli_query($conn, "select * from nominee_temp where nom_pf_number='$value'");
		...
		case 'get_all_report_detail':
			...
			$conn = dbcon1();
			$sql1 = mysqli_query($conn, "select * from present_work_temp where preapp_billunit='$b_v' OR ogd_billunit='$b_v'");
			...
			while ($f = mysqli_fetch_array($sql1)) {
				...
				$fetch = mysqli_query($conn, "select * from biodata_temp where pf_number='" . $f['preapp_pf_number'] . "'");
				...
				$fetch = mysqli_query($conn, "select * from medical_temp where medi_pf_number='" . $f['preapp_pf_number'] . "'");
				...
				$fetch = mysqli_query($conn, "select * from appointment_temp where app_pf_number='" . $f['preapp_pf_number'] . "'");
				...
				$fetch = mysqli_query($conn, "select * from present_work_temp where preapp_pf_number='" . $f['preapp_pf_number'] . "'");
				...
				$fetch = mysqli_query($conn, "select * from family_temp where emp_pf='" . $f['preapp_pf_number'] . "'");
				...
				$fetch = mysqli_query($conn, "select * from nominee_temp where nom_pf_number='" . $f['preapp_pf_number'] . "'");
		...
		case 'get_report':
			...
			if ($f_n == 'biodata') {
				...
				$sql2 = mysqli_query($conn, "select * from biodata_temp b INNER JOIN present_work_temp p ON b.pf_number=p.preapp_pf_number where date(b.date_time) between '$f_d' and '$t_d' and (p.preapp_billunit='$b_v' OR p.ogd_billunit='$b_v')");
				...
				$sql2 = mysqli_query($conn, "select * from medical_temp b INNER JOIN present_work_temp p ON b.medi_pf_number=p.preapp_pf_number where date(b.datetime) between '$f_d' and '$t_d' and (p.preapp_billunit='$b_v' OR p.ogd_billunit='$b_v')");

				//echo "select * from medical_temp b INNER JOIN present_work_temp p ON b.medi_pf_number=p.preapp_pf_number where date(b.datetime) between '$f_d' and '$t_d' and (p.preapp_billunit='$b_v' OR p.ogd_billunit='$b_v')".mysqli_error();
				...
				$sql2 = mysqli_query($conn, "select * from appointment_temp b INNER JOIN present_work_temp p ON b.app_pf_number=p.preapp_pf_number where date(b.date_time) between '$f_d' and '$t_d' and (p.preapp_billunit='$b_v' OR p.ogd_billunit='$b_v')");
				...
				$sql2 = mysqli_query($conn, "select * from present_work_temp where date_time between '$f_d' and '$t_d' and (preapp_billunit='$b_v' OR ogd_billunit='$b_v')");
				...
				$sql2 = mysqli_query($conn, "select * from family_temp b INNER JOIN present_work_temp p ON b.fmy_pf_number=p.preapp_pf_number where date(b.date_time) between '$f_d' and '$t_d' and (p.preapp_billunit='$b_v' OR p.ogd_billunit='$b_v')");
				...
				$sql2 = mysqli_query($conn, "select * from nominee_temp b INNER JOIN present_work_temp p ON b.nom_pf_number=p.preapp_pf_number where date(b.date_time) between '$f_d' and '$t_d' and (p.preapp_billunit='$b_v' OR p.ogd_billunit='$b_v')");
		...
		case 'find_bank_detail':
			...
			if (!empty($ifsc)) {
				$conn = dbcon();
				$sql = mysqli_query($conn, "select * from bank_detail where `ifsc_code`='$ifsc'");
		...
		case 'get_nom_value':
			...
			$data = '';
			$val = $_POST['val'];
			$sql = mysqli_query($conn, "select * from family_temp where id='$val'");
		...
		case 'fetch_history':
			...
			$query = mysqli_query($conn, "select final_transaction_id,$column,date(date_time) as date, time(date_time) as time from $table where $pfnm='$pf_no'") or die(mysqli_error($conn));
			$cnt = 0;
			$sr = 1;
		...
		case 'get_training':
			...
			$sqlreligion = mysqli_query($conn, "select * from training_type");
			while ($rwDept = mysqli_fetch_array($sqlreligion)) {
			...
			$sqlreligion = mysqli_query($conn, "select * from department");
			while ($rwDept = mysqli_fetch_array($sqlreligion)) {
			...
			$sqlreligion = mysqli_query($conn,"select * from designations");
			while ($rwDept = mysqli_fetch_array($sqlreligion)) {
		...
		case 'get_property':
			...
			$conn = dbcon();
			$sqlreligion = mysqli_query($conn, "select * from property_type");
			while ($rwDept = mysqli_fetch_array($sqlreligion)) {
				$data .= "<option value='" . $rwDept["id"] . "'>" . $rwDept["type"] . "</option>";
			}
		...
		case 'get_advance':
			...
			$conn = dbcon();
			$sqlreligion = mysqli_query($conn, "select * from advance");
			while ($rwDept = mysqli_fetch_array($sqlreligion)) {
		...
		case 'get_award':
			...
			$conn = dbcon();
			$sqlDept = mysqli_query($conn, "select * from awarded_by");
			...
			$conn = dbcon();
			$sqlDept = mysqli_query($conn, "select * from awards");
			while ($rwDept = mysqli_fetch_array($sqlDept)) {
		...
		case 'get_penalty':
			...
			$penalty .= "<script>$('.calender_picker_dyn$p_count').datepicker({format:'dd-mm-yyyy',autoclose:true,forceParse:false}).inputmask('dd-mm-yyyy',{placeholder: 'dd-mm-yyyy',showMaskOnHover: true,showMaskOnFocus: true});$('.select2_dyn$p_count').select2();</script>";
		...
		case 'get_scale_all':
			...
			$value = $_POST['value'];
			if ($value == '2' || $value == '3') {
				$sql = mysqli_query($conn, "select * from `scale` where `pay_scale_type`='$value'");
				while ($result = mysqli_fetch_array($sql)) {
		...
		case 'get_property_item':
			...
			$sql = mysqli_query($conn, "SELECT * FROM `property_item` WHERE `type`='$pro_type'");
			while ($result = mysqli_fetch_array($sql)) {
		...
		case 'fetch_property':
			...
			//echo "select * from property_temp where pro_pf_number='$pf_no'".mysqli_error();
		...
		case 'fetchtraining':
			...
			$pf_no = $_POST['tra_pf'];
			$data = '';
			$sql = mysqli_query($conn, "select * from training_temp where pf_number='$pf_no'");
			//echo "select * from training_temp where pf_number='$pf_no'".mysqli_error();
			$sql_fetch = mysqli_num_rows($sql);
			if ($sql_fetch > 0) {
				while ($result = mysqli_fetch_array($sql)) {
		...
		case 'fetch_present_penalty':
			...
			$pf_no = $_POST['pre_pf'];
			$data = '';
			$sql = mysqli_query($conn, "select * from penalty_temp where pen_pf_number='$pf_no'");
			//echo "select * from present_work_temp where preapp_pf_number='$pf_no'".mysqli_error();
			$sql_fetch = mysqli_num_rows($sql);
			if ($sql_fetch > 0) {
				while ($result = mysqli_fetch_array($sql)) {
		...
		case 'get_appointment_data':
			...
			while ($sql_res = mysqli_fetch_assoc($sql_row)) {
				$data .= "" . fetch_all_dept($sql_res['app_department']) . "$" . fetch_all_desig($sql_res['app_designation']) . "$" . $sql_res['other_designation'] . "$" . date('d-m-Y', strtotime($sql_res['app_date'])) . "$" . date('d-m-Y', strtotime($sql_res['app_regul_date'])) . "$" . get_all_pay_scale_type($sql_res['app_payscale']) . "$" . get_all_scale($sql_res['app_scale'], $sql_res['app_payscale']) . "$" . fetch_all_level($sql_res['app_level']) . "$" . fetch_all_group($sql_res['app_group']) . "$" . get_station($sql_res['app_station']) . "$" . $sql_res['other_station'] . "$" . $sql_res['app_billunit'] . "$" . $sql_res['app_rop'] . "$" . get_depot($sql_res['app_depot']) . "$" . $sql_res['app_refno'] . "$" . date('d-m-Y', strtotime($sql_res['app_letter_date'])) . "$" . $sql_res['app_remark'] . "$" . fetch_all_appo_type($sql_res['app_type']) . "$" . $sql_res['app_station'] . "$" . $sql_res['app_depot'] . "$" . $sql_res['app_payscale'] . "$" . $sql_res['app_type'] . "";
			}
			echo $data;
			break;
		...
		case 'fetch_present_work':
			...
			$pf_no = $_POST['pre_pf'];
			$data = '';
			$sql = mysqli_query($conn, "select * from present_work_temp where preapp_pf_number='$pf_no'");
			$sql_fetch = mysqli_num_rows($sql);
			if ($sql_fetch > 0) {
				while ($result = mysqli_fetch_array($sql)) {
		...
		case 'get_pf':
			...
			$conn = dbcon();
			$sql = mysqli_query($conn, "select * from nominee_order_type");
			// echo "select * from nominee_order_type".mysqli_error();
			while ($result = mysqli_fetch_array($sql)) {
			...
			$conn = dbcon1();
			session_start();
			$sql = mysqli_query($conn, "select * from family_temp where emp_pf='" . $_SESSION['set_update_pf'] . "'");
			//echo "select * from family_temp where emp_pf='73858510651".mysqli_error();
			while ($result = mysqli_fetch_array($sql)) {
		...
		case 'get_pincode':
			...
			$sql = mysqli_query($conn, "select distinct(pincode) from pincode where state_u_t='$state'");
			while ($result = mysqli_fetch_array($sql)) {
		...
		case 'get_medical_detail':
			...
			$sql = mysqli_query($conn, "select * from medical_temp where medi_pf_number='$pf_no' ORDER BY id LIMIT 1");
		...
		case 'get_station':
			...
			if ($division == 'SUR') {
				$sql = mysqli_query($conn, "select * from station where divisioncode='SUR'");
				while ($res = mysqli_fetch_array($sql)) {
					$data .= "<option value='" . $res['stationcode'] . "'>(" . $res['stationcode'] . ")" . $res['stationdesc'] . "</option>";
				}
			} else {
				$sql = mysqli_query($conn, "select * from station where divisioncode='$division'");
				while ($result = mysqli_fetch_array($sql)) {
		...
		case 'get_separate':
			...
			$sql = mysqli_query($conn, "select * from billunit where id='$got_billunit'");
			while ($result = mysqli_fetch_array($sql)) {
		...
		case 'get_bill_unit_depot':
			...
			$unit = $_POST['unit'];
			if ($unit == '0107') {
				$sql = mysqli_query($conn, "select * from billunit where au='0107'");
				while ($res = mysqli_fetch_array($sql)) {
					$data .= "<option value='" . $res['id'] . "'>" . $res['billunit'] . "/" . $res['deopt'] . "</option>";
				}
			} else {
				$sql = mysqli_query($conn, "select * from billunit where au='$unit'");
				while ($result = mysqli_fetch_array($sql)) {
		...
		case 'get_division':
			...
			$zone = $_POST['zone'];
			if ($zone == '01') {
				$sql = mysqli_query($conn, "select * from division where divncode='SUR'");
				while ($sql_fetch = mysqli_fetch_array($sql)) {
					$data = "<option selected value='" . $sql_fetch['divncode'] . "'>" . $sql_fetch['longdesc'] . "</option>";
				}

				$sq = mysqli_query($conn, "select * from division where divncode <> 'SUR'");
				while ($res = mysqli_fetch_array($sq)) {
					$data .= "<option value='" . $res['divncode'] . "'>" . $res['longdesc'] . "</option>";
				}
			} else {
				$sql = mysqli_query($conn, "select * from division where rlycode='$zone'");
				while ($result = mysqli_fetch_array($sql)) {
		...
		case 'get_division1':
			...
			$zone = $_POST['zone'];
			if ($zone == '01') {
				$sql = mysqli_query($conn, "select * from unit where au='0107'");
				...
				$sql = mysqli_query($conn, "select * from unit where rlycode='$zone'");
				while ($result = mysqli_fetch_array($sql)) {
		...
		case 'add_education':
			...
			$sql = mysqli_query($conn, "insert into education(education)values('$religion')");
			if ($sql) {
		...
		case 'add_recruitment':
			...
			$sql = mysqli_query($conn, "insert into recruitment_code(recruitment_code)values('$recruit')");
			if ($sql) {
		...
		case 'add_penalty':
			...
			$sql = mysqli_query($conn, "insert into penalty_effected(penalty_effected)values('$penalty')");
			if ($sql) {
		...
		case 'add_property':
			...
			$sql = mysqli_query($conn, "insert into property_source(property_source)values('$property')");
			if ($sql) {
		...
		case 'add_award':
			...
			$sql = mysqli_query($conn, "insert into awards(awards)values('$awards')");
			if ($sql) {
		...
		case 'add_community':
			...
			$sql = mysqli_query($conn, "insert into community(code,shortdesc,longdesc)values('$var','$shortdesc','$longdesc')");
			if ($sql) {
		...
		case 'add_religion':
			...
			$longdesc = $_POST['name'];
			$var = strtoupper($longdesc[0] . $longdesc[1]);
			$sql = mysqli_query($conn, "insert into religion(code,shortdesc,longdesc)values('$var','$shortdesc','$longdesc')");
			if ($sql) {
		...
		case 'add_movable_item':
			...
			$sql = mysqli_query($conn, "insert into property_item_movable(property_item_movable)values('$movable')");
			if ($sql) {
		...
		case 'add_increment':
			...
			$sql = mysqli_query($conn, "insert into increment_type(increment_type)values('$incr_type')");
			/*echo "insert into increment_type(increment_type)values('$incr_type')".mysqli_error();*/
			if ($sql) {
		...
		case 'add_penalty_awarded':
			...
			$sql = mysqli_query($conn,"insert into penalty_awarded(penalty_awarded)values('$penalty_awarded')");
			if ($sql) {
		...
		case 'add_inmovable_item':
			...
			$sql = mysqli_query($conn, "insert into property_item_inmovable(property_item_inmovable)values('$property_item')");
			if ($sql) {
		...
		case 'get_billunit_search':
			$search = $_REQUEST['search'];
			$conn = dbcon();
			$query = mysqli_query($conn, "select * from billunit where dept_id='$search'");
			// echo "select * from billunit where dept_id='$search'".mysqli_error();
			$cnt = 0;
			$cnt1 = 0;
			...
			if (mysqli_affected_rows($conn) > 0) {
				...
				echo "<label>Select All</label>&emsp;<input type='checkbox' onClick='toggle(this)' id='' name='' class='check-all'/>";
				while ($res = mysqli_fetch_array($query)) {

					echo "<td height='100' width='25%' style='margin:5px;'><table><tr><td><input class='bill_unit_chkbox check' type='checkbox' id='" . $res['billunit'] . "' name='billunit[]' attr1='$cnt1'  attr='" . $res['billunit'] . "' style='margin:15px;' /></td><td style='font-size:18px;'><span>" . $res['billunit'] . "." . $res['deopt'] . "</span></td></tr></table></td>";
...
<?php
				...
				echo "<td><center><div class='alert alert-warning'><h4>No Record Available for This 	Billunit</h4></div></center></td>";
			}
			break;
		...
		case 'get_predata':
			$conn = dbcon1();
			$pf = $_REQUEST['val3'];
			$query = mysqli_query($conn, "select b.pf_number, b.emp_name,b.zone,b.aadhar_number,b.division,p.preapp_pf_number,p.preapp_department,p.preapp_designation,p.ps_type,p.preapp_scale,p.preapp_level from biodata_temp b inner join present_work_temp p on b.pf_number=p.preapp_pf_number where b.pf_number='$pf'");

			$sql_fetch = mysqli_num_rows($query);
			if ($sql_fetch > 0) {
				while ($result = mysqli_fetch_array($query)) {
					$data['emp_name'] = $result['emp_name'];
					$data['aadhar_number'] = $result['aadhar_number'];
					$data['pf_number'] = "SURP_" . $result['pf_number'];
					$data['preapp_designation'] = get_designation($result['preapp_designation']);
					$data['preapp_department'] = get_department($result['preapp_department']);
				}
			} else {
				$data = "";
			}
		...
		case 'save_user':
			$conn = dbcon1();
			$pf = $_POST['bio_pf_no'];
			$usertype = $_POST['emp_usertype'];
			$username = $_POST['emp_username'];
			$name = $_POST['emp_name1'];
			$password = $_POST['emp_pass'];


			$single_billunit = $_POST['text_approve'];
			// echo $single_billunit;
			$single_billunit1 = implode(',', $single_billunit);
			// echo $single_billunit1;
			$single_billunit_sliced = substr($single_billunit1, 1);
			// echo $single_billunit_sliced;
			$all_billunit = $_POST['text_approve1'];
			// echo $all_billunit;
			$all_billunit1 = implode(',', $all_billunit);

			if ($single_billunit_sliced == '') {
				$final_billunit = $all_billunit1;
			} else {
				$final_billunit = $single_billunit_sliced;
			}
			$pass = hashPassword($password, SALT1, SALT2);


			$query1 = mysqli_query($conn, "select * from user_login where pf_no='$pf'");
			// echo "select * from user_login where pf_no='$pf'".mysqli_error();
			$res = mysqli_num_rows($query1);
			if ($res > 0) {
				echo "<script>window.location='add_user.php';alert('User Already Exists');</script>";
			} else {
				$query = mysqli_query($conn, "insert into user_login(`pf_no`, `role`,`adminname`, `username`, `password`, `multi_bill_unit`)values('$pf','$usertype','$name','$username','$pass','$final_billunit')");

				// echo ("insert into user_login(`pf_no`, `role`,`adminname`, `username`, `password`, `multi_bill_unit`)values('$pf','$usertype','$name','$username','$pass','$billunit')").mysqli_error();

				if ($query) {
					echo "<script>window.location='user_mgt.php';alert('New User Added Successfully');</script>";
				} else {
					echo "<script>window.location='add_user.php';alert('User not Added');</script>";
				}
			}
			break;
		...
		case 'forget_pass':
			$conn = dbcon1();
			$pf_number = $_POST['forget_pf_no'];
			$aadhar_no = $_POST['forget_aadher'];

			$query = mysqli_query($conn,"select b.pf_number,b.aadhar_number,u.pf_no from biodata_temp b inner join user_login u on b.pf_number=u.pf_no where pf_number='$pf_number' AND aadhar_number='$aadhar_no'");
			$res = mysqli_num_rows($query);

			if ($res > 0) {
				echo "<script>window.location='../change_pass.php?pf=$pf_number';alert('User is Exist');</script>";
			} else {
				echo "<script>window.location='../forget_pass.php';alert('User Not Exist');</script>";
			}
			break;

		case 'reset_pass':
			$pf_num = $_POST['user_pf'];
			//echo $pf_num;
			$new_pass = $_POST['new_pass'];
			$pass = hashPassword($new_pass, SALT1, SALT2);
			// echo $pass;
			$conn = dbcon1();
			$query = mysqli_query($conn,"update user_login set password='$pass' where pf_no='$pf_num'");

			echo "<script>window.location='../index.php';alert('Password Changed Successfully');</script>";
			break;
		...
		case 'only_sr_view':
			...
			// echo $pan;
			$conn = dbcon1();
			$query = mysqli_query($conn,"select id,pf_number,pan_number from biodata_temp where pf_number='$pf' AND pan_number='$pan'");

			$row = mysqli_fetch_array($query);
			// echo "select id,pf_number,pan_number from biodata_temp where pf_number='$pf' AND pan_number='$pan'".mysqli_error();
			$res = mysqli_num_rows($query);
			...
			if ($res > 0) {
				...
				$_SESSION['id'] = $row['id'];
				$_SESSION['name'] = 'guest';
				echo "<script>window.location='display_sr_tab.php?pf=$pf';</script>";
			}
			break;

		case 'get_report_billunit':
			$dept = $_REQUEST['dept'];
			$sql = mysqli_query($conn,"select * from billunit where dept_id='$dept'");
			$data = '';
			$data .= "<option selected disabled hidden>Select Billunit</option>";
			while ($res = mysqli_fetch_array($sql)) {
				$data .= "<option value='" . $res['id'] . "'>" . $res['billunit'] . "</option>";
			}
			echo $data;
			break;

		case 'get_retired_type':
			$sql = mysqli_query($conn,"select * from retirement_type");
			$data = '';
			$data .= "<option selected disabled hidden>Select Retirement Type</option>";
			while ($res = mysqli_fetch_array($sql)) {
		...
		case 'get_transferd_type':
			$sql = mysqli_query($conn,"select * from order_type_transfer");
			$data = "";
			$data .= "<option selected disabled hidden>Select Transfer Type</option>";
			while ($res = mysqli_fetch_array($sql)) {
		...
		case 'get_penalty_sub_type':

			$pen_id = $_REQUEST['pen_type'];
			$data = "";
			$sql = mysqli_query($conn,"select * from major_minor_penalty where penalty_type='$pen_id'");
			while ($res = mysqli_fetch_array($sql)) {
		...
		case 'deact_user':
			...
			$pf = $_REQUEST['pf'];
			$attr = $_REQUEST['attribute'];
			if ($attr == 'deact') {
				$conn = dbcon1();
				$query = mysqli_query($conn,"update user_login set act_deact='1' where pf_no='$pf'");
				echo "<script>alert('User deactivated... !!!');</script>";
			} else {
				$conn = dbcon1();
				$query = mysqli_query($conn,"update user_login set act_deact='0' where pf_no='$pf'");
