...
/*function get_state_data($state){
	...
	if($result){
		...
		while($state_data = mysql_fetch_array($result)){
...
function get_state_data($state){
	...
	if($state!=""){
		...
		$sql=mysql_query("select * from `state` where longdesc='$state'");
		...
		if($cnt>0)
		{
			while($result=mysql_fetch_array($sql))
			{
				if($state == $result['longdesc'] || $state == $result['statecode']){
					...
					$sql1=mysql_query("select * from `state` where longdesc<>'$state'");
...
function get_pincode_data($pincode, $statecode){
	...
	if(strlen($statecode) == 2){
		...
		$fetched = mysql_query($state_sql);
		while($fetched_row = mysql_fetch_assoc($fetched)){
	...
	if($result){
		...
		while($pincode_data = mysql_fetch_array($result)){
...
function get_reli_data($religion){
	...
	if($result){
		...
		while($religion_data = mysql_fetch_array($result)){
...
function get_community_data($community){
	...
	if($result){
		...
		while($community_data = mysql_fetch_array($result)){
...
function get_recruitment_data($recruitment){
	...
	if($result){
		...
		while($recruitment_data = mysql_fetch_array($result)){
