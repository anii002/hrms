...
if (isset($_REQUEST['action'])) {
	switch (strtolower($_REQUEST['action'])) {
		...
		case 'update_biodata':
			...
			$bio_sql1 = mysqli_query($conn, "SELECT `imagefile` FROM `biodata_temp` WHERE `pf_number` = '" . $_SESSION['set_update_pf'] . "'") or die(mysqli_error($conn));
			...
			if (!empty($_FILES['imgs']['name'])) {
				$s_q = mysqli_query($conn, "update `biodata_temp` set `imagefile`='' where `pf_number`='$bio_pf_number'") or die(mysqli_error($conn));
			...
			if ($count == 1) {
				...
				for ($i = 0; $i < count($tmp_name_array); $i++) {
					...
					$q = mysqli_query($conn, "select count from sr_doc where pf_number='$bio_pf_no' order by id desc limit 1");
					...
					$sql_img = mysqli_query($conn, "insert into sr_doc(pf_number,biodata,count,update_date,uploaded_by) values('$bio_pf_no','$newfilename','$count',CURRENT_TIMESTAMP,'1')") or die(mysqli_error($conn));
