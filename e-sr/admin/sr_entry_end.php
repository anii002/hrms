...
<script>
	$(document).ready(function() {
		...
		$(document).on("click", ".add_source", function() {
			...
			// Get the suffix from the clicked element's ID
			var pt = $(this).attr('id');
			var t = pt.slice(-1);

			// Define the HTML structure for the new source
			var source = `
        <br>
    ...
    `;
			...
			// Append the new source to the target div
			$("#add_source_div" + t).append(source);

			// Initialize or reinitialize the select2 plugin if needed
			// $(".select2").select2();
		});
