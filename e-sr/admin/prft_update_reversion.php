				...
				<div class="box box-warning box-solid">
					...
					<div class="box-body">
						<div class="tab-pane" id="prft">
							...
							if ($pf_no != 'rev') {
								$query = mysqli_query($conn, "select * from  prft_reversion_temp where rev_pf_no='$pf_no' and id='$id'");
								$cnt = mysqli_num_rows($query);
								$result = mysqli_fetch_array($query);

								$action = "Viewed PRFT Reversion single record On SR Entry";
							} else {
								$action = "Visited Add Reversion Page On SR Entry";
							}
											...
											<div class="form-group">
												...
												<div class="col-md-8 col-sm-8 col-xs-12">
													<select class="form-control primary odrtype1 select2" id="rev_ordertype" name="rev_ordertype" style="width:100%;">
														...
														<?php
														$conn = dbcon();
														$sqlDept = mysqli_query($conn, "select * from reversion_order_type");
									...
									<div class="row">
										...
										<div class="col-md-6 col-sm-12 col-xs-12">
											<div class="form-group">
												...
												<div class="col-md-8 col-sm-8 col-xs-12">
													<textarea type="text" class="form-control primary" id="rev_remark" name="rev_remark"></textarea>
												</div>
											</div>
										</div>
									</div>
											...
											<div class="row">
												<div class="col-md-12 col-sm-12 col-xs-12">
													<div class="form-group">
														<label class="control-label col-md-4 col-sm-3 col-xs-12">Rate Of Pay<span class=""></span></label>
														<div class="col-md-8 col-sm-8 col-xs-12">
															<input type="text" class="form-control primary depot onlyNumber" id="rep_to_rop" name="rep_to_rop">
														</div>
													</div>
												</div>
											</div><br>
...
<script type="text/javascript">
	...
	$('#rev_ordertype').on('select2:close', function(e) {
		$("#rev_letterno").focus();
	});
	$('#rev_frm_dept').on('select2:close', function(e) {
		$("#rev_frm_desig").focus();
	});
	$('#rev_frm_desig').on('select2:close', function(e) {
		$("#rev_frm_otherdesig").focus();
	});
	$('#rev_frm_ps_type_9').on('select2:close', function(e) {
		$("#rev_frm_group").focus();
	});
	$('#rev_frm_group').on('select2:close', function(e) {
		$("#stationa").focus();
	});
	$('#rev_to_dept').on('select2:close', function(e) {
		$("#rev_to_desig").focus();
	});
	$('#rev_to_desig').on('select2:close', function(e) {
		$("#rev_to_otherdesig").focus();
	});
	$('#rev_to_ps_type_a').on('select2:close', function(e) {
		$("#rev_to_group").focus();
	});
	$('#rev_to_group').on('select2:close', function(e) {
		$("#stationb").focus();
	});
	$('#rep_from_rev_carried').on('select2:close', function(e) {
		$("#rep_rev_wefdate").focus();
	});
	...
	if (rev != 'rev') {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$('#rev_remark').html(arr['rev_remark']);
			}
		});
