 	...
 	<div style="border:1px solid #67809f;padding:30px;">
 		...
 		<?php
			...
			if (isset($_SESSION['set_update_pf'])) {
				$conn = dbcon1();
				$sql = mysqli_query($conn, "select * from  medical_temp where medi_pf_number='" . $_SESSION['set_update_pf'] . "'");
 							...
 							<div class="col-md-6 col-sm-12 col-xs-12">
 								<?php
									...
									if ($_SESSION['set_update_pf'] != "") {
										...
										$sql1 = mysqli_query($conn, "select * from medical_temp where medi_pf_number='$last_pf' ORDER BY id DESC LIMIT 1");
 								...
 								<div class="form-group">
 									...
 									<div class="col-md-8 col-sm-8 col-xs-12">
 										<select name="medi_cat_pme" id="medi_cat_pme" class="form-control select2" style="margin-top:0px; width:100%;" required>
 											...
 											<?php
												$conn = dbcon();
												$sqlDept = mysqli_query($conn,"select * from medical_pme_class");
												while ($rwDept = mysqli_fetch_array($sqlDept)) {
												?>
 						...
 						<div class="row">
 							...
 							<div class="col-md-6 col-sm-12 col-xs-12">
 								<div class="form-group">
 									...
 									<div class="col-md-8 col-sm-8 col-xs-12">
 										<input type="text" id="med_cer_date" name="med_cer_date" class="form-control calender_picker_future_date_disabled readonly" placeholder="Click here to Select Date" required>
 									</div>
 								</div>
 							</div>
 						</div>
 						...
 						<div class="row">
 							...
 							<div class="col-md-6 col-sm-12 col-xs-12">
 								<div class="form-group">
 									...
 									<div class="col-md-8 col-sm-12 col-xs-12">
 										<input type="text" id="med_ref_date" name="med_ref_date" class="form-control calender_picker_future_date_disabled readonly" placeholder="Select Date">
 									</div>
 								</div>
 							</div>
 						</div>
 				...
 				<form method="post" id="form_medical" class="apply_readonly">
 						...
 						<div class="col-md-12">
 							<?php
								if ($_SESSION['set_update_pf'] != "") {
									$conn = dbcon1();
									$last_pf = $_SESSION['set_update_pf'];
									$sql = mysqli_query($conn,"select * from medical_temp where medi_pf_number='$last_pf'");
									$result = mysqli_num_rows($sql);
									if ($result > 0) {
 					...
 					<?php
						if ($_SESSION['set_update_pf'] != "") {
							$conn = dbcon1();
							$last_pf = $_SESSION['set_update_pf'];
							$sql = mysqli_query($conn,"select * from medical_temp where medi_pf_number='$last_pf'  ORDER BY id  DESC LIMIT 1");
							$cnt = mysqli_num_rows($sql);
							if ($cnt > 0) {
								while ($result = mysqli_fetch_array($sql)) {
							...
							if ($_SESSION['set_update_pf'] != "") {
								...
								if ($cnt > 0) {
									while ($result = mysqli_fetch_array($sql)) {
										...
										$conn = dbcon1();
										$sql1 = mysqli_query($conn,"select emp_name,sr_no from biodata_temp where pf_number='$medi_pf_number'");
