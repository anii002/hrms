						...
						<form method="post" id="frmaddemployee" action="frmmultipleemp.php" enctype="multipart/form-data" accept="image/jpg,image/png,image/gif,image/jpeg">
							...
							<div class="table-responsive"><br>
								<table class='table table-striped  table-hover' id='tbl_employee' style="font-size:10px;text-align:left;">
									...
									<tbody>
										<?php
										$sqlemployee = mysqli_query($conn, "select empid,year,emplcode,dept,design,station,payscale,year,uploadfile,empname from tbl_employee order by empid asc");
										...
										?>
											<tr class="headings">
												...
												<?php
												$i = 0;
												$sql = mysqli_query($conn,"SELECT years FROM year order by id desc limit 7");
												while ($rev = mysqli_fetch_array($sql)) {

													$sql_query = mysqli_query($conn,"select scanned_img.image,scanned_apr.reporttype from scanned_img inner join scanned_apr on scanned_img.empid = scanned_apr.empid where scanned_img.empid='" . $emppf . "' and scanned_img.year='" . $rev['years'] . "'");
												...
												<?php
														...
														$sqlreason = mysqli_query($conn,"select reasontype from tbl_reason where  empcode='$emppf' AND financialyear='$demo_year'");
