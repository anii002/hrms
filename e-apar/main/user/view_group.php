			...
			<form method="POST" action="ajaxassign.php">
				<table class='table table-striped table-bordered table-hover' id='tbl_employee'>
					...
					if ($fetch = mysqli_fetch_array($query)) {
						...
						if ($ResultSet['assigning_group'] == 'on') {
							echo "<a href='reassign_group.php?gid=$G_id' class='btn bg-navy btn-flat'><i class='fa fa-mail-reply'> </i> Re-Assigned</a>";
						}
						if ($ResultSet['report_generation'] == 'on') {
							echo "&nbsp;<a href='frmgroupreport.php?gid=$G_id' class='btn btn-primary btn-flat'>
				<i class='fa fa-print'></i>&nbsp;&nbsp;&nbsp;Generate Report</a>";
						}
						if ($ResultSet['editing'] == 'on') {

							echo "&nbsp;<a href='frmaddmoreemp.php?gid=$G_id&grpname=" . $result_set['group_name'] . "' style='width:150px;' class='btn btn-warning btn-flat' ><i class='fa fa-users'></i> Edit Employee</a>
				";
						}
						echo "</td></tbody>";
						//echo $ResultSet['report_generation']." ".$_SESSION['Access_level'];
					} else {
					?>
						<tbody>
							...
							<td>
								...
								<select multiple="multiple" class="form-control" name="selection[]" id="selection">
									...
									while ($result_setuser = mysqli_fetch_array($query)) {
										echo "<option value='" . $result_setuser['userid'] . "'>" . $result_setuser['fullname'] . "</option>";
									}
									?>
