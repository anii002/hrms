					...
					<form method="get" action="#">
						<table class='table table-striped table-bordered table-hover' id='example'>
							...
							<a href='delete_group.php?id=" . $result['group_id'] . "'>Delete</a>";
								...
								$sql = mysqli_query($conn,"SELECT * FROM `group_master` where group_id in (select groupid from tbl_assignto where assignid in (select assignedid from assignto_tbl where emp_id='" . $_SESSION['SESS_USER_ID'] . "'))") or die(mysqli_error($conn));
								while ($result = mysqli_fetch_array($sql)) {
									$sql_cnt = mysqli_query($conn,"select count(distinct empid) from group_details where group_id='" . $result['group_id'] . "'");
									$sql_fetch = mysqli_fetch_array($sql_cnt);
