                ...
                <div class="box-body">
                    ...
                    <form method="post" id="frmaddemployee" action="frmmultipleemp.php" enctype="multipart/form-data" accept="image/jpg,image/png,image/gif,image/jpeg">
                        ...
                        <div class="table-responsive" style="width:100%;height:100%;overflow-x:scroll;">
                            <?php
                            // Validate and fetch value from $_GET['dept']
                            $deptname = isset($_GET['dept']) ? $_GET['dept'] : '';
                            if (!empty($deptname)) {
                                echo "<input type='hidden' value='$deptname' name='deptn'>";
                                
                                // Query database only if deptname is not empty
                                ...
                                if (mysqli_num_rows($sqlemployee) > 0) {
                                    ...
                                    while ($rwEmployee = mysqli_fetch_array($sqlemployee, MYSQLI_ASSOC)) {
                                        ...
                                        while ($rev = mysqli_fetch_array($sql)) {
                                            ...
                                            if (isset($result['image']) && !empty($result['image'])) {
                                                ...
                                                if (isset($rwReason["reasontype"]) && !empty($rwReason["reasontype"])) {
                                    ...
                                    echo "<p>No employees found for department '$deptname'.</p>";
                                }
                            } else {
                                echo "<p>No department selected or invalid department name.</p>";
                            }
                            ?>
                        </div>
                    </form>
                    ...
                    <?php
                    // Check if $_POST['submit'] is set before using it
                    if (isset($_POST['submit'])) {
                        if (isset($_POST['year_list'])) {
                            ...
                            echo "No year list data received.";
                        }
                    }
                    ?>
