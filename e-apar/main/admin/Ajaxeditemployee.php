  <?php
	...
	if (isset($_POST["btnbasic"])) {
		$sql_fetch = mysqli_query($conn, "SELECT * FROM scanned_apr WHERE empid='$empcode'");
		if ($result = mysqli_fetch_array($sql_fetch)) {
			if (mysqli_query($conn, "update tbl_employee set contact='$contact', dept='$dept',design='$designation',station='$station',payscale='$payscale',empname='$empname',substantive='$substantive',sevencpcpaylevel='$sevencpcpaylevel',stsc='$stsc',modifiedby='$session',modifieddate=NOW() where empid=$empid") && mysqli_query($conn, "update scanned_apr set dept='$dept',designation='$designation'
		...
		if (
			mysqli_query($conn, "update tbl_employee set contact='$contact', dept='$dept',design='$designation',station='$station',payscale='$payscale',integrity='$interity',empname='$empname',substantive='$substantive',sevencpcpaylevel='$sevencpcpaylevel',stsc='$stsc',modifiedby='$session',modifieddate=NOW() where empid='$empid'")
			&&
			mysqli_query($conn, "insert into scanned_apr(year,empid,empname,dept,designation,station,payscale,integrity,substantive,sevencpcpaylevel,
