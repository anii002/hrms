...
dbcon();
	...
	$cmbselect = $_POST["cmbselect"];
	$ncmbselect = $_POST["ncmbselect"];
	$txtnotifi = $_POST["txtnotifi"];
	$cmbselect2 = $_POST["cmbselect2"];
				...
				<table class='table table-striped table-bordered table-hover' id='example'  >
						...
						while($rwGroup=mysql_fetch_array($sql_group))
						{
							...
							while($rev = mysql_fetch_array($sql))
							{
								...
								<td><?php echo "<input type='text' size='10' value=".$rwyear["totalgrade"]." style='border:0px;' readonly>"; $cnt=$cnt+$rwyear["totalgrade"]; ?></td>
								<?php
								$record[$j++]=$rwyear["totalgrade"];
								}
								else
								{
									$get_id=$_POST["ngrade$num"];
									$record[$j++]=$get_id;
									echo "<td><input type='text' style='border:0px' value='".$_POST["ngrade$num"]."' ></td>";
									$num++;
								}
								
							}
						
							 // echo "<td><input type='text' style='border:none;' size='4' value='".$_POST["nvalue$rowgrpid"]."' readonly></td>";
							// $record[$j++]=$_POST["nvalue$rowgrpid"];
