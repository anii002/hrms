					...
					<tbody>
						...
						<td>
							...
							<select multiple="multiple" class="form-control" name="selection[]" id="selection">
								<?php
								$query = mysqli_query($conn, "select * from tbl_user where userid not in(select emp_id from assignto_tbl where assignedid in (select assignid from tbl_assignto where groupid='$G_id'))");
...
if (isset($_POST["sub"])) {
	...
	$query = mysqli_query($conn, "select * from tbl_assignto where groupid='$groupid'");
