				...
				<div class="box-body">
					<form method="post" id="frmaddemployee" action="frmmultipleemp.php" enctype="multipart/form-data" accept="image/jpg,image/png,image/gif,image/jpeg">
						...
						<thead>
							...
							<td style='display:none;' id='tdempid$empid'><input type='checkbox' name='check_list[]' value='$empid'/></td>
							...
							<td id='tdstation$empid'>$payscale</td>";
								...
								while ($rev = mysqli_fetch_array($sql)) {
									...
									$sql_query = mysqli_query($conn,"select * from scanned_img where empid='" . $empid . "' AND year='" . $rev['years'] . "'");
							...
							?>
									...
									<?php
										...
										$sqlreason = mysqli_query($conn,"select * from tbl_reason where  empcode='$empid' AND financialyear='$demo_year'");
										$rwReason = mysqli_fetch_array($sqlreason);

										if (isset($rwReason["reasontype"]) != '') {
											echo "<td id='tduploadfile$empid'><a href='frmshowreasone.php?emppf=$empid&year=$demo_year&empid=$empid'  id='$empid'>Reason</a></td>";
										} else {
