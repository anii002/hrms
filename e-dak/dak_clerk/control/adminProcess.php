...
switch ($_REQUEST['action']) {
  ...
  case 'fetch_empid':
    $data = '';
    $query_id = mysql_query("SELECT emp_id from tbl_user where section='" . $_POST['id'] . "'", $db_edak);
    $fetch_id = mysql_fetch_array($query_id);
  ...
  case 'add_edak':
    ...
    $gist_letter=mysql_real_escape_string($_POST['gist_letter']);
    $rd_from = mysql_real_escape_string($_POST['rd_from']);
    $query_id = mysql_query("INSERT INTO `tbl_dak`(`unique_dak_no`, `recd_from`, `dt_of_letter`, `gist_of_letter`, `marked_to`, `curr_date`, `source`, `station_id`,`status`, `added_by`) VALUES ('" .$u_dak_no  . "','" . $rd_from . "','" . $_POST['dt_letter'] . "','" . $gist_letter . "','" . $_POST['eid'] . "','" . $_POST['cur_date'] . "','" . $_POST['src'] . "','".$_POST['station']."','1','" . $_SESSION['emp_id'] . "')", $db_edak);

    $dak_forward = mysql_query("INSERT INTO `tbl_dak_forward`(`unique_dak_no`, `marked_from`, `marked_to`, `station_id`,`forwarded_date`,  `status`) VALUES ('" . $u_dak_no. "','" . $_SESSION['emp_id'] . "','" . $eid . "','".$_POST['station']."','" . $datet . "','1')", $db_edak);
  ...
  $query_id=mysql_query("Update tbl_dak set unique_dak_no='".$u_dak_no."',dt_of_letter='".$dt_letter."',gist_of_letter='".$gist_letter."',marked_to='".$marked_to."',updated_date='".$up_date."',source='".$src."' where id='".$_POST['tbl_dak_id']."'",$db_edak);


  $query_id2=mysql_query("Update tbl_dak_forward set unique_dak_no='".$u_dak_no."',marked_from='".$marked_from."',marked_to='".$marked_to."',forwarded_date='".$fw_date."' where id='".$_POST['tbl_dak_forward_id']."'",$db_edak);
