                    ...
                    <table class="table table-bordered table-stripped table-responsive" id="example1">
                        ...
                        <tbody>
                            ...
                            while ($row = mysqli_fetch_array($rst_emp)) {
                                ...
                                $sql = "SELECT id,type_name,created_date,status from tbl_ack_sorder where form_reference_id='" . $row['form_reference_id'] . "' and emp_id='" . $_SESSION['emp_id'] . "'";
                                $ackmt_tbl = mysqli_query($db_edar,$sql);
                                // var_dump($ackmt_tbl);
                                $fetch_ack = mysqli_fetch_array($ackmt_tbl);
                                if ($fetch_master["ack_ids"] == null) {
                                    if ($c > 0) {
                                        ...
                                        echo "<td class='text-danger'>You in given days not replied acknowledgement and explanation.</td>";
                                    }
                                } else if ($fetch_master['ack_given'] == '1' && $fetch_master['expl_given'] == null) {
                                    ...
                                    $d = (int) $expl_interval->format('%R%a');
                                    if ($d > 0) {
                                ...
                                } else if ($fetch_master['ack_given'] == '1' and $fetch_master['expl_given'] == '1') {
                                    $fw_date1 = $fetch_ack['created_date'];
                                    $current_date1 = date_create(date('d-m-Y'));
