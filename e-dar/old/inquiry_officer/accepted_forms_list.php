                ...
                <div class="portlet-body">
                    <table class="table table-bordered table-stripped table-responsive">
                        ...
                        <tbody>
                            ...
                            // $sql = "SELECT DISTINCT(fw.emp_pf),approved_date FROM `tbl_form_forward` fw INNER JOIN tbl_form_master_entry f_m_e WHERE f_m_e.current_status IN ('3','5','6','7','8') AND fw.fw_from='$current_id' AND fw.approved_date is not NULL AND NOW() < (SELECT DATE_ADD(DATE(`approved_date`),INTERVAL 10 DAY) AS DATEADDED from tbl_form_forward WHERE fw_from='$current_id' AND status='3' ORDER BY id DESC LIMIT 1) ORDER BY fw.id DESC";
                            $sql = "SELECT DISTINCT(fw.emp_pf),approved_date FROM `tbl_form_forward` fw INNER JOIN tbl_form_master_entry f_m_e ON f_m_e.form_ref_id=fw.form_reference_id WHERE f_m_e.current_status IN ('3','5','6','7','8') AND fw.fw_from='$current_id' AND fw.approved_date is not NULL AND NOW() < (SELECT DATE_ADD(DATE(`approved_date`),INTERVAL 10 DAY) AS DATEADDED from tbl_form_forward WHERE fw_from='$current_id' AND status='3' ORDER BY id DESC LIMIT 1) ORDER BY fw.id DESC";
                            ...
                            if (mysql_num_rows($rst_pending_list) > 0) {
                                ...
                                while ($rw_pending_list = mysql_fetch_assoc($rst_pending_list)) {
                                    ...
                                    <tr>
                                        <td>$sr</td>
                                        <td>$emp_name</td>
                                        <td><a href="view_forms_details_new.php?display=view&emp_pf=$emp_pf" class="btn btn-info"><i class="fa fa-eye"></i></a>
                                        </td>
                                    </tr>
xyz;
                                    $sr++;
