...
switch ($action) {
    ...
    case "forward_form":
        ...
        $hold_status = 1;
        if (forward_forms($emp_pf, $fw_emp_id, $fw_remark, $hold_status)) {
            $Result["res"] = "success";
        }
        echo json_encode($Result);
    ...
    case 'delete_frms':
        ...
        if (($key = array_search($form_id, $frm_ids)) !== false) {
            ...
            $update_master_frm = mysql_query("UPDATE tbl_form_master_entry set form_ids='" . $removed_frm_id . "' where emp_pf='" . $emp_pf . "' form_ref_id='" . $ref_id . "'", $db_edar);
