...
function save_office_note(
    ...
    $rst_query = mysql_query($check_query, $db_edar);
    if (mysql_num_rows($rst_query) > 0) {
...
function update_master_entry_office_id($emp_pf, $form_ref_id)
{
    ...
    $rst_check = mysql_query($sql_check, $db_edar);
    if (mysql_num_rows($rst_check) > 0) {
        $rw_check = mysql_fetch_assoc($rst_check);
        $id = $rw_check["id"];
...
function accept_io_form($emp_pf, $current_status)
{
    ...
    if (mysql_query($sql_clerk, $db_edar)) {
        if (update_master_entry_status($current_status, $emp_pf, $emp_form_ref)) {
            return true;
        }
    }
...
function add_sorder($emp_pf, $emp_form_ref, $type_id, $desc_note)
{
    ...
    if (mysql_num_rows($rst_check_query) > 0) {
        ...
        $query = mysql_query("UPDATE `tbl_ack_sorder` SET `desc_note`='$desc_note' WHERE id='$check_id' and `form_reference_id`='$emp_form_ref' and `emp_id`='$emp_pf'", $db_edar);
    } else {
        $query = mysql_query("INSERT INTO `tbl_ack_sorder`(`form_id`, `form_reference_id`, `emp_id`, `type_name`, `desc_note`, `created_date`, `status`) VALUES ('$form_ids','$emp_form_ref','$emp_pf','$type_id','$desc_note','$current_date','1')", $db_edar);
        ...
        $sql_tbl_note = mysql_query("SELECT * from tbl_ack_sorder where emp_id='$emp_pf' and form_reference_id='$emp_form_ref'", $db_edar);
        ...
        if ($fetch_sql['speaking_ids'] == null) {
            $sql_up = mysql_query("UPDATE tbl_form_master_entry set speaking_ids='" . $id . "' where emp_pf='$emp_pf' and form_ref_id='$emp_form_ref'", $db_edar);
        } else {

            $note_id = implode(",", $push);
            $sql_up = mysql_query("UPDATE tbl_form_master_entry set speaking_ids='" . $note_id . "' where emp_pf='$emp_pf' and form_ref_id='$emp_form_ref'", $db_edar);
        }
    }
...
function forward_da_to_emp($emp_pf, $fw_emp_id, $fw_remark, $status)
{
    ...
    if (mysql_query($sql, $db_edar)) {
        $update_all_frm_id = mysql_query("UPDATE tbl_form_details set inquiry_o_pf='$fw_emp_id' where emp_id='$emp_pf' and  form_reference_id='$ref_id'", $db_edar);
