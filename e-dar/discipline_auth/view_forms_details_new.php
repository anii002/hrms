                ...
                <div class="portlet-body">
                    ...
                    <div class="row pull-right rp-20">
                        <?php if (isset($_GET["baction"])) {
                            ...
                            if (in_array("ioaccept", $baction_array)) {
                                echo "<button class=\"btn btn-success btn-space\" id=\"btn_accept\" data-toggle=\"modal\" data-target=\"#mdl_io_accept\">Accept</button>";
                            }
                            if (in_array("ioreject", $baction_array)) {
                                echo "<button class=\"btn btn-danger btn-space\" id=\"btn_reject\" data-toggle=\"modal\"
                                data-target=\"#mdl_io_reject\">Reject</button>";
                            }
...
$(document).ready(function() {
    ...
    $("#btn_add_speaking_order").click(function(e) {
        e.preventDefault();
        var emp_pf = $("#lst_emp_pf").val();
        if (emp_pf == 0 || emp_pf == null) {
            alert("Please Select the employee");
            ...
            // alert(emp_pf);
            window.location = "add_speaking.php?emp_pf=" + emp_pf;
        }
    });
    $("#btn_add_office_note").click(function(e) {
        e.preventDefault();
        var emp_pf = $("#lst_emp_pf").val();
        if (emp_pf == 0 || emp_pf == null) {
            alert("Please Select the employee");
            ...
            // alert(emp_pf);
            window.location = "add_office_note.php?emp_pf=" + emp_pf;
        }
    });
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                ...
                if (Response.res == "success") {
                    ...
                    AlertBox("Success", "Form Forwarded Successfully!");
                    setTimeout(() => {
    ...
    // ! DA Accept which is send by io
    $("#mdl_form_io_accept").submit(function(e) {
        e.preventDefault();
        var postData = new FormData();
        var postData = new FormData($(this)[0]);
        var emp_pf = $("#lst_emp_pf").val();
        postData.append("emp_pf", emp_pf);
        postData.append("action", "accept_io_form");
        $.ajax({
            url: "control/da_process.php",
            type: 'POST',
            data: postData,
    ...
    // !Reject case which is send by IO
    $("#mdl_form_io_reject").submit(function(e) {
        e.preventDefault();
        if ($("#mld_reject_io_remark").val() == '') {
            AlertBox("Error", "Please Ener the Reason of Rejecting?");
            $("#mld_reject_io_remark").focus();
        } else {
            var postData = new FormData();
            var postData = new FormData($(this)[0]);
            var emp_pf = $("#lst_emp_pf").val();
            postData.append("emp_pf", emp_pf);
            postData.append("action", "reject_io_form");
            $.ajax({
                url: "control/da_process.php",
                type: 'POST',
                data: postData,
...
<div id="mdl_forward" class="modal modal-width fade modal-scroll" data-replace="true" tabindex="-1" role="dialog"
    ...
    <form action="control/da_process.php" method="post" id="mdl_fm_forward">
...
<div id="mdl_io_reject" class="modal modal-width fade modal-scroll" data-replace="true" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalLabel" aria-hidden="true">
    <form action="control/da_process.php" method="post" id="mdl_form_io_reject">
        <div class="modal-header btn-orange-moon">
            <button type="button" class="close" data-dismiss="modal" aria-label=""><span>×</span></button>
            <h4 class="modal-title" style="text-align:center">Rejected Form</h4>
        </div>
        <div class="modal-body">
            <div class="row">
                <div class="col-md-3">
                    <label for="mld_reject_io_remark">Reject Reason: </label>
                </div>
                <div class="col-md-9">
                    <textarea name="mld_reject_io_remark" class="form-control" id="mld_reject_io_remark" cols="60"
                        rows="3"></textarea>
                </div>
            </div>
        </div>
...
<!--//? : Accept IO Confirmation Modal   -->

<div id="mdl_io_accept" class="modal modal-width fade modal-scroll" data-replace="true" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalLabel" aria-hidden="true">
    <form action="control/da_process.php" method="post" id="mdl_form_io_accept">
        <div class="modal-header btn-orange-moon">
            <button type="button" class="close" data-dismiss="modal" aria-label=""><span>×</span></button>
            <h4 class="modal-title" style="text-align:center">Accept Form</h4>
        </div>
