...
if (isset($_REQUEST['action'])) {
	switch (strtolower($_REQUEST['action'])) {  
		...
		case 'get_prft_promotion_2':
		  ...
		  $qry=mysql_query("SELECT * from prft_promotion_temp where pro_pf_no='$prft_pf_no' and id='$prft_id'");
			while($fetch=mysql_fetch_assoc($qry))
			{
				...
				$data['pro_letter_date']=$fetch['pro_letter_date'];
				$data['pro_wef']=$fetch['pro_wef'];
				...
				$data['pro_carri_wef']=$fetch['pro_carri_wef'];
				$data['pro_carri_date_of_incr']=$fetch['pro_carri_date_of_incr'];
				$data['pro_car_re_accept_ltr_no']=$fetch['pro_car_re_accept_ltr_no'];
				$data['pro_car_re_accept_ltr_date']=$fetch['pro_car_re_accept_ltr_date'];
				$data['pro_car_re_wef_date']=$fetch['pro_car_re_wef_date'];
		  ...
		  $data['rev_letter_date']=$fetch['rev_letter_date'];
		  $data['rev_wef']=$fetch['rev_wef'];
		  ...
		  $data['rev_carri_wef']=$fetch['rev_carri_wef'];
		  $data['rev_carri_date_of_incr']=$fetch['rev_carri_date_of_incr'];
		  $data['rev_car_re_accept_ltr_no']=$fetch['rev_car_re_accept_ltr_no'];
		  $data['rev_car_re_accept_ltr_date']=$fetch['rev_car_re_accept_ltr_date'];
		  
		  $data['rev_car_re_wef_date']=$fetch['rev_car_re_wef_date'];
			...
			if($sql_fetch>0)
			{
				while($result=mysql_fetch_array($sql))
				{
				  ...
				  $data['trans_letter_no']=$result['trans_letter_no'];
					  $data['trans_letter_date']=$result['trans_letter_date'];
					//$data['ps_type']=$result['ps_type'];
					$data['trans_wef']=$result['trans_wef'];
					...
					//Yes
					$data['trans_carri_wef']=$result['trans_carri_wef'];
				    $data['trans_carri_date_of_incr']=$result['trans_carri_date_of_incr'];
				    //No
				    $data['trans_car_re_accept_ltr_no']=$result['trans_car_re_accept_ltr_no'];
				    $data['trans_car_re_accept_ltr_date']=$result['trans_car_re_accept_ltr_date'];
				    $data['trans_car_re_wef_date']=$result['trans_car_re_wef_date'];
		...
		//fixation process_main
		 ...
		 case 'get_prft_fixtion':
			...
			// echo "select * from prft_fixation_temp where fix_pf_no='$pf_no'".mysql_error();
			$sql_fetch=mysql_num_rows($sql);
			if($sql_fetch>0)
			{
				while($result=mysql_fetch_array($sql))
				{
					...
					$data['fix_letter_date']=$result['fix_letter_date'];
					
					$data['fix_wef']=$result['fix_wef'];
					...
					$data['fix_carri_wef']=$result['fix_carri_wef'];
				    $data['fix_carri_date_of_incr']=$result['fix_carri_date_of_incr'];


				    $data['fix_car_re_accept_ltr_no']=$result['fix_car_re_accept_ltr_no'];
				    $data['fix_car_re_accept_ltr_date']=$result['fix_car_re_accept_ltr_date'];
				    $data['fix_car_re_wef_date']=$result['fix_car_re_wef_date'];
		...
		break;
		 ...
		 //Online Office Order 
		  ...
		  date_default_timezone_set('Asia/Kolkata');
			...
			$zone='01';
			$division='SUR';
			$odr_type=$_POST['o_o_for'];
			$office_order_dept=$_POST['o_dept'];
			$office_order_desig=$_POST['o_desig'];
			$order_date=date('Y-m-d', strtotime($_POST['o_order_date']));
			$officee_order_no=$_POST['o_order_no'];
			$subject=$_POST['o_order_sub'];
			$description=$_POST['o_order_descp'];
			$long_description=$_POST['o_long_desc'];
			$qu=mysql_query('select count from online_office_order order by id desc limit 1');
			$fetch=mysql_fetch_array($qu);
			if($fetch['count']=='')
			{
				$counter=$counter + 1;
			}
			...
			{
				$counter=$fetch['count'] + 1;
			}
			
			for($i=1;$i<=$row_count;$i++)
			{
			$emp_name=$_POST['o_order_emp_name_'.$i];
			$emp_old_desig=$_POST['o_order_present_desig_'.$i];
			$emp_old_ps_type=$_POST['o_order_old_ps_type_'.$i];
			
			if($emp_old_ps_type==1)
			{
				$old_ps=$_POST['o_order_old_3scale_'.$i];
			}
			else if($emp_old_ps_type==2 || $emp_old_ps_type==3 || $emp_old_ps_type=4)
			{
					$old_ps=$_POST['o_order_old_ps_'.$i];
			}
				$old_lvl=$_POST['o_order_old_level_'.$i];			
			$emp_new_desig=$_POST['o_order_new_desig_'.$i];
			$emp_new_ps_type=$_POST['o_order_new_ps_type_'.$i];
				if($emp_new_ps_type==1)
			{
				$new_ps=$_POST['o_order_new_3scale_'.$i];
			}
			else if($emp_new_ps_type=2 || $emp_new_ps_type==3 || $emp_new_ps_type==4)
			{
				$new_ps=$_POST['o_order_new_ps_'.$i];
			}
			$new_ps_lvl=$_POST['o_order_new_level_'.$i];
			$remark=$_POST['o_order_remark_'.$i];
			
				$sql="insert into `online_office_order`(`zone`, `division`, `trans_id`,`order_type`,`office_order_dept`, `office_order_desig`, `order_date`, `officee_order_no`, `subject`, `description`, `emp_name`, `emp_old_desig`, `emp_old_ps_type`, `emp_old_scale`, `emp_old_lvl`, `emp_new_desig`, `emp_new_ps_type`, `emp_new_scale`, `emp_new_lvl`, `remark`, `long_description`,`date_time`,`count`)values('01','SUR','$trans_id','$odr_type','$office_order_dept','$office_order_desig','$order_date','$officee_order_no','$subject','$description','$emp_name','$emp_old_desig','$emp_old_ps_type','$old_ps','$old_lvl','$emp_new_desig','$emp_new_ps_type','$new_ps','$new_ps_lvl','$remark','$long_description',Now(),'$counter')";
			...
			if($result)
		   {
				 echo "<script>window.location='online_office_order.php';alert('Online Office Order Added 	Successfully');</script>";   
		   }
		   else
		   {
				 echo "<script>window.location='online_office_order.php';alert('Not Added');</script>";
		   }
		 break;
		...
		case 'update_training':
           ...
           if($hidden_tr_count=="")
			...
			else
			{
				...
				$f_q=mysql_query("select count from training_temp where pf_number='".$tra_pf_no."' and id='".$hidden_training."'");
			...
			{
				echo "<script>window.location='training_update.php';alert('NOT Updated');</script>";
			}
		break;
		...
		{	
			...
			if($sq){
					...
					while($fetch_sql=mysql_fetch_array($sq))
					{
						...
						{
							$sql1=("insert into `penalty_track`(`zone`,`division`,`temp_transaction_id`,`final_transaction_id`,`pen_pf_number`,`pen_oldpf_number`, `pen_type`, `pen_issued`, `pen_effetcted`, `pen_letterno`, `pen_letterdate`, `pen_chargestatus`, `pen_chargeref`, `pen_from`, `pen_to`, `pen_remark`, `updated_by`, `date_time`, `updated_fields`, `updated_reason`, `updated_datetime`,`letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`)values('01','SUR','$trans_id','$trans_id','$pen_pf_no','','$pen_type','$pen_issued','$pen_effected','$pen_letter_no','$pen_letter_date','$pen_chargesheet_status','$pen_chargesheet_ref','$pen_from_date','$pen_to_date','$pen_remark','$update_by','Now()','','','','','','','','','')");
		...
		case 'update_increment':
			...
			//	echo "inc var".$incr_level;
				//echo "inc pst".$_POST['level_drop_'.$i];
				...
				echo "select * from `increment_temp` where `incr_pf_number`='$incr_pf'".mysql_error();
				$fetch_sql=mysql_num_rows($sql);
				...
				{	
					...
					while($fetch_sql=mysql_fetch_array($sq))
					{
						...
						else
						{
							...
							//echo "insert into `increment_track`(`temp_transaction_id`,`final_transaction_id`, `zone`, `division`, `incr_pf_number`, `incr_type`, `incr_date`, `ps_type`, `incr_scale`, `incr_level`, `incr_oldrop`, `incr_rop`, `incr_personel`, `incr_special`, `incr_nextdate`, `incr_remark`, `date_time`, `updated_by`,  `updated_fields`, `updated_reason`, `updated_datetime`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`,`count`)values('$transaction_id','$final_transaction_id','$zone','$division','$incr_pf','$incr_type','$incr_date','$ps_type_4','$incr_scale','$incr_level','','$incr_rop','','','','$incr_remark',Now(),$updated_by,'','','','','','','','','','$count')".mysql_error();
				...
				//echo "UPDATE `increment_temp` SET `temp_transaction_id`='$transaction_id',`zone`='$zone',`division`='$division',`incr_pf_number`='$incr_pf',`incr_type`='$incr_type',`incr_date`='$incr_date',`ps_type`='$ps_type_4',`incr_scale`='$incr_scale',`incr_level`='$incr_level',`incr_rop`='$incr_rop',`incr_remark`='$incr_remark',`updated_by`='$updated_by',`date_time`=Now() WHERE incr_pf_number='$incr_pf' and id='$incr_hidden_id'".mysql_error();
		...
		case 'update_initial_medical':
				...
				//echo "insert into medical_temp(temp_transaction_id,zone,division,medi_pf_number,medi_examtype,medi_cate,medi_class,medi_design,medi_certino,medi_certidate,medi_refno,medi_refdate,medi_remark,datetime,updated_by,medi_dob,medi_appo_date)values('$trans_id','01','SUR','$medi_pf_no','$medi_exam','$medi_cat','$medi_cat_pme','$in_med_desig','$medi_cer_no','$med_cer_date','$med_ref','$med_ref_date','$med_remark',NOW(),'$update_by','$med_dob','$med_appo_date')".mysql_error();
				
				
				$sql2=mysql_query("insert into medical_track(zone,temp_transaction_id,final_transaction_id,medi_pf_number,medi_examtype,medi_cate,medi_class,medi_design,medi_certino,medi_certidate,medi_refno,medi_refdate,medi_remark,datetime,updated_by,medi_dob,medi_appo_date)values('01','$trans_id','$trans_id','$medi_pf_no','$medi_exam','$medi_cat','$medi_cat_pme','$in_med_desig','$medi_cer_no','$med_cer_date','$med_ref','$med_ref_date','$med_remark',NOW(),'$update_by','$med_dob','$med_appo_date')");
				
				//echo "insert into medical_track(zone,temp_transaction_id,final_transaction_id,medi_pf_number,medi_examtype,medi_cate,medi_class,medi_design,medi_certino,medi_certidate,medi_refno,medi_refdate,medi_remark,datetime,updated_by,medi_dob,medi_appo_date)values('01','$trans_id','$trans_id','$medi_pf_no','$medi_exam','$medi_cat','$medi_cat_pme','$in_med_desig','$medi_cer_no','$med_cer_date','$med_ref','$med_ref_date','$med_remark',NOW(),'$update_by','$med_dob','$med_appo_date')".mysql_error();
	...
	case 'add_biodata':
		  date_default_timezone_set('Asia/Kolkata');
			...
			$all_bio_mark = addslashes(implode(',',$bio_mark));			
			...
			$bio_reis=$_POST['bio_reis'];
			$bio_bank_address=$_POST['bio_bank_address'];
			session_start();
			$update_by=$_SESSION['id'];
			...
			$newfilename = $bio_pf_no.".jpg";
			$folder_name="upload_doc/"; 
			...
			if($result1 && $result2)
			{
				    echo "<script>window.location='sr_entry.php';alert('Bio Data Added Successfully');</script>";
			}else
			{
				  echo "<script>window.location='sr_entry.php';alert('Bio Data NOT Added Successfully');</script>";
			} 
		break;
		...
		break;
		 ...
		 case 'add_training':
			...
			for($i=1;$i<=$training_count;$i++)
			{
				...
				//echo "insert into `training_track`(`zone`, `division`, `temp_transaction_id`, `final_transaction_id`, `pf_number`, `last_date`, `training_from`, `letter_no`, `description`, `remarks`, `training_type`,`tr_inst`,`tr_dept`,`tr_desig`, `due_date`, `training_to`, `letter_date`, `updated_by`, `date_time`, `updated_fields`, `updated_reason`,`uploaded_status`, `updated_datetime`, `letter_number`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`, `count`)values('01','SUR','$trans_id','$trans_id','$tra_pf_no','$tra_last_date','$tra_from','$tra_letter_no','$tra_desc','$tra_remarks','$tra_type','$inst','$tr_dept','$tr_desig','$tra_due_date','$tra_to','$tra_letter_date','$update_by',Now(),'','','','','','','','','','','$i')".mysql_error();
				
				//echo "insert into `training_temp`(`temp_transaction_id`,`zone`,`division`,`pf_number`,`old_pf_number`, `last_date`, `training_type`,`tr_inst`,`tr_dept`,`tr_desig`, `due_date`, `training_from`, `letter_no`, `letter_date`, `description`, `remarks`, `updated_by`, `date_time`, `updated_fields`, `updated_reason`, `updated_datetime`, `letter_number`, `letter_datetime`,`uploaded_letter`,`uploaded_status`, `approved_by`, `approved_datetime`, `training_to`, `count`)values('$trans_id','01','SUR','$tra_pf_no','','$tra_last_date','$tra_type','$inst','$tr_dept','$tr_desig','$tra_due_date','$tra_from','$tra_letter_no','$tra_letter_date','$tra_desc','$tra_remarks','$update_by',Now(),'','','','','','','','','','$tra_to','$i')".mysql_error();
			}
		...
		{ 
			...
			if($i > $pre_result)
			{
				...
				//$sql2=mysql_query("INSERT INTO `family_track`(`zone`, `division`, `temp_transaction_id`, `final_transaction_id`, `emp_pf`, `fmy_pf_number`,`fmy_updatedate`, `fmy_member`, `fmy_rel`, `fmy_gender`, `fmy_dob`, `updated_by`, `date_time`) VALUES (01,'SUR','$trans_id','$trans_id','$fc_pf_main','$fc_pf_no','$fc_updated_date','$fc_fam_mem_name','$fc_mem_rel','$fc_mem_gender','$fc_fam_mem_dob','$update_by',Now())");
		...
		session_start();
		$update_by=$_SESSION['id'];
		$pf_no=$_POST['pd_pf_no'];
		if($pro_count=="")
		{
			$pro_count=1;
		}
			
		for($i=1; $i <= $pro_count; $i++)
		{
			$src = $_REQUEST['pd_sourcr_type'.$i];
			$src_2 = implode(',',$src);
			...
			$prop_type=$_POST['pd_pro_type'.$i];
			$item=$_POST['pd_item_name'.$i];
			$other_item=$_POST['pd_othr_item_name'.$i];
			$mkmd=$_POST['pd_make_model'.$i];
			$dop=date('Y-m-d', strtotime($_POST['pd_dop'.$i]));
			$loc=$_POST['pd_location'.$i];
			$reg=$_POST['pd_reg_no'.$i];
			$area=$_POST['pd_area'.$i];
			$surno=addslashes($_POST['pd_sur_no'.$i]);
			$totcost=$_POST['pd_total_cost'.$i];
			//$source=$_POST['pd_source'.$i];
			//$source_type=$_POST['pd_sourcr_type'.$i];
			//$amount=$_POST['pd_source_amt'.$i];
			$letter_no=$_POST['pd_letter_no'.$i];
			$letter_date=date('Y-m-d', strtotime($_POST['pd_letter_date'.$i]));
			$remarks=$_POST['prop_remark'.$i];
			
			$sql1=("insert into `property_temp` (`temp_transaction_id`,`zone`,`division`,`pro_pf_number`,`pro_oldpf_number`,`pro_type`,`pro_item`,`pro_otheritem`,`pro_make`,`pro_dop`,`pro_location`,`pro_regno`,`pro_area`,`pro_surveyno`,`pro_cost`,`pro_source`,`pro_sourcetype`,`pro_amount`,`pro_letterno`,`pro_letterdate`,`pro_remark`,`updated_by`,`date_time`,`updated_fields`,`updated_reason`,`updated_datetime`,`letter_no`,`letter_datetime`,`uploaded_letter`,`approved_status`,`approved_by`,`approved_datetime`,`count`)values('$trans_id',01,'SUR','$pf_no','','$prop_type','$item','$other_item','$mkmd','$dop','$loc','$reg','$area','$surno','$totcost','','$src_2','$amt2','$letter_no','$letter_date','$remarks','$update_by',Now(),'','','','','','','','','','$i')");
			
			$sql2=("insert into `property_track` (`zone`,`division`,`temp_transaction_id`,`final_transaction_id`,`pro_pf_number`,`pro_oldpf_number`,`pro_type`,`pro_item`,`pro_otheritem`,`pro_make`,`pro_dop`,`pro_location`,`pro_regno`,`pro_area`,`pro_surveyno`,`pro_cost`,`pro_source`,`pro_sourcetype`,`pro_amount`,`pro_letterno`,`pro_letterdate`,`pro_remark`,`updated_by`,`date_time`,`updated_fields`,`updated_reason`,`updated_datetime`,`letter_no`,`letter_datetime`,`uploaded_letter`,`approved_status`,`approved_by`,`approved_datetime`,`count`)values(01,'SUR','$trans_id','$trans_id','$pf_no','','$prop_type','$item','$other_item','$mkmd','$dop','$loc','$reg','$area','$surno','$totcost','','$src_2','$amt2','$letter_no','$letter_date','$remarks','$update_by',Now(),'','','','','','','','','','$i')");
		...
		if($res1 && $res2)
			{
				echo "<script>window.location='sr_entry.php';alert('Property Details Added Successfully');</script>";
			}
			else{
				echo "<script>window.location='sr_entry.php';alert('Property Details NOT Added Successfully');</script>";
			}
		break;
		
		case 'add_present_work_detail':
				...
				if($sql && $sq2)
				{
					  echo "<script>alert('Present Working Details Added Successfully');window.location='sr_entry.php'</script>";
				}else{
					  echo "<script>alert('Present Working Details NOT Added Successfully');window.location='sr_entry.php'</script>";
				} 
		break;
		...
		case 'add_appointment':
				...
				$app_remark=addslashes($_POST['app_remark']);
				$app_otherstation=$_POST['app_otherstation'];				
				$app_bill_unit=$_POST['depot3'];
				date_default_timezone_set('Asia/Kolkata');
		...
		case 'add_increment':
			...
			{	
				...
				$sql1=mysql_query("insert into `increment_temp`(`temp_transaction_id`, `zone`, `division`, `incr_pf_number`, `incr_type`, `incr_date`, `ps_type`, `incr_scale`, `incr_level`, `incr_oldrop`, `incr_rop`, `incr_personel`, `incr_special`, `incr_nextdate`, `incr_remark`, `date_time`, `updated_by`,  `updated_fields`, `updated_reason`, `updated_datetime`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`)values('$transaction_id','$zone','$division','$incr_pf','$incr_type','$incr_date','$ps_type_4','$incr_scale','$incr_level','$incr_oldrop','$incr_rop','$incr_personel','$incr_special','$incr_nxt_incr','$incr_remark',Now(),$updated_by,'','','','','','','','','')");
			
				$sql2=mysql_query("insert into `increment_track`(`temp_transaction_id`,`final_transaction_id`, `zone`, `division`, `incr_pf_number`, `incr_type`, `incr_date`, `ps_type`, `incr_scale`, `incr_level`, `incr_oldrop`, `incr_rop`, `incr_personel`, `incr_special`, `incr_nextdate`, `incr_remark`, `date_time`, `updated_by`,  `updated_fields`, `updated_reason`, `updated_datetime`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`)values('$transaction_id','$final_transaction_id','$zone','$division','$incr_pf','$incr_type','$incr_date','$ps_type_4','$incr_scale','$incr_level','$incr_oldrop','$incr_rop','$incr_personel','$incr_special','$incr_nxt_incr','$incr_remark',Now(),$updated_by,'','','','','','','','','')");
			}
			if($sql1 && $sql2){
				 echo "<script>window.location='sr_entry.php';alert('Increment Recorded Successfully');</script>";
			}else{
				 echo "<script>window.location='sr_entry.php';alert('NOT Recorded');</script>";
			}
		 break;
		 ...
		 case 'add_lastentry';
		  date_default_timezone_set('Asia/Kolkata');
		   ...
		   $le_retirement_date=date('Y-m-d', strtotime($_POST['le_retirement_date']));
		   $le_des_retire=$_POST['le_des_retire'];
		   $le_dept=$_POST['le_dept'];
		   $station11=$_POST['station11'];
		   $le_rop=$_POST['le_rop'];
		   $billunit20=$_POST['billunit20'];
		   $depot20=$_POST['depot20'];
		   ...
		   $sql="insert into `lastentry_temp`(`zone`, `division`, `trans_id`, `pf_number`, `old_pf_number`, `date_of_join`, `retire_type`, `retire_date`, `retire_designation`, `department`, `station`, `rop`, `bill_unit`, `scale`, `depot`, `emp_category`, `total_years`,`total_months`, `total_days`, `no_years`, `no_months`, `no_days`, `lap`, `lhap`, `advance_leave`,`updated_by`,`date_time`,`updated_fields`,`updated_reason`,`updated_datetime`,`letter_no`,`letter_datetime`,`uploaded_letter`,`approved_status`,`approved_by`,`approved_datetime`)values('01','SUR','$trans_id','$le_pf_no','','$le_doj','$le_retiredment_type','$le_retirement_date','$le_des_retire','$le_dept','$station11','$le_rop','$billunit20','$le_scale_level','$depot20','$le_emp_cat','$le_total_year','$le_total_month','$le_total_day','$le_total_year2','$le_total_month2','$le_total_day2','$le_lap','$le_lhap','$le_advance','$update_by',Now(),'','','','','','','','','')";
		   
		   $result=mysql_query($sql);
		   
		   if($result)
		   {
			  echo "<script>window.location='sr_entry.php';alert('Last Entry Added Successfully');</script>";   
		   }
		   else
		   {
			   echo "<script>window.location='sr_entry.php';alert('Not Added');</script>";
		...
		case 'update_appointment':
			...
			if($sql_fetch==0){
				...
				// $result2=mysql_query($appoint_track_sql);
				 ...
				 // echo "UPDATE `appointment_temp` SET `app_type`='$initial_type',`app_department`='$app_dept',`app_designation`='$app_desig',`other_designation`='$app_other_desig',`app_date`='$app_date',`app_regul_date`='$app_datereg',`app_payscale`='$ps_type_1',`app_scale`='$app_scale',`app_level`='$app_level',`app_group`='$app_group',`app_station`='$station_id3',`other_station`='$app_otherstation',`app_rop`='$app_rop',`app_depot`='$app_depot',`app_refno`='$app_letterno',`app_letter_date`='$app_letterdate',`app_remark`='$app_remark',`date_time`=Now(),`updated_by`='$updated_by' WHERE `app_pf_number`=$app_pf_no ".mysql_error();
				
				// echo "INSERT INTO `appointment_track`( `zone`, `division`, `temp_transaction_id`, `final_transaction_id`, `app_pf_number`, `app_department`, `app_designation`, `app_other_desig`, `app_date`, `app_regul_date`, `app_payscale`, `app_scale`, `app_level`, `app_group`, `app_station`, `other_station`, `app_billunit`, `app_rop`, `app_depot`, `app_refno`, `app_letter_date`, `app_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_datetime`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`) VALUES ('SUR', '06', '".$temp_transaction_id."', '".$final_transaction_id."', '".$app_pf_no."', '".$app_dept."', '".$app_desig."', '".$app_other_desig."', '".$app_date."', '".$app_datereg."', '".$ps_type_1."', '".$app_scale."', '".$app_level."', '".$app_group."', '".$station_id3."', '".$app_otherstation."', '".$app_bill_unit."', '".$app_rop."', '".$app_depot."', '".$app_letterno."', '".$app_letterdate."', '".$app_remark."', Now(), '".$updated_by."', '', '', '', '', '', '', '', '', '')".mysql_error();
		...
		break;
	 	...
	 	case 'add_prtf_promotion':
				...
				// Promotion To
				$pm_to_dept=$_POST['pm_frm_dept'];
				$pm_to_desig=$_POST['pm_frm_desig'];
				$pm_to_otherdesig=$_POST['pm_frm_otherdesig'];
				...
				$re_to_othr_desig=$_POST['re_to_otr_desig'];
				$re_to_pay_scale_level=$_POST['re_to_pay_scale'];
				$re_to_group=$_POST['re_to_group'];
				echo $to_otr_desig."_".$re_to_pay_scale."_".$re_to_group;
				$re_to_place=$_POST['re_to_place'];
				$re_to_rop=$_POST['re_to_rop'];
				$depot_bill_unit8=$_POST['billunit8'];
				$depot8=$_POST['depot8'];
				...
				$s=mysql_query("select * from prft_promotion_temp where pro_pf_no='$pm_pf'");
				$re=mysql_num_rows($s);
				if($re>0){
					$reu=mysql_fetch_array($s);
					$count=$reu['count']+1;
				}
				
				
				if($pm_ordertype=='LDCE' || $pm_ordertype=='GDCE' || $pm_ordertype=='LGS' || $pm_ordertype=='Departmental' || $pm_ordertype=='Re-structuring')
				{
				$sql=mysql_query("INSERT INTO `prft_promotion_temp`(`temp_transaction_id`, `zone`, `division`, `pro_pf_no`, `pro_order_type`, `pro_letter_no`, `pro_letter_date`, `pro_wef`, `pro_frm_dept`, `pro_frm_desig`, `pro_frm_othr_desig`, `pro_frm_pay_scale_type`, `pro_frm_scale`, `pro_frm_level`, `pro_frm_group`, `pro_frm_station`, `pro_frm_othr_station`, `pro_frm_rop`,`rop_to`,`pro_frm_billunit`, `pro_frm_depot`, `pro_to_dept`, `pro_to_desig`, `pro_to_othr_desig`, `pro_to_pay_scale_type`, `pro_to_scale`, `pro_to_level`, `pro_to_group`, `pro_to_station`, `pro_to_othr_station`, `pro_to_billunit`, `pro_to_depot`, `pro_carried_out_type`, `pro_carri_wef`, `pro_carri_date_of_incr`, `pro_car_re_accept_ltr_no`, `pro_car_re_accept_ltr_date`, `pro_car_re_wef_date`, `pro_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`,`count`) VALUES ('$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$pm_frm_dept','$pm_frm_desig','$pm_frm_otherdesig','$pm_frm_ps_type_3','$pm_frm_scale','$pm_frm_level','$pm_frm_group','$station_id1','$pm_frm_otherstation','$pm_frm_rop','$pm_to_rop','$depot_bill_unit2','$depot2','$pm_to_dept','$pm_to_desig','$pm_to_otherdesig','$pm_to_ps_type_3','$pro_to_scale','$pm_to_level','$pm_to_group','$station_id7','$pm_to_otherstation','$depot_bill_unit5','$depot5','$prtf_carried','$prtf_incrdate','$prtf_wefdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','','$count')");
				
				$sql1=mysql_query("INSERT INTO `prft_promotion_track`(`temp_transaction_id`,`final_transaction_id`, `zone`, `division`, `pro_pf_no`, `pro_order_type`, `pro_letter_no`, `pro_letter_date`, `pro_wef`, `pro_frm_dept`, `pro_frm_desig`, `pro_frm_othr_desig`, `pro_frm_pay_scale_type`, `pro_frm_scale`, `pro_frm_level`, `pro_frm_group`, `pro_frm_station`, `pro_frm_othr_station`, `pro_frm_rop`,`rop_to`,`pro_frm_billunit`, `pro_frm_depot`, `pro_to_dept`, `pro_to_desig`, `pro_to_othr_desig`, `pro_to_pay_scale_type`, `pro_to_scale`, `pro_to_level`, `pro_to_group`, `pro_to_station`, `pro_to_othr_station`, `pro_to_billunit`, `pro_to_depot`, `pro_carried_out_type`, `pro_carri_wef`, `pro_carri_date_of_incr`, `pro_car_re_accept_ltr_no`, `pro_car_re_accept_ltr_date`, `pro_car_re_wef_date`, `pro_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`,`count`) VALUES ('$transaction_id','$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$pm_frm_dept','$pm_frm_desig','$pm_frm_otherdesig','$pm_frm_ps_type_3','$pm_frm_scale','$pm_frm_level','$pm_frm_group','$station_id1','$pm_frm_otherstation','$pm_frm_rop','$pm_to_rop','$depot_bill_unit2','$depot2','$pm_to_dept','$pm_to_desig','$pm_to_otherdesig','$pm_to_ps_type_3','$pro_to_scale','$pm_to_level','$pm_to_group','$station_id7','$pm_to_otherstation','$depot_bill_unit5','$depot5','$prtf_carried','$prtf_incrdate','$prtf_wefdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','','$count')");
				 
				}
				else if($pm_ordertype=='Deputation'){
					
					$sql=mysql_query("INSERT INTO `prft_promotion_temp`(`temp_transaction_id`, `zone`, `division`, `pro_pf_no`, `pro_order_type`, `pro_letter_no`, `pro_letter_date`, `pro_wef`, `pro_frm_dept`, `pro_frm_desig`, `pro_frm_othr_desig`, `pro_frm_pay_scale_type`, `pro_frm_scale`, `pro_frm_level`, `pro_frm_group`, `pro_frm_station`, `pro_frm_othr_station`, `pro_frm_rop`,`pro_frm_billunit`, `pro_frm_depot`, `pro_to_dept`, `pro_to_desig`, `pro_to_othr_desig`, `pro_to_scale`, `pro_to_group`, `pro_to_station`,`rop_to`,`pro_to_billunit`, `pro_to_depot`, `pro_carried_out_type`, `pro_carri_wef`, `pro_carri_date_of_incr`, `pro_car_re_accept_ltr_no`, `pro_car_re_accept_ltr_date`, `pro_car_re_wef_date`, `pro_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`) VALUES ('$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$dp_frm_dept','$dp_frm_desig','$dp_frm_otherdesig','$dp_frm_ps_type_3','$dp_frm_scale','$dp_frm_level','$dp_frm_group','$station_id8','$dp_frm_otherstation','$dp_frm_rop','$depot_bill_unit6','$depot6','$dp_to_dept','$dp_to_desig','$dp_to_othr_desig','$dp_to_pay_scale_level','$dp_to_grp','$dp_to_place','$dp_to_rop','$depot_bill_unit7','$depot7','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','')");
					
					$sql1=mysql_query("INSERT INTO `prft_promotion_track`(`temp_transaction_id`, `final_transaction_id`, `zone`, `division`, `pro_pf_no`, `pro_order_type`, `pro_letter_no`, `pro_letter_date`, `pro_wef`, `pro_frm_dept`, `pro_frm_desig`, `pro_frm_othr_desig`, `pro_frm_pay_scale_type`, `pro_frm_scale`, `pro_frm_level`, `pro_frm_group`, `pro_frm_station`, `pro_frm_othr_station`, `pro_frm_rop`,`pro_frm_billunit`, `pro_frm_depot`, `pro_to_dept`, `pro_to_desig`, `pro_to_othr_desig`, `pro_to_scale`, `pro_to_group`, `pro_to_station`,`rop_to`,`pro_to_billunit`, `pro_to_depot`, `pro_carried_out_type`, `pro_carri_wef`, `pro_carri_date_of_incr`, `pro_car_re_accept_ltr_no`, `pro_car_re_accept_ltr_date`, `pro_car_re_wef_date`, `pro_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`) VALUES ('$transaction_id','$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$dp_frm_dept','$dp_frm_desig','$dp_frm_otherdesig','$dp_frm_ps_type_3','$dp_frm_scale','$dp_frm_level','$dp_frm_group','$station_id8','$dp_frm_otherstation','$dp_frm_rop','$depot_bill_unit6','$depot6','$dp_to_dept','$dp_to_desig','$dp_to_othr_desig','$dp_to_pay_scale_level','$dp_to_grp','$dp_to_place','$dp_to_rop','$depot_bill_unit7','$depot7','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','')");
					
					
				}
				else{
				
					$sql=mysql_query("INSERT INTO `prft_promotion_temp`(`temp_transaction_id`, `zone`, `division`, `pro_pf_no`, `pro_order_type`, `pro_letter_no`, `pro_letter_date`, `pro_wef`, `pro_frm_dept`, `pro_frm_desig`, `pro_frm_othr_desig`, `pro_frm_pay_scale_type`, `pro_frm_scale`, `pro_frm_level`, `pro_frm_group`, `pro_frm_station`, `pro_frm_othr_station`, `pro_frm_rop`,`pro_frm_billunit`, `pro_frm_depot`, `pro_to_dept`, `pro_to_desig`, `pro_to_othr_desig`, `pro_to_scale`, `pro_to_group`, `pro_to_station`,`rop_to`,`pro_to_billunit`, `pro_to_depot`, `pro_carried_out_type`, `pro_carri_wef`, `pro_carri_date_of_incr`, `pro_car_re_accept_ltr_no`, `pro_car_re_accept_ltr_date`, `pro_car_re_wef_date`, `pro_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`) VALUES ('$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$re_frm_dept','$re_frm_desig','$$re_to_otr_desig','$re_frm_ps_type_3','$re_frm_scale','$re_frm_level','$re_frm_group','$station_id9','$re_frm_otherstation','$re_frm_rop','$depot_bill_unit9','$depot9','$re_to_dept','$re_to_desig','$re_to_othr_desig','$re_to_pay_scale_level','$re_to_group','$re_to_place','$re_to_rop','$depot_bill_unit8','$depot8','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','')");
					
					$sql1=mysql_query("INSERT INTO `prft_promotion_track`(`temp_transaction_id`, `final_transaction_id`,`zone`, `division`, `pro_pf_no`, `pro_order_type`, `pro_letter_no`, `pro_letter_date`, `pro_wef`, `pro_frm_dept`, `pro_frm_desig`, `pro_frm_othr_desig`, `pro_frm_pay_scale_type`, `pro_frm_scale`, `pro_frm_level`, `pro_frm_group`, `pro_frm_station`, `pro_frm_othr_station`, `pro_frm_rop`,`pro_frm_billunit`, `pro_frm_depot`, `pro_to_dept`, `pro_to_desig`, `pro_to_othr_desig`, `pro_to_scale`, `pro_to_group`, `pro_to_station`,`rop_to`,`pro_to_billunit`, `pro_to_depot`, `pro_carried_out_type`, `pro_carri_wef`, `pro_carri_date_of_incr`, `pro_car_re_accept_ltr_no`, `pro_car_re_accept_ltr_date`, `pro_car_re_wef_date`, `pro_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`) VALUES ('$transaction_id','$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$re_frm_dept','$re_frm_desig','$$re_to_otr_desig','$re_frm_ps_type_3','$re_frm_scale','$re_frm_level','$re_frm_group','$station_id9','$re_frm_otherstation','$re_frm_rop','$depot_bill_unit9','$depot9','$re_to_dept','$re_to_desig','$re_to_othr_desig','$re_to_pay_scale_level','$re_to_group','$re_to_place','$re_to_rop','$depot_bill_unit8','$depot8','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','')");
					
					//echo "INSERT INTO `prft_promotion_temp`(`temp_transaction_id`, `zone`, `division`, `pro_pf_no`, `pro_order_type`, `pro_letter_no`, `pro_letter_date`, `pro_wef`, `pro_frm_dept`, `pro_frm_desig`, `pro_frm_othr_desig`, `pro_frm_pay_scale_type`, `pro_frm_scale`, `pro_frm_level`, `pro_frm_group`, `pro_frm_station`, `pro_frm_othr_station`, `pro_frm_rop`,`pro_frm_billunit`, `pro_frm_depot`, `pro_to_dept`, `pro_to_desig`, `pro_to_othr_desig`, `pro_to_scale`, `pro_to_group`, `pro_to_station`,`rop_to`,`pro_to_billunit`, `pro_to_depot`, `pro_carried_out_type`, `pro_carri_wef`, `pro_carri_date_of_incr`, `pro_car_re_accept_ltr_no`, `pro_car_re_accept_ltr_date`, `pro_car_re_wef_date`, `pro_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`) VALUES ('$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$re_frm_dept','$re_frm_desig','$re_frm_otherdesig','$re_frm_ps_type_3','$re_frm_scale','$re_frm_level','$re_frm_group','$station_id9','$re_frm_otherstation','$re_frm_rop','$depot_bill_unit9','$depot9','$re_to_dept','$re_to_desig','$re_to_othr_desig','$re_to_pay_scale_level','$re_to_grp','$re_to_place','$re_to_rop','$depot_bill_unit7','$depot7','$prtf_carried','$prtf_carr_wef_date','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_wefdate','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','')".mysql_error();
				}
				
				
				if($sql && $sql1)
				{
					  echo "<script>alert('PRTF Added Successfully');window.location='sr_entry.php';</script>";
				}else{
					   echo "<script>alert('PRTF not Added');window.location='sr_entry.php';</script>";
				}
		break;
		
		//  Reversion
		
		case 'add_prtf_reversion':
			 	$zone='01';
				$division='SUR';
				...
				$pm_frm_otherstation=$_POST['rev_to_otherstation'];
				...
				$s=mysql_query("select * from prft_reversion_temp where rev_pf_no='$pm_pf'");
				$re=mysql_num_rows($s);
				if($re>0){
					$reu=mysql_fetch_array($s);
					$count=$reu['count']+1;
				...
				$sql=mysql_query("INSERT INTO `prft_reversion_temp`(`temp_transaction_id`, `zone`, `division`, `rev_pf_no`, `rev_order_type`, `rev_letter_no`, `rev_letter_date`, `rev_wef`, `rev_frm_dept`, `rev_frm_desig`, `rev_frm_othr_desig`, `rev_frm_pay_scale_type`, `rev_frm_scale`, `rev_frm_level`, `rev_frm_group`, `rev_frm_station`, `rev_frm_othr_station`, `rev_frm_billunit`, `rev_frm_depot`, `rev_to_dept`, `rev_to_desig`, `rev_to_othr_desig`, `rev_to_pay_scale_type`, `rev_to_scale`, `rev_to_level`, `rev_to_group`, `rev_to_station`, `rev_to_othr_station`, `rev_to_billunit`, `rev_to_depot`, `rev_carried_out_type`, `rev_carri_wef`, `rev_carri_date_of_incr`, `rev_car_re_accept_ltr_no`, `rev_car_re_accept_ltr_date`, `rev_car_re_wef_date`, `rev_car_re_remark`, `rev_frm_rop`, `rev_to_rop`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`,`count`) VALUES ('$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$pm_frm_dept','$pm_frm_desig','$pm_frm_otherdesig','$pm_frm_ps_type_3','$pm_frm_scale','$pm_frm_level','$pm_frm_group','$station_id1','$pm_frm_otherstation','$depot_bill_unit2','$depot2','$pm_to_dept','$pm_to_desig','$pm_to_otherdesig','$pm_to_ps_type_3','$pro_to_scale','$pm_to_level','$pm_to_group','$station_id7','$pm_to_otherstation','$depot_bill_unit5','$depot5','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark','$pm_frm_rop','$pm_to_rop',Now(),'$updated_by','','','','','','','','','','$count')");
				
				$sql1=mysql_query("INSERT INTO `prft_reversion_track`(`temp_transaction_id`,`final_transaction_id`, `zone`, `division`, `rev_pf_no`, `rev_order_type`, `rev_letter_no`, `rev_letter_date`, `rev_wef`, `rev_frm_dept`, `rev_frm_desig`, `rev_frm_othr_desig`, `rev_frm_pay_scale_type`, `rev_frm_scale`, `rev_frm_level`, `rev_frm_group`, `rev_frm_station`, `rev_frm_othr_station`, `rev_frm_billunit`, `rev_frm_depot`, `rev_to_dept`, `rev_to_desig`, `rev_to_othr_desig`, `rev_to_pay_scale_type`, `rev_to_scale`, `rev_to_level`, `rev_to_group`, `rev_to_station`, `rev_to_othr_station`, `rev_to_billunit`, `rev_to_depot`, `rev_carried_out_type`, `rev_carri_wef`, `rev_carri_date_of_incr`, `rev_car_re_accept_ltr_no`, `rev_car_re_accept_ltr_date`, `rev_car_re_wef_date`, `rev_car_re_remark`, `rev_frm_rop`, `rev_to_rop`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`,`count`) VALUES ('$transaction_id','$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$pm_frm_dept','$pm_frm_desig','$pm_frm_otherdesig','$pm_frm_ps_type_3','$pm_frm_scale','$pm_frm_level','$pm_frm_group','$station_id1','$pm_frm_otherstation','$depot_bill_unit2','$depot2','$pm_to_dept','$pm_to_desig','$pm_to_otherdesig','$pm_to_ps_type_3','$pro_to_scale','$pm_to_level','$pm_to_group','$station_id7','$pm_to_otherstation','$depot_bill_unit5','$depot5','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark','$pm_frm_rop','$pm_to_rop',Now(),'$updated_by','','','','','','','','','','$count')");
		...
		case 'add_prtf_transfer':
				...
				$pm_frm_station=$_POST['statione'];
				$pm_frm_otherstation=$_POST['tran_frm_otherstation'];
				$pm_frm_rop=$_POST['tran_frm_rop'];
				$pm_frm_bill_unith=$_POST['billunith'];
				$pm_frm_depoth=$_POST['depoth'];
				...
				$pm_to_station=$_POST['stationg'];
				$pm_to_otherstation=$_POST['tran_to_otherstation'];
				$pm_to_rop=$_POST['tran_to_rop'];
				$pm_to_bill_unitj=$_POST['billunitj'];
				$pm_to_depotj=$_POST['depotj'];
				...
				$s=mysql_query("select * from prft_transfer_temp where trans_pf_no='$pm_pf'");
				$re=mysql_num_rows($s);
				if($re>0){
					$reu=mysql_fetch_array($s);
					$count=$reu['count']+1;
				...
				$sql=mysql_query("INSERT INTO `prft_transfer_temp`(`temp_transaction_id`, `zone`, `division`, `trans_pf_no`, `trans_order_type`, `trans_letter_no`, `trans_letter_date`, `trans_wef`, `trans_frm_dept`, `trans_frm_desig`, `trans_frm_othr_desig`, `trans_frm_pay_scale_type`, `trans_frm_scale`, `trans_frm_level`, `trans_frm_group`, `trans_frm_station`, `trans_frm_othr_station`, `trans_frm_rop`, `trans_frm_billunit`, `trans_frm_depot`, `trans_to_dept`, `trans_to_desig`, `trans_to_othr_desig`, `trans_to_pay_scale_type`, `trans_to_scale`, `trans_to_level`, `trans_to_group`, `trans_to_station`, `trans_to_othr_station`, `trans_to_rop`, `trans_to_billunit`, `trans_to_depot`, `trans_carried_out_type`, `trans_carri_wef`, `trans_carri_date_of_incr`, `trans_car_re_accept_ltr_no`, `trans_car_re_accept_ltr_date`, `trans_car_re_wef_date`, `trans_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`,`count`) VALUES ('$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$pm_frm_dept','$pm_frm_desig','$pm_frm_otherdesig','$pm_frm_ps_type_3','$pm_frm_scale','$pm_frm_level','$pm_frm_group','$pm_frm_station','$pm_frm_otherstation','$pm_frm_rop','$pm_frm_bill_unith','$pm_frm_depoth','$pm_to_dept','$pm_to_desig','$pm_to_otherdesig','$pm_to_ps_type_3','$pm_to_scale','$pm_to_level','$pm_to_group','$pm_to_station','$pm_to_otherstation','$pm_to_rop','$pm_to_bill_unitj','$pm_to_depotj','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','','$count')");
				
				$sql1=mysql_query("INSERT INTO `prft_transfer_track`(`temp_transaction_id`,`final_transaction_id`, `zone`, `division`, `trans_pf_no`, `tran_order_type`, `tran_letter_no`, `tran_letter_date`, `tran_wef`, `tran_frm_dept`, `tran_frm_desig`, `tran_frm_othr_desig`, `tran_frm_pay_scale_type`, `tran_frm_scale`, `tran_frm_level`, `tran_frm_group`, `tran_frm_station`, `tran_frm_othr_station`, `tran_frm_rop`, `tran_frm_billunit`, `tran_frm_depot`, `tran_to_dept`, `tran_to_desig`, `tran_to_othr_desig`, `tran_to_pay_scale_type`, `tran_to_scale`, `tran_to_level`, `tran_to_group`, `tran_to_station`, `tran_to_othr_station`, `tran_to_rop`, `tran_to_billunit`, `tran_to_depot`, `tran_carried_out_type`, `tran_carri_wef`, `tran_carri_date_of_incr`, `tran_car_re_accept_ltr_no`, `tran_car_re_accept_ltr_date`, `tran_car_re_wef_date`, `tran_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`,`count`) VALUES ('$transaction_id','$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$pm_frm_dept','$pm_frm_desig','$pm_frm_otherdesig','$pm_frm_ps_type_3','$pm_frm_scale','$pm_frm_level','$pm_frm_group','$pm_frm_station','$pm_frm_otherstation','$pm_frm_rop','$pm_frm_bill_unith','$pm_frm_depoth','$pm_to_dept','$pm_to_desig','$pm_to_otherdesig','$pm_to_ps_type_3','pm_to_scale','pm_to_level','pm_to_group','pm_to_station','pm_to_otherstation','pm_to_rop','pm_to_bill_unitj','pm_to_depotj','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','','$count')");
				

					


				//echo "INSERT INTO `prft_transfer_temp`(`temp_transaction_id`, `zone`, `division`, `trans_pf_no`, `trans_order_type`, `trans_letter_no`, `trans_letter_date`, `trans_wef`, `trans_dept`, `trans_desig`, `trans_othr_desig`, `trans_pay_scale_type`, `trans_scale`, `trans_level`, `trans_group`, `trans_station`, `trans_othr_station`, `trans_rop`, `trans_billunit`, `trans_depot`, `trans_carried_out_type`, `trans_carri_wef`, `trans_carri_date_of_incr`, `trans_car_re_accept_ltr_no`, `trans_car_re_accept_ltr_date`, `trans_car_re_wef_date`, `trans_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`) VALUES ('$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$pm_frm_dept','$pm_frm_desig','$pm_frm_otherdesig','$pm_frm_ps_type_3','$pm_frm_scale','$pm_frm_level','$pm_frm_group','$station_id1','$pm_frm_otherstation','$pm_frm_rop','$depot_bill_unit2','$depot2','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','')".mysql_error();
				
				//echo "INSERT INTO `prft_transfer_track`(`temp_transaction_id`, `final_transaction_id`, `zone`, `division`, `trans_pf_no`, `trans_order_type`, `tran_letter_no`, `tran_letter_date`, `tran_wef`, `tran_dept`, `tran_desig`, `tran_othr_desig`, `tran_pay_scale_type`, `tran_scale`, `tran_level`, `tran_group`, `tran_station`, `tran_othr_station`, `tran_rop`, `tran_billunit`, `tran_depot`, `tran_carried_out_type`, `tran_carri_wef`, `tran_carri_date_of_incr`, `tran_car_re_accept_ltr_no`, `tran_car_re_accept_ltr_date`, `tran_car_re_wef_date`, `tran_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`) VALUES ('$transaction_id','$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$pm_frm_dept','$pm_frm_desig','$pm_frm_otherdesig','$pm_frm_ps_type_3','$pm_frm_scale','$pm_frm_level','$pm_frm_group','$station_id1','$pm_frm_otherstation','$pm_frm_rop','$depot_bill_unit2','$depot2','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','')".mysql_error();
...
case 'add_prtf_fixation':
				...
				$s=mysql_query("select * from prft_fixation_temp where fix_pf_no='$pm_pf'");
				$re=mysql_num_rows($s);
				if($re>0){
					$reu=mysql_fetch_array($s);
					$count=$reu['count']+1;
...
/* 								echo $re['pro_order_type']."=".$pm_ordertype."<br>";
						...
						if($fetch)
						{
							...
							if($re['pro_order_type']==$pm_ordertype && $re['pro_letter_no']==$pm_letterno && $re['pro_letter_date']==$pm_letterdate && $re['pro_wef']==$pm_wef && $re['pro_frm_dept']==$dp_frm_dept && $re['pro_frm_desig']==$dp_frm_desig && $re['pro_frm_othr_desig']==$dp_frm_otherdesig && $re['pro_frm_pay_scale_type']==$dp_frm_ps_type_3 && $re['pro_frm_scale']==$dp_frm_scale && $re['pro_frm_level']==$dp_frm_level && $re['pro_frm_group']==$dp_frm_group && $re['pro_frm_station']==$station_id8 && $re['pro_frm_othr_station']==$dp_frm_otherstation && $re['pro_frm_rop']==$dp_frm_rop && $re['pro_frm_billunit']==$depot_bill_unit6 && $re['pro_frm_depot']==$depot6 && $re['pro_to_dept']==$dp_to_dept && $re['pro_to_desig']==$dp_to_desig && $re['pro_to_othr_desig']==$dp_to_othr_desig && $re['pro_to_scale']==$dp_to_pay_scale_level && $re['pro_to_group']==$dp_to_grp && $re['pro_to_station']==$dp_to_place && $re['rop_to']==$dp_to_rop && $re['pro_to_billunit']==$depot_bill_unit7 && $re['pro_to_depot']==$depot7 && $re['pro_carried_out_type']==$prtf_carried && $re['pro_carri_wef']==$prtf_wefdate && $re['pro_carri_date_of_incr']==$prtf_incrdate && $re['pro_car_re_accept_ltr_no']==$prtf_acc_ltr_no && $re['pro_car_re_accept_ltr_date']==$prtf_acc_ltr_date && $re['pro_car_re_wef_date']==$prtf_carr_wef_date && $re['pro_car_re_remark']==$prtf_carr_remark)
							{
								...
								$sql1=mysql_query("INSERT INTO `prft_promotion_track`(`temp_transaction_id`, `final_transaction_id`, `zone`, `division`, `pro_pf_no`, `pro_order_type`, `pro_letter_no`, `pro_letter_date`, `pro_wef`, `pro_frm_dept`, `pro_frm_desig`, `pro_frm_othr_desig`, `pro_frm_pay_scale_type`, `pro_frm_scale`, `pro_frm_level`, `pro_frm_group`, `pro_frm_station`, `pro_frm_othr_station`, `pro_frm_rop`,`pro_frm_billunit`, `pro_frm_depot`, `pro_to_dept`, `pro_to_desig`, `pro_to_othr_desig`, `pro_to_scale`, `pro_to_group`, `pro_to_station`,`rop_to`,`pro_to_billunit`, `pro_to_depot`, `pro_carried_out_type`, `pro_carri_wef`, `pro_carri_date_of_incr`, `pro_car_re_accept_ltr_no`, `pro_car_re_accept_ltr_date`, `pro_car_re_wef_date`, `pro_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`,`count`) VALUES ('$transaction_id','$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$dp_frm_dept','$dp_frm_desig','$dp_frm_otherdesig','$dp_frm_ps_type_3','$dp_frm_scale','$dp_frm_level','$dp_frm_group','$station_id8','$dp_frm_otherstation','$dp_frm_rop','$depot_bill_unit6','$depot6','$dp_to_dept','$dp_to_desig','$dp_to_othr_desig','$dp_to_pay_scale_level','$dp_to_grp','$dp_to_place','$dp_to_rop','$depot_bill_unit7','$depot7','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','','$count')");
							}
						}
						...
						$sql=mysql_query("UPDATE `prft_promotion_temp` SET `pro_order_type`='$pm_ordertype', `pro_letter_no`='$pm_letterno', `pro_letter_date`='$pm_letterdate', `pro_wef`='$pm_wef', `pro_frm_dept`='$dp_frm_dept', `pro_frm_desig`='$dp_frm_desig', `pro_frm_othr_desig`='$dp_frm_otherdesig', `pro_frm_pay_scale_type`='$dp_frm_ps_type_3', `pro_frm_scale`='$dp_frm_scale', `pro_frm_level`='$dp_frm_level', `pro_frm_group`='$dp_frm_group', `pro_frm_station`='$station_id8', `pro_frm_othr_station`='$dp_frm_otherstation', `pro_frm_rop`='$dp_frm_rop',`pro_frm_billunit`='$depot_bill_unit6', `pro_frm_depot`='$depot6', `pro_to_dept`='$dp_to_dept', `pro_to_desig`='$dp_to_desig', `pro_to_othr_desig`='$dp_to_othr_desig', `pro_to_scale`='$dp_to_pay_scale_level', `pro_to_group`='$dp_to_grp', `pro_to_station`='$dp_to_place',`rop_to`='$dp_to_rop',`pro_to_billunit`='$depot_bill_unit7', `pro_to_depot`='$depot7', `pro_carried_out_type`='$prtf_carried', `pro_carri_wef`='$prtf_wefdate', `pro_carri_date_of_incr`='$prtf_incrdate', `pro_car_re_accept_ltr_no`='$prtf_acc_ltr_no', `pro_car_re_accept_ltr_date`='$prtf_acc_ltr_date', `pro_car_re_wef_date`='$prtf_carr_wef_date', `pro_car_re_remark`='$prtf_carr_remark', `date_time`=Now(), `updated_by`='$updated_by' where `pro_pf_no`='$pm_pf' and id='$pm_id'");
							...
							if($fetch)
							{
								...
								if($re['pro_pf_no']==$pm_pf && $re['pro_order_type']==$pm_ordertype && $re['pro_letter_no']==$pm_letterno && $re['pro_letter_date']==$pm_letterdate && $re['pro_wef']==$pm_wef && $re['pro_frm_dept']==$re_frm_dept && $re['pro_frm_desig']==$re_frm_desig && $re['pro_frm_othr_desig']==$re_frm_otherdesig && $re['pro_frm_pay_scale_type']==$re_frm_ps_type_3 && $re['pro_frm_scale']==$re_frm_scale && $re['pro_frm_level']==$re_frm_level && $re['pro_frm_group']==$re_frm_group && $re['pro_frm_station']==$station_id9 && $re['pro_frm_othr_station']==$re_frm_otherstation && $re['pro_frm_rop']==$re_frm_rop && $re['pro_frm_billunit']==$depot_bill_unit9 && $re['pro_frm_depot']==$depot_bill_unit9 && $re['pro_to_dept']==$re_to_dept && $re['pro_to_desig']==$re_to_desig && $re['pro_to_othr_desig']==$re_to_otr_desig && $re['pro_to_scale']==$re_to_pay_scale && $re['pro_to_group']==$re_to_group && $re['pro_to_station']==$re_to_place && $re['rop_to']==$re_to_rop && $re['pro_to_billunit']==$billunit8 && $re['pro_to_depot']==$depot8 && $re['pro_carried_out_type']==$prtf_carried && $re['pro_carri_wef']==$prtf_wefdate && $re['pro_carri_date_of_incr']==$prtf_incrdate && $re['pro_car_re_accept_ltr_no']==$prtf_acc_ltr_no && $re['pro_car_re_accept_ltr_date']==$prtf_acc_ltr_date && $re['pro_car_re_wef_date']==$prtf_carr_wef_date && $re['pro_car_re_remark']==$prtf_carr_remark  )
								{
									...
									echo $re['pro_frm_billunit']."=".$depot_bill_unit9."<br>"; 
									echo $re['pro_frm_depot']."=".$depot_bill_unit9."<br>"; 
									...
									echo $re['pro_to_billunit']."=".$billunit8."<br>"; 
									echo $re['pro_to_depot']."=".$depot8."<br>"; 
									echo $re['pro_carried_out_type']."=".$prtf_carried."<br>"; 
									echo $re['pro_carri_wef']."=".$prtf_wefdate."<br>"; 
									echo $re['pro_carri_date_of_incr']."=".$prtf_incrdate."<br>"; 
									echo $re['pro_car_re_accept_ltr_no']."=".$prtf_acc_ltr_no."<br>"; 
									...
									$sql1=mysql_query("INSERT INTO `prft_promotion_track`(`temp_transaction_id`, `final_transaction_id`,`zone`, `division`, `pro_pf_no`, `pro_order_type`, `pro_letter_no`, `pro_letter_date`, `pro_wef`, `pro_frm_dept`, `pro_frm_desig`, `pro_frm_othr_desig`, `pro_frm_pay_scale_type`, `pro_frm_scale`, `pro_frm_level`, `pro_frm_group`, `pro_frm_station`, `pro_frm_othr_station`, `pro_frm_rop`,`pro_frm_billunit`, `pro_frm_depot`, `pro_to_dept`, `pro_to_desig`, `pro_to_othr_desig`, `pro_to_scale`, `pro_to_group`, `pro_to_station`,`rop_to`,`pro_to_billunit`, `pro_to_depot`, `pro_carried_out_type`, `pro_carri_wef`, `pro_carri_date_of_incr`, `pro_car_re_accept_ltr_no`, `pro_car_re_accept_ltr_date`, `pro_car_re_wef_date`, `pro_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`,`count`) VALUES ('$transaction_id','$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$re_frm_dept','$re_frm_desig','$re_frm_otherdesig','$re_frm_ps_type_3','$re_frm_scale','$re_frm_level','$re_frm_group','$station_id9','$re_frm_otherstation','$re_frm_rop','$depot_bill_unit9','$depot_bill_unit9','$re_to_dept','$re_to_desig','$re_to_otr_desig','$re_to_pay_scale','$re_to_group','$re_to_place','$re_to_rop','$billunit8','$depot8','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','','$count')");
									
									//echo "INSERT INTO `prft_promotion_track`(`temp_transaction_id`, `final_transaction_id`,`zone`, `division`, `pro_pf_no`, `pro_order_type`, `pro_letter_no`, `pro_letter_date`, `pro_wef`, `pro_frm_dept`, `pro_frm_desig`, `pro_frm_othr_desig`, `pro_frm_pay_scale_type`, `pro_frm_scale`, `pro_frm_level`, `pro_frm_group`, `pro_frm_station`, `pro_frm_othr_station`, `pro_frm_rop`,`pro_frm_billunit`, `pro_frm_depot`, `pro_to_dept`, `pro_to_desig`, `pro_to_othr_desig`, `pro_to_scale`, `pro_to_group`, `pro_to_station`,`rop_to`,`pro_to_billunit`, `pro_to_depot`, `pro_carried_out_type`, `pro_carri_wef`, `pro_carri_date_of_incr`, `pro_car_re_accept_ltr_no`, `pro_car_re_accept_ltr_date`, `pro_car_re_wef_date`, `pro_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`) VALUES ('$transaction_id','$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$re_frm_dept','$re_frm_desig','$re_frm_otherdesig','$re_frm_ps_type_3','$re_frm_scale','$re_frm_level','$re_frm_group','$station_id9','$re_frm_otherstation','$re_frm_rop','$billunit8','$depot8','$re_to_dept','$re_to_desig','$re_to_otr_desig','$re_to_pay_scale','$re_to_group','$re_to_place','$re_to_rop','$depot_bill_unit9','$depot_bill_unit9','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','')".mysql_error()."<br>";
						...
						//echo "UPDATE  `prft_promotion_temp` SET  `pro_order_type`='$pm_ordertype', `pro_letter_no`='$pm_letterno', `pro_letter_date`='$pm_letterdate', `pro_wef`='$pm_wef', `pro_frm_dept`='$re_frm_dept', `pro_frm_desig`='$re_frm_desig', `pro_frm_othr_desig`='$re_frm_otherdesig', `pro_frm_pay_scale_type`='$re_frm_ps_type_3', `pro_frm_scale`='$re_frm_scale', `pro_frm_level`='$re_frm_level', `pro_frm_group`='$re_frm_group', `pro_frm_station`='$station_id9', `pro_frm_othr_station`='$re_frm_otherstation', `pro_frm_rop`='$re_frm_rop',`pro_frm_billunit`='$depot_bill_unit9', `pro_frm_depot`='$depot9', `pro_to_dept`='$re_to_dept', `pro_to_desig`='$re_to_desig', `pro_to_othr_desig`='$re_to_otr_desig', `pro_to_scale`='$re_to_pay_scale', `pro_to_group`='$re_to_group', `pro_to_station`='$re_to_place',`rop_to`='$re_to_rop',`pro_to_billunit`='$billunit8', `pro_to_depot`='$depot8', `pro_carried_out_type`='$prtf_carried', `pro_carri_wef`='$prtf_wefdate', `pro_carri_date_of_incr`='$prtf_incrdate', `pro_car_re_accept_ltr_no`='$prtf_acc_ltr_no', `pro_car_re_accept_ltr_date`='$prtf_acc_ltr_date', `pro_car_re_wef_date`='$prtf_carr_wef_date', `pro_car_re_remark`='$prtf_carr_remark', `date_time`=Now(), `updated_by`='$updated_by' where `pro_pf_no`='$pm_pf' and id='$pm_id'".mysql_error()."<br>";
					}
					...
					if($pm_ordertype=='LDCE' || $pm_ordertype=='GDCE' || $pm_ordertype=='LGS' || $pm_ordertype=='Departmental' || $pm_ordertype=='Re-structuring' || $pm_ordertype=='MACP-1' || $pm_ordertype=='MACP-2' || $pm_ordertype=='MACP-3')
					{
						...
						$sql1=mysql_query("INSERT INTO `prft_promotion_track`(`temp_transaction_id`, `final_transaction_id`,`zone`, `division`, `pro_pf_no`, `pro_order_type`, `pro_letter_no`, `pro_letter_date`, `pro_wef`, `pro_frm_dept`, `pro_frm_desig`, `pro_frm_othr_desig`, `pro_frm_pay_scale_type`, `pro_frm_scale`, `pro_frm_level`, `pro_frm_group`, `pro_frm_station`, `pro_frm_othr_station`, `pro_frm_rop`,`pro_frm_billunit`, `pro_frm_depot`, `pro_to_dept`, `pro_to_desig`, `pro_to_othr_desig`, `pro_to_scale`, `pro_to_group`, `pro_to_station`,`rop_to`,`pro_to_billunit`, `pro_to_depot`, `pro_carried_out_type`, `pro_carri_wef`, `pro_carri_date_of_incr`, `pro_car_re_accept_ltr_no`, `pro_car_re_accept_ltr_date`, `pro_car_re_wef_date`, `pro_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`,`count`) VALUES ('$transaction_id','$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$re_frm_dept','$re_frm_desig','$re_frm_otherdesig','$re_frm_ps_type_3','$re_frm_scale','$re_frm_level','$re_frm_group','$station_id9','$re_frm_otherstation','$re_frm_rop','$depot_bill_unit9','$depot9','$re_to_dept','$re_to_desig','$re_to_otr_desig','$re_to_pay_scale','$re_to_group','$re_to_place','$re_to_rop','$billunit8','$depot8','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','','$count')");
				...
				if($fix_id!=""){
					...
					if($res>0)
					{
						...
						if($re['fix_pf_no']==$pm_pf && $re['fix_order_type']==$pm_ordertype && $re['fix_letter_no']==$pm_letterno && $re['fix_letter_date']==$pm_letterdate && $re['fix_wef']==$pm_wef && $re['fix_remark']==$pm_fx_remark && $re['fix_frm_rop']==$pm_oldrop && $re['fix_frm_ps_type']==$pm_fx_ps_type_e && $re['fix_frm_scale']==$pm_fx_frm_scale && $re['fix_frm_level']==$pm_fx_frm_level && $re['fix_to_rop']==$pm_newrop && $re['fix_to_ps_type']==$pm_fx_to_type_p && $re['fix_to_scale']==$pm_fx_to_scale && $re['fx_to_level']==$pm_fx_to_level && $re['fix_carried_out_type']==$prtf_carried && $fix_carri_wef==$prtf_wefdate && $fix_carri_date_of_incr==$prtf_incrdate && $re['fix_car_re_accept_ltr_no']==$prtf_acc_ltr_no && $fix_car_re_accept_ltr_date==$prtf_acc_ltr_date && $fix_car_re_wef_date==$prtf_carr_wef_date && $re['fix_car_re_remark']==$prtf_carr_remark)
						{
							...
							/* echo $re['zone']."=".$zone."<br>";
							echo $re['division']."=".$division."<br>";
...
/* 						echo "trans_carri_wef".$trans_carri_wef."<br>"; 
						...
						if($re['trans_order_type']==$pm_ordertype && $re['trans_letter_no']==$pm_letterno && $re['trans_letter_date']==$pm_letterdate && $re['trans_wef']==$pm_wef && $re['trans_frm_dept']==$pm_frm_dept && $re['trans_frm_desig']==$pm_frm_desig && $re['trans_frm_othr_desig']==$pm_frm_otherdesig  && $re['trans_frm_pay_scale_type']==$pm_frm_ps_type_3 && $re['trans_frm_scale']==$pm_frm_scale && $re['trans_frm_level']==$pm_frm_level && $re['trans_frm_group']==$pm_frm_group && $re['trans_frm_station']==$pm_frm_station && $re['trans_frm_othr_station']==$pm_frm_otherstation && $re['trans_frm_rop']==$pm_frm_rop && $re['trans_frm_billunit']==$pm_frm_bill_unith && $re['trans_frm_depot']==$pm_frm_depoth && $re['trans_to_dept']==$pm_to_dept && $re['trans_to_desig']==$pm_to_desig && $re['trans_to_othr_desig']==$pm_to_otherdesig && $re['trans_to_pay_scale_type']==$pm_to_ps_type_3 && $re['trans_to_scale']==$pm_to_scale && $re['trans_to_level']==$pm_to_level && $re['trans_to_group']==$pm_to_group && $re['trans_to_station']==$pm_to_station && $re['trans_to_othr_station']==$pm_to_otherstation && $re['trans_to_rop']==$pm_to_rop && $re['trans_to_billunit']==$pm_to_bill_unitj && $re['trans_to_depot']==$pm_to_depotj && $re['trans_carried_out_type']==$prtf_carried && $trans_carri_wef==$prtf_wefdate && $trans_carri_date_of_incr==$prtf_incrdate && $re['trans_car_re_accept_ltr_no']==$prtf_acc_ltr_no && $trans_car_re_accept_ltr_date==$prtf_acc_ltr_date && $trans_car_re_wef_date==$prtf_carr_wef_date && $re['trans_car_re_remark']==$prtf_carr_remark)
						{
							...
							//echo "INSERT INTO `prft_transfer_track`(`temp_transaction_id`,`final_transaction_id`, `zone`, `division`, `trans_pf_no`, `tran_order_type`, `tran_letter_no`, `tran_letter_date`, `tran_wef`, `tran_frm_dept`, `tran_frm_desig`, `tran_frm_othr_desig`, `tran_frm_pay_scale_type`, `tran_frm_scale`, `tran_frm_level`, `tran_frm_group`, `tran_frm_station`, `tran_frm_othr_station`, `tran_frm_rop`, `tran_frm_billunit`, `tran_frm_depot`, `tran_to_dept`, `tran_to_desig`, `tran_to_othr_desig`, `tran_to_pay_scale_type`, `tran_to_scale`, `tran_to_level`, `tran_to_group`, `tran_to_station`, `tran_to_othr_station`, `tran_to_rop`, `tran_to_billunit`, `tran_to_depot`, `tran_carried_out_type`, `tran_carri_wef`, `tran_carri_date_of_incr`, `tran_car_re_accept_ltr_no`, `tran_car_re_accept_ltr_date`, `tran_car_re_wef_date`, `tran_car_re_remark`, `date_time`, `updated_by`, `updated_fields`, `updated_reason`, `updated_date_time`, `letter_no`, `letter_datetime`, `uploaded_letter`, `approved_status`, `approved_by`, `approved_datetime`,`count`) VALUES ('$transaction_id','$transaction_id','$zone','$division','$pm_pf','$pm_ordertype','$pm_letterno','$pm_letterdate','$pm_wef','$pm_frm_dept','$pm_frm_desig','$pm_frm_otherdesig','$pm_frm_ps_type_3','$pm_frm_scale','$pm_frm_level','$pm_frm_group','$pm_frm_station','$pm_frm_otherstation','$pm_frm_rop','$pm_frm_bill_unith','$pm_frm_depoth','$pm_to_dept','$pm_to_desig','$pm_to_otherdesig','$pm_to_ps_type_3','$pm_to_scale','$pm_to_level','$pm_to_group','$pm_to_station','$pm_to_otherstation','$pm_to_rop','$pm_to_bill_unitj','$pm_to_depotj','$prtf_carried','$prtf_wefdate','$prtf_incrdate','$prtf_acc_ltr_no','$prtf_acc_ltr_date','$prtf_carr_wef_date','$prtf_carr_remark',Now(),'$updated_by','','','','','','','','','','$count')".mysql_error();
						}
					}											
					
					$sql1=mysql_query("UPDATE `prft_transfer_temp` SET `temp_transaction_id`='$transaction_id',`zone`='$zone',`division`='$division',`trans_pf_no`='$pm_pf',`trans_order_type`='$pm_ordertype',`trans_letter_no`='$pm_letterno',`trans_letter_date`='$pm_letterdate',`trans_wef`='$pm_wef',`trans_frm_dept`='$pm_frm_dept',`trans_frm_desig`='$pm_frm_desig',`trans_frm_othr_desig`='$pm_frm_otherdesig',`trans_frm_pay_scale_type`='$pm_frm_ps_type_3',`trans_frm_scale`='$pm_frm_scale',`trans_frm_level`='$pm_frm_level',`trans_frm_group`='$pm_frm_group',`trans_frm_station`='$pm_frm_station',`trans_frm_othr_station`='$pm_frm_otherstation',`trans_frm_rop`='$pm_frm_rop',`trans_frm_billunit`='$pm_frm_bill_unith',`trans_frm_depot`='$pm_frm_depoth',`trans_to_dept`='$pm_to_dept',`trans_to_desig`='$pm_to_desig',`trans_to_othr_desig`='$pm_to_otherdesig',`trans_to_pay_scale_type`='$pm_to_ps_type_3',`trans_to_scale`='$pm_to_scale',`trans_to_level`='$pm_to_level',`trans_to_group`='$pm_to_group',`trans_to_station`='$pm_to_station',`trans_to_othr_station`='$pm_to_otherstation',`trans_to_rop`='$pm_to_rop',`trans_to_billunit`='$pm_to_bill_unitj',`trans_to_depot`='$pm_to_depotj',`trans_carried_out_type`='$prtf_carried',`trans_carri_wef`='$prtf_wefdate',`trans_carri_date_of_incr`='$prtf_incrdate',`trans_car_re_accept_ltr_no`='$prtf_acc_ltr_no',`trans_car_re_accept_ltr_date`='$prtf_acc_ltr_date',`trans_car_re_wef_date`='$prtf_carr_wef_date',`trans_car_re_remark`='$prtf_carr_remark',`date_time`=Now(),`updated_by`=$updated_by WHERE id='$tran_id'");
					
					//echo "UPDATE `prft_transfer_temp` SET `temp_transaction_id`='$transaction_id',`zone`='$zone',`division`='$division',`trans_pf_no`='$pm_pf',`trans_order_type`='$pm_ordertype',`trans_letter_no`='$pm_letterno',`trans_letter_date`='$pm_letterdate',`trans_wef`='$pm_wef',`trans_frm_dept`='$pm_frm_dept',`trans_frm_desig`='$pm_frm_desig',`trans_frm_othr_desig`='$pm_frm_otherdesig',`trans_frm_pay_scale_type`='$pm_frm_ps_type_3',`trans_frm_scale`='$pm_frm_scale',`trans_frm_level`='$pm_frm_level',`trans_frm_group`='$pm_to_group',`trans_frm_station`='$pm_frm_station',`trans_frm_othr_station`='$pm_frm_otherstation',`trans_frm_rop`='$pm_frm_rop',`trans_frm_billunit`='$pm_frm_bill_unith',`trans_frm_depot`='$pm_frm_depoth',`trans_to_dept`='$pm_to_dept',`trans_to_desig`='$pm_to_desig',`trans_to_othr_desig`='$pm_to_otherdesig',`trans_to_pay_scale_type`='$pm_to_ps_type_3',`trans_to_scale`='$pm_to_scale',`trans_to_level`='$pm_to_level',`trans_to_group`='$pm_to_group',`trans_to_station`='$pm_to_station',`trans_to_othr_station`='$pm_to_otherstation',`trans_to_rop`='$pm_to_rop',`trans_to_billunit`='$pm_to_bill_unitj',`trans_to_depot`='$pm_to_depotj',`trans_carried_out_type`='$prtf_carried',`trans_carri_wef`='$prtf_wefdate',`trans_carri_date_of_incr`='$prtf_incrdate',`trans_car_re_accept_ltr_no`='$prtf_acc_ltr_no',`trans_car_re_accept_ltr_date`='$prtf_acc_ltr_date',`trans_car_re_wef_date`='$prtf_carr_wef_date',`trans_car_re_remark`='$prtf_carr_remark',`date_time`=Now(),`updated_by`=$updated_by WHERE id='$tran_id'".mysql_error();
		  ...
		  break;
		  //Online Office Order Code Start----------------------------------------------------------------------
		 ...
		 break;
//Online Office Order Code Start End -----------------------------------------------------------------
