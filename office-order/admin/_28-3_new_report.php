...
<div style="padding:10px;margin:5px;padding-top:20px;margin-top:15px;">
	...
	<div class="tab-content">
		<div class="tab-pane active in" id="prft">
			...
			<div id='loadingmessage' style='display: none; margin-left: 30%; margin-top: -265px;'>
		  <img src='bgLoad.gif'/>
		</div>
						...
						if(isset($_POST['btn_view']))
						{
							...
							while($result=mysql_fetch_array($sql))
							{
								...
								while($fetch=mysql_fetch_array($sql1)){
									...
									$row_cnt=0;
									$inc_row_count=0;
									$fetch_record=mysql_query("select pf_number from biodata_temp where pf_number='$emp_pf'");
									...
									$fetch_record=mysql_query("select medi_pf_number from medical_temp where medi_pf_number='$emp_pf'");
									$r=mysql_num_rows($fetch_record);
									...
									$fetch_record=mysql_query("select app_pf_number from appointment_temp where app_pf_number='$emp_pf'");
									$r=mysql_num_rows($fetch_record);
									...
									$fetch_record=mysql_query("select preapp_pf_number from present_work_temp where preapp_pf_number='$emp_pf'");
									$r=mysql_num_rows($fetch_record);
								...
								echo "<td><a href='#' data-toggle='modal' data-target='#myModal' bill_value='$bill_id' class='complete_pf' title='hey'>$total_count</td>";
...
$(document).on("click",".complete_pf",function(){
	...
	$.ajax({
		type:"post",
		url:"process.php",
		data:"action=get_complete_pf_report&b_v="+b_v,
		success:function(data){
		debugger;
		//alert(data);
			debugger;
