							...
							<div class="row">
								...
								<div class="col-md-4">
									<div class="form-group">
										...
										<div class="input-group">
										<textarea rows="5" cols="50" class='form-control' id=reply_<?php echo $row['id'];?> name="reply"></textarea>
										</div>
									</div>
								</div>	
						</div>
...
<script>
    ...
    $(document).on("click",".action_btn",function(){
  		...
  		$.ajax({
			type:"post",
			url:"control/adminProcess.php",
			data:{action: 'updatefeedback', id : id,reply : reply},
			success:function(data){
				//alert(data);
				if(data=="1"){
					alert("Reply Submitted Successfully..."); window.location='view_feedback_admin.php';
				}
				else{
					alert("Failed...");
				}
