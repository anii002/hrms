						...
						<div class="tab-content">
							...
							<div class="tab-pane h" id="tab_15_2">
								...
								<div class="form-body ">
									...
									<?php
									$con = dbcon1();
									$sql = mysqli_query($con, "SELECT * from nomination_note where ex_emp_pfno='" . $_GET['ex_emp_pfno'] . "'");
									$res = mysqli_fetch_array($sql);
							...
							<div class="tab-pane h" id="tab_15_3">
								<div class="form-body ">
									<?php
									$con = dbcon1();
									$sql = mysqli_query($con, "SELECT dpo_remark,concern_WI,name from drmpsurh_cga.nomination_note,drmpsurh_sur_railway.register_user where register_user.emp_no=nomination_note.ex_emp_pfno and ex_emp_pfno='" . $_GET['ex_emp_pfno'] . "'");
									$res = mysqli_fetch_array($sql);
									$pf = $res['concern_WI'];
									$con = dbcon2();
									$sql2 = mysqli_query($con, "SELECT name from drmpsurh_sur_railway.register_user where register_user.emp_no='" . $res['concern_WI'] . "'");
							...
							<div class="form-actions right">
								<?php
								$con = dbcon1();
								$sql = mysqli_query($con, "SELECT hold_status,rcc_note_status from forward_application where id='" . $_GET['id'] . "' and ex_emp_pfno='" . $_GET['ex_emp_pfno'] . "'");
								$res = mysqli_fetch_array($sql);
								if ($res['hold_status'] == 1 && $res['rcc_note_status'] == 1) {
...
<div class="modal fade" id="basic" tabindex="-1" role="basic" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<form action="control/adminProcess.php?action=nomiFW_wi" method="post">
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<!-- <select name="fw_to_pfno" id="fw_to_pfno" class="select2me form-control" style="width: 100%;" tabindex="-1" aria-hidden="true" required>
										<option value="" selected disabled>Select Recruitment cell</option>
										  <?php
											// dbcon1();
											// dbcon2();
											// $query_emp =mysqli_query("SELECT register_user.name as name,login.pf_number as pf_number,login.* from drmpsurh_cga.login,drmpsurh_sur_railway.register_user where register_user.emp_no=login.pf_number AND role='7' ");
								...
								<input type="text" class="form-control" name="" value="<?php echo $name; ?>" readonly="">
								<input type="hidden" name="fw_to_pfno" value="<?php echo $pf;  ?>">
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group">
								...
								if ($name != '') {
									...
									echo '<button type="submit" class="btn blue">Forward</button>';
								}
								?>
