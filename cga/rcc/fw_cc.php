			...
			<div class="portlet box blue btnz">
					...
					<div class="row">
													...
													<table  border="1" style="width: 85%;">
														<tbody>
															<?php 
																...
																$sql=mysql_query("SELECT str_to_date(doa,'%d/%m/%Y')as rlyjoindate1,str_to_date(date_of_missing,'%d/%m/%Y')as date_of_missing1,str_to_date(retirementdate,'%d/%m/%Y')as retirementdate1,resgister_user.*  from resgister_user where emp_no='".$_GET['ex_emp_pfno']."' ");
         														$res=mysql_fetch_array($sql);
															?>
															<tr>
																<td style="width: 30px;">1</td>
												...
												<div class="row">	
													...
													<table  border="1" style="width: 85%;">
														<tbody>
															<?php 
															dbcon2();
																//$sql=mysql_query("SELECT str_to_date(doa,'%d/%m/%Y')as rlyjoindate1,str_to_date(retirementdate,'%d/%m/%Y')as retirementdate1,str_to_date(date_of_expiry,'%d/%m/%Y')as date_of_expiry1,prmaemp.* from prmaemp where empno='".$_GET['ex_emp_pfno']."' ");
																$sql=mysql_query("SELECT str_to_date(doa,'%d/%m/%Y')as rlyjoindate1,str_to_date(date_of_expiry,'%d/%m/%Y')as date_of_expiry1,str_to_date(retirementdate,'%d/%m/%Y')as retirementdate1,resgister_user.* from resgister_user where emp_no='".$_GET['ex_emp_pfno']."' ");
         														$res=mysql_fetch_array($sql);
							...
							<div class="form-actions right">
												...
												<?php
												dbcon1();
								$query_emp =mysql_query("SELECT * FROM `forward_application` where  ex_emp_pfno='".$_GET['ex_emp_pfno']."' order by id desc limit 1 ");
									...
									<select name="fw_to_pfno" id="fw_to_pfno" class="select2me form-control" style="width: 100%;" tabindex="-1" aria-hidden="true" required>
										<option value="" selected disabled>Select Cofidential Clerk</option>
										  <?php
											 dbcon2();
											 dbcon1();
											 $query_emp =mysql_query("SELECT name as name,login.pf_number as pf_number,login.* from drmpsurh_cga.login,drmpsurh_sur_railway.resgister_user where resgister_user.emp_no=login.pf_number AND role='6' ");
											 echo mysql_error();
