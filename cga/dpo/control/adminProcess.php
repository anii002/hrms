...
switch ($_REQUEST['action']) {
  ...
  case 'submit_dpo':
    ...
    $sql = mysqli_query($con, "SELECT pf_number,unit_id from login where pf_number='" . $fw_to_pf . "' and role='7'");
  ...
  case 'concern_wi':
    $data = 0;
    $con = dbcon1();
    $sql = mysqli_query($con, "UPDATE nomination_note set status=1,dpo_remark='" . $_POST['remark'] . "',concern_wi='" . $_POST['wi_pfno'] . "' where ex_emp_pfno='" . $_POST['ex_emp_pfno'] . "' and category='" . $_POST['case'] . "'");
    //echo mysqli_error();
  ...
  case 'fw_to_srDpo':
    ...
    //   $sql=mysqli_query("SELECT unit_id from login where pf_number='".$fw_to_pf."'");
    //   $value=mysqli_fetch_array($sql);
  ...
  case 'reject':
    ...
    $update = mysqli_query($con, "UPDATE `forward_application` SET depart_time='" . $date . "',return_status='1',remark='" . $_POST['remark'] . "',rejected_by='" . $rcc . "' where ex_emp_pfno='" . $ex_emp_pfno . "' AND id='" . $fw_tbl_id . "'  ");
    // $re_update=mysqli_query($update);
