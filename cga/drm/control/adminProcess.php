...
switch ($_REQUEST['action']) {
  ...
  case 'approve_drm':
    ...
    $sql = mysqli_query($con,"SELECT pf_number,unit_id from login where role='3'");
  ...
  case 'add_questios':
    ...
    if (move_uploaded_file($tmp_name, $upload_dir)) {
      ...
      $sql = mysqli_query($con,"INSERT INTO `add_syllabus`(`name_of_que_paper`, `year`, `uploaded_date`, `uploaded_by`, `uploaded_file_path`) VALUES('" . $que_name . "','" . $year . "','" . $date . "','" . $_SESSION['admin_id'] . "','" . $dir . "')");
  ...
  case 'RemoveFile':
    ...
    $id = $_REQUEST['id'];
    $data = 0;
    $qu = mysqli_query($con,"SELECT * from add_syllabus where id='" . $id . "'");
  ...
  case 'addRule':
    ...
    if (move_uploaded_file($tmp_name, $upload_dir)) {
      ...
      echo $sql = mysqli_query($con,"INSERT INTO `rules_n_regulations`(`title`, `rules_path`, `created_at`) VALUES ('" . $_POST['title'] . "','" . $dir . "','" . $date . "')");
  ...
  case 'removeRuleFile':
    ...
    $id = $_REQUEST['id'];
    $data = 0;
    $qu = mysqli_query($con,"SELECT * from rules_n_regulations where id='" . $id . "'");
    $res = mysqli_fetch_array($qu);
