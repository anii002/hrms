...
include('adminFunction.php');
  switch($_REQUEST['action'])
  {
    ...
    case 'forward_application':
          ...
          dbcon1();
          $sql=mysql_query("SELECT unit_id from drmpsurh_cga.login where pf_number='".$fw_to."'");
    ...
    case 'applicant_add':
      ...
      dbcon2();
      $sqql=mysql_query("SELECT dob from resgister_user where emp_no='".$ex_emp_pf."'");
      $r_sqql=mysql_fetch_array($sqql);
      ...
      dbcon1();
       $sql1=("INSERT INTO `applicant_registration`(`ex_emp_pfno`, `ex_empname`, `ex_empdept`, `ex_empdesig`, `applicant_name`, `applicant_dob`, `applicant_mobno`, `applicant_emailid`, `applicant_panno`, `applicant_aadharno`, `username`, `psw`, `applicant_gender`, `applicant_qualifiaction`, `mariatial_status`,`permanent_address`,`identification_mark1`,`identification_mark2`, `category`, `caste`, `eligible_category`, `status`, `fw_status`, `created_at`,`relation_with`,`added_by`,`remark`,`status_add_data`,`ss_status`) VALUES('".$ex_emp_pf."','".$ex_empname."','".$ex_department."','".$ex_design."','".$appl_name."','".$aapl_dob."','".$appl_mobile."','','','','".$_POST['username']."','".hashPassword($password,SALT1,SALT2)."','".$appl_gender."','','','','','','".$category."','','','0','0','".$date."','','".$_SESSION['unitid']."','','','')");
        ...
        dbcon2();
        $ins_user_p=mysql_query("SELECT pf_num from user_permission where pf_num='".$ex_emp_pf."'");
        $check=mysql_num_rows($ins_user_p);
        if($check == 0)
        {
        ...
        {
          $update=mysql_query("UPDATE user_permission set cga='0' where pf_num='".$ex_emp_pf."' ");
      ...
      dbcon1();
         $sql=mysql_query("UPDATE `applicant_registration` SET `applicant_name`='".$_POST['appl_name']."',`applicant_dob`='".$_POST['aapl_dob']."',`applicant_mobno`='".$_POST['appl_mobile']."',`applicant_gender`='".$_POST['appl_gender']."',`category`='".$_POST['category']."' where ex_emp_pfno='".$_POST['p_emp_pfno']."' and username='".$_POST['p_username']."'");
