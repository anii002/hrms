...
<!--Content page--->
					...
					<table id="example1" class="table table-bordered table-hover">
							...
							$month=$month2[0];
								...
								$query = "SELECT taentryform_master.reference, taentryform_master.TAYear, taentryform_master.TAMonth,taentryform_master.created_at, SUM(taentryforms.distance) AS distance, SUM(taentryforms.amount) as rate FROM taentryform_master INNER JOIN taentryforms ON taentryform_master.reference = taentryforms.reference_id WHERE taentryforms.reference_id in (select taentryform_master.reference from taentryform_master where taentryform_master.empid='".$_SESSION['empid']."') AND TAYear='".date('Y')."' AND TAMonth like '%".$month."%' GROUP BY taentryforms.reference_id";
									...
									while($val = mysql_fetch_array($result))
									{
										...
										<tr>
											<td>".$val['reference']."</td>
											<td>".get_employee($_SESSION['empid'])."</td>
											<td>".$val['TAYear']."</td>
											<td>".$val['TAMonth']."</td>
											<td>".$val['distance']."</td>
											<td>".$val['rate']."</td>
											...
											<td>
											<a href='show_seperate_claim1.php?id=".$val['reference']."' class='btn btn-primary'>Show</a></td>
										</tr>
										";
									}
							?>
						...
						//				";
							...
							<tbody>
								...
								// {
								// 	$query = "SELECT MONTHNAME( taentryform_master.created_at ) as created, taentryform_master.reference, taentryform_master.TAYear, taentryform_master.empid, taentryform_master.TAMonth, SUM(taentryforms.distance) AS distance, SUM(taentryforms.amount) as rate FROM taentryform_master INNER JOIN taentryforms ON taentryform_master.reference = taentryforms.reference_id WHERE taentryform_master.reference IN (select reference_id  from forward_data where forward_data.fowarded_to='".$_SESSION['empid']."' and admin_approve='1' and admin_returned_status='0' and Month(taentryform_master.created_at)='".$month."') group by taentryform_master.reference";
							           
								// }
								// else
								// {
								// 	$query = "SELECT MONTHNAME( taentryform_master.created_at ) as created, taentryform_master.reference, taentryform_master.TAYear, taentryform_master.empid, taentryform_master.TAMonth, SUM(taentryforms.distance) AS distance, SUM(taentryforms.amount) as rate FROM taentryform_master INNER JOIN taentryforms ON taentryform_master.reference = taentryforms.reference_id WHERE taentryform_master.reference IN (select reference_id  from forward_data where forward_data.fowarded_to='".$_SESSION['empid']."' AND forward_data.depart_time != '' and Month(taentryform_master.created_at)='".$month."') group by taentryform_master.reference";
								// }
								// //echo $query;
								// 	$result = mysql_query($query);
 ...
 <?php
	require_once("../../global/officer_footer.php");
 ?>
