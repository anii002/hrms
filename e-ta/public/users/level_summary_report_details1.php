...
$get_month = explode("-",$_REQUEST['id']);
	...
	<?php
				...
				<div class="box box-info">
					<!--span>For Reject TA press <i class='fa fa-close btn btn-sm btn-danger' style='width: 25px !important;padding: 0px !important' aria-hidden='true'></i> , & For View TA Details press <i class='fa fa-eye btn-sm btn btn-primary' style='width: 25px !important;padding: 0px !important' aria-hidden='true'></i></span-->
					<div class="box-body">
		 ...
		 <table id="" class="table table-hover" style="page-break-inside: inherit; border-collapse:collapse" width="100%" border="1">
							...
							if(isset($_SESSION['billunit']))
							{
			                    ...
			                    { 
									...
									else if($count == '1')
									{
										...
										echo "<td style='width:40px; border-right:1px solid white; text-align:center'>$total</td>";										
									}
									else{	}
									$query1 = "SELECT e.pfno, e.name, e.desig, e.bp, ta.reference_id, SUM(ta.amount) as sum from employees as e INNER JOIN taentryforms as ta ON e.pfno = ta.empid WHERE ta.reference_id IN(SELECT reference FROM tbl_summary_details where summary_id = '".$_REQUEST['id']."' and reject_pending='0' and empid='".$val['pfno']."') GROUP BY ta.reference_id order by ta.taDate";
									$result1 = mysql_query($query1);
									...
									while($res = mysql_fetch_array($result1))
									{
										...
										if(mysql_affected_rows() > 0)
										{
											while($res_data = mysql_fetch_array($res1))
											{
												if($_SESSION['role']=='6')
												{
													$data .= "<button class='btn btn-danger btn-sm return_btn hide-print' value='" . $res['reference_id']."' data-toggle='modal' emp='".$val['pfno']."' data-target='#return' >Return </button>";
			...
			?>
				...
				<div class="row">
					...
					{
						 echo "<button class='btn btn-success' style='margin-left:10px;' data-toggle='modal' data-target='#forward'>Approve and Forward</button>";
					}else if($hold_status=='1' && $_SESSION['role']=='5' || $_SESSION['role']=='6') {
							if($hold_status == '1' && $_SESSION['role'] == '6'){
					...
					//echo "<button class='btn btn-info hide-print' style='margin-left:10px;' onclick='winprint();' >Print</button>"; ?>
				</div>
			</div>
						</div>
					</div>
      ...
      <div class="modal-body" style="padding:20px;">
        ...
        <div class="col-xs-7">
            <select name="forwardName" class="form-control" style="width: 100%;display:none;" required="required">
              <?php 
                $query = "SELECT department.deptname,users.empid as pfno,role FROM `users`,department where users.dept=department.dept_id AND role=11 AND users.dept=(SELECT dept_id from employees,department WHERE employees.dept=department.deptname AND pfno='$emp_id')";
                $result = mysql_query($query);
                $value = mysql_fetch_array($result);

                $query1 = mysql_query("SELECT name,desig,pfno from employees where pfno='".$value['pfno']."'");
                while($value1 = mysql_fetch_array($query1))
                {
                  echo "<option selected='selected' value='".$value1['pfno']."'>".$value1['name']."</option>";
                }
              ?>
            </select>
