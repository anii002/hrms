	...
	<?php
				...
				<div class="box box-info">
					...
					<div class="box-body">
						...
						<thead>
							<tr id="tr1">
								<th colspan="15"  class=" remove_border">
							...
							if(in_array($val['BU'], $temp))
							{
			                    ...
			                    { 
		                			...
		                			if($count == '3'){
		                				...
		                				echo "<td style='width:10px; text-align:center'>$total</td>";
									}else if($count == '2')
									{
										...
										echo "<td style='width:10px; border-right:1px solid white; text-align:center'>$total</td>";										
									}else{}
							...
							//print_r($ref_data);
			                    ...
			                    { 
		                			...
		                			if($count == '3'){
		                				...
		                				echo "<!--td style='width:40px; border-right:1px solid white'></td--><td style='width:10px; text-align:center'>$total</td>";
									}else if($count == '2')
									{
										...
										echo "<!--td style='width:40px; border-right:1px solid white'></td--><td style='width:10px; border-right:1px solid white; text-align:center'>$total</td>";										
									}else{}
									...
									{
										$cont_total = 0;
										$sql = "SELECT * FROM forward_data WHERE hold_status = '1' AND depart_time = '' AND reference_id = '".$res['reference_id']."'";
										$res1 = mysql_query($sql);
										if($res1){
											while($res_data = mysql_fetch_array($res1))
											{
												...
												if($_SESSION['role']=='5')
												{
												$data .= "<button class='btn btn-danger hide-print reject_btn hide-print' value='".$res['reference_id']."' data-toggle='modal' emp='".$val['pfno']."' data-target='#ReturntoEst' >Reject </button>";
												$data .= "<a style='margin-left:5px;' class='btn btn-primary hide-print' href='show_seperate_claim.php?id=".$res['reference_id']."'>view </a>";
												}
												if($_SESSION['role']=='2')
												{
												$data .= "<a class='btn btn-primary hide-print' href='show_seperate_claim.php?id=".$res['reference_id']."'>view </a>";
		...
		//							$sum = $sum + (int)$val['sum'];
			...
			echo '<tr>
				<td colspan="9" style="text-align:right; font-size:16px; font-weight:bold; padding-right:60px">Total</td>
			...
			<!--table id="" class="table table-bordered table-hover">
							...
							<tbody-->
								...
								//print_r($temp);
								$query = "SELECT e.BU, e.pfno, e.name, e.desig, e.bp, ta.reference_id, SUM(ta.amount) as sum from employees as e INNER JOIN taentryforms as ta ON e.pfno = ta.empid WHERE ta.reference_id IN(SELECT reference FROM tbl_summary_details where summary_id = '".$_REQUEST['id']."' AND reject_pending='0') GROUP BY ta.reference_id";
									//echo $query;
									$result = mysql_query($query);
									...
									while($val = mysql_fetch_array($result))
									{
										...
										//echo "select fowarded_to from forward_data where reference_id='".$val['reference_id']."' order by id desc";
										$innerquery = mysql_query("select fowarded_to from forward_data where reference_id='".$val['reference_id']."' order by id desc");
										$resultinner = mysql_fetch_array($innerquery);
										//echo $resultinner['fowarded_to']."<br/>";
										if(!isset($_SESSION['billunit'])){
											...
											if($resultinner['fowarded_to'] == $_SESSION['empid'])		//
											{
												...
												<td>$cnt</td>
												<td>".$val['pfno']."</td>
												<td>".$val['name'].",BU:".$val['BU']."</td>
												<td>".$val['desig']."</td>
												<td>".$val['bp']."</td>
												<td>".$val['sum']."</td>"; 
												$temp_total = $val['sum'] + $temp_total;
												if($_SESSION['role']=='6')
												{
											...
											if(in_array($val['BU'], $temp))
											{
												array_push($data, $val['reference_id']);
												$ref = $val['reference_id'];
												/*echo "
												<tr>
												<td>$cnt</td>
								...
								<!--tr>
									...
									<td style="font-size:16px; font-weight:bold;"><?php if(isset($_SESSION['billunit'])){ echo $temp_total;}else{ echo $sum; } ?></td>
								</tr>
