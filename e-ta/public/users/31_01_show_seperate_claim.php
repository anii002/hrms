...
if(isset($_REQUEST['id']))
{
  ...
  ?>
                ...
                <tbody>
                  ...
                  function get_employee($id)
                  {
                    ...
                    while($val_first = mysql_fetch_array($result_first))
                    {
						...
						$pfno=substr($_REQUEST['id'], 0, -12);
						$query_bu=mysql_query("SELECT BU from employees where pfno='$pfno'");
						$result_bu = mysql_fetch_assoc($query_bu);
						$bill_unit=$result_bu['BU'];
      ...
      <div class="row">
        ...
        <?php 
		if($bill_unit == '0107001' || $bill_unit == '0107002'){
         echo "<a href='tracking_TA.php?id=".$_REQUEST['id']."' class='btn btn-primary pull-right' style='margin-right:10px;'>Track</a>";
		}else{
			echo "<a href='tracking_TA_officers.php?id=".$_REQUEST['id']."' class='btn btn-primary pull-right' style='margin-right:10px;'>Track</a>";
		}
		  ...
		  if($value_select>0 && $role!=3 && $role!=2 && $role!=5 && $role!=6)
		  {
			 ?>
		...
		{
			if($value_empid['admin_approve']!='1' && $value_select>0 && $role!=2 && $role!=5 && $role!=6)
			{
		?>
