							...
							<tbody>
								<?php
									...
									while($val = mysql_fetch_assoc($result))
									{
										...
										if(!array_key_exists($pf_no, $ref_array)){
											$ref_array[$pf_no] = '';
											$array = $val['reference_id'];
											$ref_array[$pf_no] = $array;
										}else{
											if(is_null($ref_array[$pf_no])){
												$temp = $val['reference_id'];
												$ref_array[$pf_no] = $temp;	
											}else{
												$temp = $ref_array[$pf_no];
												$temp = $val['reference_id'];
												$ref_array[$pf_no] = $temp;
											}
			...
			{	
				print_r($ref_array);
                    ...
                    { 
						...
						echo "<br/><br/>";
                    	/*foreach ($percent_value as $percenta => $per_val) 
                    	{*/
							...
							echo $keys_v[0];
							$count = count($keys);
                    		//$result  = implode(',' ,$percent_value);
                    		//echo $result;
                    		/**/
                    		echo "<table border='1' width='100%'>";
                    		for($i = 0; $i < $count; $i++){
                    			if($keys[$i] == "p100"){ $hund = $result_show['cnt']; }else{ $hund = '';}
                    			if($keys[$i] == "p100"){ $hund_amount = $keys_v[$i]; }else{ $hund_amount = ''; }
                    			/*if($percenta == '70%'){ $hund = $result_show['cnt']; }if($percenta == "70%"){ $seven_amount = $per_val; }
                    			if($percenta == '30%'){ $hund = $result_show['cnt']; }if($percenta == "30%"){ $thirty_amount = $per_val; }	
*/
