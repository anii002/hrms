					...
					{	
	                    ...
	                    { 
                			...
                			if($count == '3'){
                				...
                				echo "<td style='width:40px; border-right:1px solid grey'>$total_counter</td><td style='width:80px'>$total</td>";
							}else if($count == '2')
							{
								$total = '';
                				$total_counter = '';
								if($keys[$count-$r] == "p100")
								{
									...
									$total_counter = $total_counter + $counter;
										echo"<td style='width:40px; border-right:1px solid grey'>".$counter."</td><td valign='middle' style='width:80px; border-right:1px solid grey'>".$keys_v[$count-$r]."</td>";
										echo "<td style='width:40px;  border-right:1px solid grey'>&emsp;</td><td style='width:80px; border-right:1px solid grey'>&emsp;</td>";
									}else{
										echo "<td style='width:40px; border-right:1px solid grey'>&emsp;</td><td style='width:80px; border-right:1px solid grey'>&emsp;</td>";
										$counter = $keys_v[$count-$r]/($amount1*0.3);
										$total = $total + $keys_v[$count-$r];
								...
								else if($keys[$count-$r] == "p70")
								{
									...
									$total_counter = $total_counter + $counter;
										echo"<td style='width:40px; border-right:1px solid grey'>".$counter."</td><td valign='middle' style='width:80px; border-right:1px solid grey'>".$keys_v[$count-$r]."</td>";
										echo "<td style='width:40px; border-right:1px solid grey'>&emsp;</td><td style='width:80px'>&emsp;</td>";
									}else{
										$r = $r + 1;
										echo "<td style='width:40px; border-right:1px solid grey'>&emsp;</td><td style='width:80px'>&emsp;</td>";
										$counter = $keys_v[$count-$r]/($amount1*0.7);
								...
								if($keys[$count-$r] == "p100"){
									...
									$total_counter = $total_counter + $counter;
									echo"<td style='width:40px; border-right:1px solid grey'>".$counter."</td><td valign='middle' style='width:80px; border-right:1px solid grey'>".$keys_v[$count-$r]."</td>";
									echo "<td style='width:40px; border-right:1px solid grey'>&emsp;</td><td valign='middle' style='width:80px; border-right:1px solid grey'>&emsp;</td>";
									echo "<td style='width:40px; border-right:1px solid grey'>&emsp;</td><td valign='middle' style='width:80px; border-right:1px solid grey'>&emsp;</td>";	
								}else if($keys[$count-$r] == "p70"){
									$counter = $keys_v[$count-$r]/($amount1*0.7);
									$total = $total + $keys_v[$count-$r];
								...
								echo "<td style='width:40px; border-right:1px solid grey'>$total_counter</td><td style='width:80px; border-right:1px solid grey'>$total</td>";										
							}else{}
							$query1 = "SELECT e.pfno, e.name, e.desig, e.bp, ta.reference_id, SUM(ta.amount) as sum from employees as e INNER JOIN taentryforms as ta ON e.pfno = ta.empid WHERE ta.reference_id IN(SELECT reference FROM tbl_summary_details where summary_id = '".$_REQUEST['id']."' and reject_pending='0' and empid='".$val['pfno']."') GROUP BY ta.reference_id";
							$result1 = mysql_query($query1);
							$total = 0;
							...
							while($res = mysql_fetch_array($result1))
							{
								...
								$cont_data .= "<span>".$cont_total."</span><br/>";

   							}
       						$total_sum = $total_sum + $total;
							
							if($dummy == 0){
								echo "<td  rowspan='3' style='width:100px; border-right:1px solid grey; border-left: 1px solid grey'>$cont_data</td>";
...
//							$sum = $sum + (int)$val['sum'];
			...
			echo '<tr>
				<td colspan="15" style="text-align:right; font-size:16px; font-weight:bold;">Total</td>
				<td style="font-size:16px; font-weight:bold;" class="text-center">'.$cont_sum.'</td>
				<td style="font-size:16px; font-weight:bold;" class="text-center">'.$total_sum_ta.'</td>
