					...
					<div class="row" id="hide1" style="margin-bottom:10px;">
						...
						?>
							...
							<?php
											...
											if($innerresult['hold_status']=="0" || $innerresult['hold_status']=="1")
											{
												...
												while($row = mysql_fetch_array($data))
												{
													...
													if(!array_key_exists($pf_no, $associative_emp)){
														...
														if($val['reference_id'] == '503NPS06312/2018/598707' && $percent == "p100"){
                                                              //print_r($associative_percentage);
                                                        }
													}else{
														if(!array_key_exists($month, $associative_emp[$pf_no])){
															...
															if(array_key_exists($percent, $temp_array)){
																$associative_percentage = array();
																$associative_month = array();
																$get_val = $temp_array[$percent];
																$temp_array[$percent] = $get_val + $amount;
																...
																if($val['reference_id'] == '503NPS06312/2018/598707' && $percent == "p100"){
																    //print_r($temp_array);
																    //print_r($associative_percentage);
                                                                    //echo $associative_percentage[$percent];
                                                                    //$cn++;
                                                                }
															}
														}
												...
												//echo $array_100."#".$val['reference_id'].",";
												$array = $array_100.",".$array_70.",".$array_30;
												$count_array = explode(',', $array);

	                      						$query1 = "SELECT e.pfno, e.name, e.desig, e.bp, ta.reference_id, SUM(ta.amount) as sum from employees as e INNER JOIN taentryforms as ta ON e.pfno = ta.empid WHERE ta.reference_id IN(SELECT reference FROM tbl_summary_details where summary_id = '".$_REQUEST['id']."' and empid='".$val['pfno']."') GROUP BY ta.reference_id";
									...
									//echo "<br/>".$cn."<hr/>".print_r($associative_emp)."<br/><hr>".print_r($array);
								 ?>
			...
			?>
					...
					<div class="row">
						<div class="col-xs-offset-11 col-xs-1 pull-right">
							<?php echo "<button class='btn btn-success' style='margin-left:10px;' data-toggle='modal' data-target='#forward'>Send</button>"; ?>
						</div>
					</div>
					<?php 
						}
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header" style="border-bottom-color: white;">
        ...
        <h2>Forward Summary Report to Account </h2>
      </div>
      ...
      <div class="modal-body" style="padding:20px;">
        <div class="form-group">
          ...
          <input type="hidden" name="loginid" value="<?php echo $_SESSION['empid']; ?>">
        ...
        <div class="col-xs-7">
           <select name="forwardName" class="form-control select2 required" required="required" style="width: 100%">
            	<option readonly value='' disabled selected>Select User</option>
              <?php 
                //$query = "SELECT * FROM employees where pfno IN ( select empid from users where status='1' AND role='5')";
              //echo $query = "SELECT * FROM employees e INNER JOIN users u ON u.empid = e.pfno where u.status='1' AND u.role='5'";
               $query = "SELECT * FROM employees e INNER JOIN users u ON u.empid = e.pfno where u.status='1' AND u.role='5'";
                $result = mysql_query($query);
                while($value = mysql_fetch_array($result))
                {
                  echo "<option value='".$value['empid']."'>".$value['name']."  (".$value['desig'].")</option>";
                }
              ?>
            </select>
        </div>
      ...
      <input type="hidden" name="loginid" id="loginid" value="<?php echo $_SESSION['empid']; ?>">
