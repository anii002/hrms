					...
					if($val['pfno'] == $key && $pf_cnt == '1')
					{
								...
								while ($rw = mysql_fetch_array($result1)) {
									if($val['pfno'] == $rw['pfno']){
										...
										$date = substr($monthName, 0, 3)."-".substr($rw['TAYear'], 2, 3);
								...
								//print_r($array);
								$imp = implode("<br/>", $array);
								$d .= "<span style='line-height: 1.6'>".$imp."<span><br/>";
								echo"<td  style='font-size:17px'><table>".$da."</table></td>
								<td>".$d."</td>";           //Claimed Month
								$d = '';
								echo "<td colspan='10'>";
			...
			?>
					...
					<div class="row">
						<div class="col-xs-offset-10 col-xs-2 pull-right">
							<br/>
							<?php echo "<button class='btn btn-success hide-print' style='margin-left:10px;' id='btnclick' onclick='PrintPreview()'>Print</button>"; ?>
						</div>
					</div>
					</div>
					<!-- /.box-body -->
