  ...
  $expl = explode(",",$empl_id_result['TAMonth']);
	...
	?>		<div class=""  id="info1">
        ...
        <div style="position:absolute;left:875px;top:45px;font-size:10px;" class="cls_005"><span class="cls_005">G 69 F/G 69 F/A </span></div>
				...
				<div class="row">
				<label class="" style="font-size:13px;font-weight:100;margin-left:20px;">शाखा/Branch &nbsp;<span style="border-bottom:1px solid black;"><b><?php echo $emp_result['dept']; ?></b></span>&nbsp;&nbsp;&nbsp;&nbsp;मंडल/जिला/Divison/Dist.&nbsp;&nbsp;<span style="border-bottom:1px solid black"><b>SR. DFM's OFFICE</b></span>&nbsp;&nbsp;&nbsp;मुख्यालय /Headquarters at&nbsp;<span style="border-bottom:1px solid black"><b>SUR</b></span>&nbsp;&nbsp; द्वारा किये गये कार्यो का जर्नल, जिनके बारे मैं &nbsp;<span style="border-bottom:1px solid black"><b><?php foreach($expl as $val)
         {
            echo $years[$val-1].", ";
         }
         ...
         echo " - ".$empl_id_result['TAYear']; ?></b></span> &nbsp;&nbsp; 20 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  is claimed.</label><label style="font-size:13px;font-weight:100;margin-left:20px;">पदनाम /Designation&nbsp;&nbsp;&nbsp;<span style="border-bottom:1px solid black"><b><?php echo $emp_result['desig']; ?></b></span>&nbsp;&nbsp;&nbsp;वेतन/Pay  &nbsp;&nbsp;&nbsp;<span style="border-bottom:1px solid black"><b><?php echo $emp_result['bp']; ?></b></span>&nbsp;</label>
			...
			<div class="row">		</div>
               ...
               <table id="" class="table table-bordered table-hover table-condensed">
                <thead>
                <tr>
                  <th style="line-height: 1.6">Cardpass / <br/>Token </th>
                  <th>Date</th>
                  <th>Train no.</th>
                  <th>Jaurney Type</th>
                  <th style="line-height: 1.6">Depart <br/>Station</th>
                  <th style="line-height: 1.6">Depart <br/>Time</th>
                  <th style="line-height: 1.6">Arrival <br/>Station</th>
                  <th style="line-height: 1.6">Arrival <br/>Time</th>
                  <th style="line-height: 1.6">Rate</th>
                  <th style="line-height: 1.6">Claim %</th>
                  <th style="line-height: 1.7">Objective</th>
                    ...
                    while($val_first = mysql_fetch_array($result_first))
                    {
                        ...
                        while($val = mysql_fetch_array($result))
                        {
                          ...
                          if($cnt == 1)
                            ...
                            <td>".$val['percent'];
							if($val['leave_info']=='training')
								echo "<span title='Training'>&nbsp;&nbsp;<i class='fa fa-university'></i></span>";
							if($val['leave_info']=='Exam')
								echo "<span title='Exam'>&nbsp;&nbsp;<i class='fa fa-pencil-square-o'></i></span>";
							if($val['leave_info']=='BD')
								echo "<span title='Break Down'>&nbsp;&nbsp;<i class='fa fa-chain-broken'></i></span>";
							if($val['leave_info']=='Leave')
								echo "<span title='Leave'>&nbsp;&nbsp;<i class='fa fa-bed'></i></span>";
							echo "</td>";
                           if($cnt=='1')
                            {
                                ...
                                {
                                  echo "<br/><a href='add_contigency.php?claim=".$val['reference']."&set=".$val_first['set_number']."' class='btn btn-danger hide_print'>Add Contingency </a>";
                                }
                                echo "</td>";
								}else{
									$anotherquery = mysql_query("select count(id) as total from continjency_master where reference='".$val['reference']."' and set_number='".$val_first['set_number']."'");
                                ...
                                {
                                  echo "<td rowspan='$rows' class='hide_print'>
								  <button value='".$val["reference"]."' cnt='".$val_first["set_number"]."' class='classBtn btn btn-success' data-toggle='modal' data-target='#myModal'  style='margin-top:20px; margin-left:15px' id='view_cont'>View Contijency</button>";
...
मैं प्रमाणित करता हूं कि ड्यूटी पास की गयी यात्रा तथा विराम के बारे मैं जिसके लिए इस बिल मैं यात्रा भत्ता/दैनिक  भत्ता मांगा गया हैं,किसी भी स्त्रोत से कोई यात्रा भत्ता या अन्य पारिश्रमिक नहीं लिया गया हैं।   </p>
      ...
      <div class="row">
        ...
        {
          echo "<a href='tracking_TA.php?id=".$_REQUEST['id']."' id='track' class='btn btn-primary pull-right' style='margin-right:10px;'>Track</a>";
          $sql = "SELECT * FROM forward_data WHERE reference_id = '".$_REQUEST['id']."' ORDER BY id DESC LIMIT 1";
          $res = mysql_query($sql);
          //echo mysql_affected_rows();
          while($row = mysql_fetch_array($res)){
            
            if($row['hold_status'] != '0'){
              echo "<button id='getback' class='btn btn-primary pull-right' style='margin-right:10px;' value='".$_REQUEST['id']."'>Get Back</button>";
            }
          }
...
<div id="forward" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <form action='control/adminProcess.php?action=forward_TA' method="post" id="for_user">
        ...
        <div class="col-xs-7">
            <select name="forwardName" id="forwardName" class="form-control select2 required" style="width: 100%" required>
              ...
              if(!isset($_SESSION['role'])){
                  $query = "SELECT * FROM employees where pfno IN ( select empid from users where status='1' AND role = 'BO') AND BU != '0107002' AND BU != '0107001' AND BU != ''";
                }else{
                  $query = "SELECT * FROM employees where pfno IN ( select empid from users where status='1' AND role = 'OS') AND BU != '0107002' AND BU != '0107001' AND BU != ''";
                }
                $result = mysql_query($query);
...
<!-- View Modal -->
                ...
                <div class="modal-dialog">
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      ...
                      <form action="update_cont.php" method="post">
                    ...
                    <div class="modal-footer">
                        <input type="hidden" name="claim" id="hide_reference"/>
                        <input type="hidden" name="set" id="hide_setNumber"/>
 ...
 <script type="text/javascript">
   ...
   $(document).on("click","#getback",function(){
      var check = confirm("Are you sure, you want to get back this forwarded TA");
      var id = $(this).val();
      if(check)
      {
          $.ajax({
            url: 'control/adminProcess.php?action=getbackclaimedta',
            type: 'POST',
            data: {id: id},
          })
          .done(function(html) {
            if(html=="true"){
              alert('Please check unclaimed TA list');
              window.location='Show_unclaimed_TA.php';
            }
            else
            {
