                  ...
                  ?>
                  	...
                  	<table class="table table-inverse table-condensed">
                  		...
                  		echo "<tr>
                  			...
                  			<td><input type='text' required class='$cnt departClass' style='width:100px; text-transform:uppercase' list='ddepart' name='departS$cnt' id='departS$cnt' placeholder='Station' ><datalist id='ddepart'>";
                  		...
                  		<?php
                  			echo "<td><input type='text' class='$cnt arrivalClass' style='width:100px; text-transform:uppercase' list='adepart' name='arrivalS$cnt' id='arrivalS$cnt' placeholder='Station'><datalist id='adepart'>";
                  			$sql = "SELECT * FROM stations";
                  			$query = mysql_query($sql);
                  			echo mysql_error();
                  			while($row = mysql_fetch_array($query)){
    	...
    	return new Date(year, month, 0).getDate();
					...
					$(document).ready(function(){
						$("#add_row").on("click",function(){
							...
							var markup = "<tr><td><input type='date' style='width:130px;border:none;' name='date"+cnt+"' id='date"+cnt+"' value='"+newmonth+"' class='"+cnt+" selectdate' required></td>                  			<td><input type='text' style='width:70px;' name='train"+cnt+"' placeholder='Train no.' class='"+cnt+" train_no' value='"+journey_type+"' onChange='trainnumber(this)'></td>                  			<td><input type='text' class='"+cnt+" departClass' style='width:100px; text-transform:uppercase' name='departS"+cnt+"' id='departS"+cnt+"' placeholder='Station' list='ddepartS"+cnt+"'><datalist id='ddepartS"+cnt+"'>";
							markup += "<?php
                  			$sql = "SELECT * FROM stations";
                  			$query = mysql_query($sql);
                  			while($row = mysql_fetch_array($query)){
							...
							?>";
							markup += "</datalist></td><td><input type='text' style='width:45px;' class='dt' name='departT"+cnt+"' tempid='"+cnt+"' id='departT"+cnt+"' data-inputmask ='\"mask\": \"99:99\"' data-mask></td><td><input type='text' class='"+cnt+" arrivalClass' style='width:100px; text-transform:uppercase' name='arrivalS"+cnt+"' id='arrivalS"+cnt+"' placeholder='Station' list='adepartS"+cnt+"'><datalist id='adepartS"+cnt+"'>";
							markup += "<?php
                  			$sql = "SELECT * FROM stations";
                  			$query = mysql_query($sql);
                  			while($row = mysql_fetch_array($query)){
