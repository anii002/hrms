...
date_default_timezone_set("Asia/Kolkata");
	...
	if($date_count==0)
	{
		...
		for($i=0;$i<=$hide_count;$i++)
		{
			...
			if($_REQUEST['misc'.$i]=='None')
			{
				...
				if($i==$hide_count)
				{
					if($_REQUEST['arrivalS'.$i] == "" )
					{	
						if($i != 0)
						{
							$currDate = $_REQUEST['date'.$i];
							$preDate = $_REQUEST['date'.$i-1];
							$dteDiff  = $currDate->diff($preDate);
							if($dteDiff != 0)
							{
						...
						else
						{
							...
							{
								$amount=0;
								echo "You already get 100% for this date1";
							}
		...
		for($i=0;$i<=$hide_count;$i++)
		{
			...
			//echo "Count = ".$i_count;
				...
				else if($i<=$hide_count)
				{
					...
					if($_REQUEST['date'.$tmp]!=$begin_Date)
					{
						...
						if($temp_date[$i-1] != $begin_Date){
					  		if($temp_station[$i-1] != $temp_arri_station[$i])
					  		{
					  			//echo $temp_date[$i]."<br/>".$temp_arri_station[$i];
					  			$valid = 0;
					  			if($beginT=="")
									$beginT="00:00";
								if($endT=="" || $i<$hide_count)
						...
						$query = "select SUM(percent) AS SUM from taentryforms where empid='".$_REQUEST['empid']."' and taDate='$date'";
						  ...
						  else
						  {
							$sum= 0;
							  ...
							  if($sum==0)
							  {
							  	//echo $temp_date[$i-1]."=".$begin_Date."=".$beginT."<br/>";
			...
			else if($_REQUEST['misc'.$i]=='Exam')
			{
				...
				if($i==0)
				{
					...
					{
					$strStart = $_REQUEST['date0']." ".$_REQUEST['departT0']; 
					if($_REQUEST['arrivalT0'] == ""){
						$arrivalT0 = "23:59";
					}else{
						$arrivalT0 = $_REQUEST['arrivalT0'];
					}
					$strEnd   = $_REQUEST['date0']." ".$arrivalT0;
					//$strEnd   = $_REQUEST['date0']." ".$_REQUEST['arrivalT0']; 
					$dteStart = new DateTime($strStart); 
					$dteEnd   = new DateTime($strEnd); 
					$dteDiff  = $dteStart->diff($dteEnd); 
	...
	echo "<table border='1' style='width:100%'><tbody>";
