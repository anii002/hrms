...
if(isset($_REQUEST['id']))
{
  ...
  ?>
      ...
      <div class="row">
        ...
        echo "<a href='#' class='btn btn-primary pull-right' style='margin-right:10px;'>Track</a>";
         $query_select = "SELECT * FROM `forward_data` WHERE reference_id='".$_REQUEST['id']."' AND depart_time is null AND fowarded_to='".$_SESSION['empid']."'";
          ...
          $query_select1 = "SELECT dept FROM `users` WHERE empid='".$_SESSION['empid']."'";
          $result_select1 = mysql_query($query_select1);
          echo mysql_error();
          $value_empid1 = mysql_fetch_array($result_select1);
          $dept=$value_empid1['dept'];
          $query_dept = mysql_query("select empid from users where dept='".$dept."' and role='11'");
          $result_dept = mysql_fetch_array($query_dept);
          $deptadmin = $result_dept['empid'];

		  if($value_select>0 && $role == "12")
		  {
			 ?>
		...
		{
			if($value_empid['admin_approve']!='1' && $value_select>0 && $role=="13")
			{
		?>
		<form action="control/adminProcess.php?action=adminapprove" method="post">
    <input type="hidden" name="empid1" value="<?php echo $_SESSION['empid']; ?>">
		<input type="hidden" name="empid" value="<?php echo $value_empid['empid']; ?>">
          <input type="hidden" name="original_id" value="<?php echo $deptadmin; ?>">
          <input type="hidden" name="ref" value="<?php echo $_REQUEST['id']; ?>">
      ...
      <div class="modal-body" style="padding:20px;">
        ...
        <div class="col-xs-7">
            <!--select name="forwardName" class="form-control select2 required" required="required" style="width: 100%">
              ...
              <?php 
               // $query = "SELECT * FROM employees where pfno IN ( select empid from users where status='1' AND role='13')";
                //$result = mysql_query($query);
