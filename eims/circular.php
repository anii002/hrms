					...
					<div class="portlet-body">
						...
						<table id="example1" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
									<th colspan="6">
										<center>CIRCULAR DETAILS - 2019</center>
									</th>
								</tr>
								<tr>
									<th>Sr No</th>
							...
							<tbody>
								...
								$counter = 0;
								// dbcon1();
								$qry = mysqli_query($conn1, "SELECT * FROM `circular` ORDER BY id DESC");
...
<script type="text/javascript">
	$(document).on('click', '.deletecircular', function() {
		var id = $(this).val();
		// alert(id);
		var result = confirm("Confirm!!! Proceed for Circular delete?");
		if (result == true) {
			$.ajax({
					url: 'control/adminProcess.php',
					type: 'POST',
					data: {
						action: 'deletecircular',
						id: id
					},
				})
				.done(function(html) {
					alert(html);
					window.location = "circular.php";
				});
		}
	});
</script>