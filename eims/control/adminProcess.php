...
include('adminFunction.php');
  ...
  break;
    ...
    break;
       ...
       case 'seniority':
          ...
          $file_size= array();
           ...
           $filepath = "http://drmpsur-hrms.in/eims/control/".$dir.$file;
   ...
   case 'enotification':
    ...
    case 'deletecircular':
        $delete_id = $_REQUEST['id'];
          if(deletecircular($delete_id))
            echo "Circular Deleted successfully";
          else
            echo "Something went wrong";
    break;
    ...
    case 'checklist':
      ...
      if(move_uploaded_file($_FILES["attach"]["tmp_name"],$dir.$file))
      {
            ...
            dbcon1();
             $query = mysql_query("INSERT INTO `checklist`(`subject`, `attach`) VALUES ('$subject','$filepath')");
            if($query)
            {
               echo "<script>alert('Checklist Submitted successfully');window.location='../checklist.php';</script>"; 
            }
            else
            {
    ...
    else
    {
      ...
      if(move_uploaded_file($_FILES["attach"]["tmp_name"],$dir.$file))
      {
            ...
            dbcon1();
             $query = mysql_query("INSERT INTO `photo_gallary`(`phototitle`, `attach`) VALUES ('$subject','$filepath')");
            if($query)
            {
               echo "<script>alert('Photo Submitted successfully');window.location='../Photo_Gallary.php';</script>"; 
            }
            else
            {
               echo "<script>alert('Not Submitted successfully');</script>"; 
  ...
  case 'transfer_registration':
      ...
      if(move_uploaded_file($_FILES["attach"]["tmp_name"],$dir.$file))
      {
            ...
            dbcon1();
             $query = mysql_query("INSERT INTO `transfer_registration`(`reg_title`,`department`, `attach`) VALUES ('$reg_title','$department','$filepath')");
            if($query)
            {
               echo "<script>alert('Transfer Registration Submitted successfully');window.location='../transfer_registration.php';</script>"; 
            }
            else
            {
               echo "<script>alert('Not Submitted successfully');window.location='../transfer_registration.php';</script>"; 
            } 
    ...
    else
    {
       ...
       case 'deletechecklist':
        $delete_id = $_REQUEST['id'];
          if(deletechecklist($delete_id))
            echo "Checklist Deleted successfully";
          else
            echo "Something went wrong";
    break;

    case 'deletePhoto_Gallary':
        $delete_id = $_REQUEST['id'];
          if(deletePhoto_Gallary($delete_id))
