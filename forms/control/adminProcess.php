...
switch ($_REQUEST['action']) {
  ...
  case 'addForms':
    ...
    if (move_uploaded_file($tmp_name, $upload_dir)) {
      ...
      $sql = mysqli_query($con1, "INSERT INTO `forms`(`title`, `file_path`, `created_by`) VALUES ('" . $_POST['title'] . "','" . $dir . "','" . $date . "')");
  ...
  case 'removeFile':
    ...
    $id = $_REQUEST['id'];
    $data = 0;
    $qu = mysqli_query($con1,"SELECT * from forms where id='" . $id . "'");
  ...
  case 'deleteclaim':
    ...
    $result = mysqli_query($con1,$query_delete);
  ...
  case 'view_contingency':
    ...
    $raw_data = mysqli_query($con1,$sql);
  ...
  case 'seniority':
    ...
    if (move_uploaded_file($_FILES["attach"]["tmp_name"], $dir . $file)) {
      ...
      $con1=dbcon1();
      $query = mysqli_query($con1,"INSERT INTO `seniority_list`(`subject`, `url`, `year`) VALUES ('$subject','$filepath','$year')");
      if ($query == TRUE) {
  ...
  case 'seniority_sugg':
    ...
    $con1=dbcon1();
    $query = mysqli_query($con1,"INSERT INTO `suggestion`(`suggestion`, `pfno`, `status`, `created_date`) VALUES ('$suggestion','$pfno','0','$date')");
    if ($query == TRUE) {
  ...
  case 'enotification':
    ...
    if (move_uploaded_file($_FILES["attach"]["tmp_name"], $dir . $file)) {
      ...
      $con1=dbcon1();
      $query = mysqli_query($con1,"INSERT INTO `e-notification1`(`ref_no`, `notification_date`, `subject`, `url`) VALUES ('$ref_no','$notification_date','$subject','$filepath')");
      if ($query == TRUE) {
  ...
  case 'circular':
    ...
    if (move_uploaded_file($_FILES["attach"]["tmp_name"], $dir . $file)) {
      ...
      $con1=dbcon1();
      $query = mysqli_query($con1,"INSERT INTO `circular`(`issued_date`, `issued_section`, `subject`, `file`) VALUES ('$issued_date','$issued_section','$subject','$filepath')");
      if ($query) {
  ...
  case 'updatesuggestion':
    ...
    $data = "0";
    $con1=dbcon1();
    $sql = mysqli_query($con1,"UPDATE `suggestion` SET `remark`='" . $suggestion . "',`status`='1',`updated_date`='" . $date . "' WHERE id='" . $id . "'");
    if ($sql) {
      $data = "1";
    } else {
      $data = "0";
