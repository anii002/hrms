...
function fetchpostschedule1($id) 
	{
		...
		if (!empty($id)) 
		{
			...
			$sql = "SELECT * FROM `post_schedule` WHERE `id` = '".$id."' AND updated_by='$ename'";
			$query = mysql_query($sql) or trigger_error("Query Failed: " . mysql_error());
			if($sql)
			{
			while($res=mysql_fetch_array($query))
	...
	function update_postschedule($hide_field,$post_dept,$post_category,$post_level,$post_fill,$post_ss,$post_mor,$post_vac,$post_vacinhg,$post_trg,$post_netvac,$post_dateass,$post_dateass2,$post_datenot,$post_datenot2,$post_exam,$post_dateexam,$post_dateofpanel,$post_dateofpanel2,$post_plan)
     {
	 $query = mysql_query("update post_schedule set dept='$post_dept',category='$post_category',level_of_pay='$post_level',mode_of_filling='$post_fill',ss='$post_ss',mor='$post_mor',vac='$post_vac',vac_in_hg='$post_vacinhg',utrg='$post_trg',netvac='$post_netvac',date_of_ass='$post_dateass',completed_ass_date='$post_dateass2',date_of_noti='$post_datenot',completed_date_of_noti='$post_datenot2',date_of_exam='$post_exam',completed_date_exam='$post_dateexam',date_of_panel='$post_dateofpanel',completed_date_panel='$post_dateofpanel2',action_plan='$post_plan' where id='$hide_field'");
	...
	function update_designation($hide_field,$update_zone,$update_dept,$update_desg)
     {
	 $query = mysql_query("update designation set zone ='$update_zone',dept='$update_dept',desg='$update_desg' where id='$hide_field'");
	 if($query)
	 {
		 return true;
	 }
	...
	{
		$query = mysql_query("delete from users where id='$delete_id'");
		if($query)
			return true;
		else{ 
			return false;
	...
	{
		$query = mysql_query("delete from designation where id='$delete_id'");
		if($query)
			return true;
		else{ 
			return false;
...
function checkpassword($pass,$user)
	{
		$query =  mysql_query("select * from users where password='$pass' AND username='$user'");
