...
if(isset($_REQUEST['action']))
{
	...
	case 'get_custom_rpt':
		...
		if($sel3!="")
			$array[2] = " date_of_ass='$sel3'";
		if($sel4!="")
			$array[3] = " date_of_exam='$sel4'";
		if($sel5!="")
			$array[4] = " date_of_panel='$sel5'";
		if($sel6!="")
			$array[5] = " date_of_noti='$sel6'";
		$newquery =""; $cnt=	0;
		for($i=0;$i<=5;$i++)
		{
			//echo $i;
		...
		//print_r($array);
		//$new_query = str_replace("#"," AND",$query);
		///echo $new_query; 
		...
		$dept=$_POST['dept'];
		 ...
		 case 'add_designation':
		   ...
		   $desg=$_POST['desi_cat'];
			...
			$sql=mysql_query("insert into category(categary,dept)values('$desg','$desg_dept')");
			if($sql){
				echo "<script>window.location='category.php';alert('Category added Successfully');</script>";
			}else{
				echo "<script>window.location='category.php';alert('NOT Added, please try again');</script>";
			}
		break;
	...
	case 'changeprofile':
     ...
     case 'users_form':
		 ...
		 $emp_password=$_POST['user_emp_password'];
		  $sql=mysql_query("insert into users(zone,division,pf_no,emp_name,dept,desg,mob_no,user_email,username,password,date_time,status)values(' $zone','$division','$pf_number','$emp_name','$emp_dept','$emp_desg','$emp_mob','$emp_email','$emp_username','$emp_password',NOW(),1)");
		  
		  
		  
		  echo "insert into users(zone,division,pf_no,emp_name,dept,desg,mob_no,user_email,username,password,date_time,status)values(' $zone','$division','$pf_number','$emp_name','$emp_dept','$emp_desg','$emp_mob','$emp_email','$emp_username','$emp_password',NOW(),1)".mysql_error();
