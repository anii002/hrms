	...
	//echo "<script>alert('$data')</script>";	
		...
		if (!empty($id)) 
		{
			...
			while($res=mysql_fetch_array($query))
			{
				...
				$data['noti_ref']=$res['notification_ref'];
				$data['panel_ref']=$res['panel_ref'];
			}
...
function fetchpostschedule1($id) 
	...
	function update_postschedule($hide_field,$post_dateass2,$post_datenot2,$noti_ref,$post_dateexam,$post_dateofpanel2,$panel_ref,$post_plan)
     {
     	$sql = "update post_schedule set completed_ass_date='$post_dateass2',completed_date_of_noti='$post_datenot2',notification_ref='$noti_ref',completed_date_exam='$post_dateexam',completed_date_panel='$post_dateofpanel2',panel_ref='$panel_ref',action_plan='$post_plan' where id='$hide_field'";
	 $query = mysql_query($sql);
	 //echo $sql;
	 //echo mysql_error();
	...
	function update_designation($hide_field,$update_desg,$update_dept)
     {
	 $query = mysql_query("update category_new set categary='$update_desg',dept='$update_dept' where id='$hide_field'");
	 if($query)
	 {
		 return true;
	 }
	...
	{
		$query = mysql_query("delete from category_new where id='$delete_id'");
		if($query)
			return true;
		else{ 
			return false;
